 ...
 * full federal CIP-2020 list: one search box (that both filters the list and
 * surfaces plain-English "closest matches"), and each code's certified CTE
 * category, definition, examples, family, and a course-level C-ID/CCN floor.
 * COE hosts the actual TOP↔CIP crosswalk (linked out) — per the CO's reframe
 * (Jenni Abbott, AA), TOP→CIP is one-to-many and we are the reference, not a
 * crosswalk recreator. No "suggest" form (CO direction).
 *
...
(function () {
  ...
  var wrapEl, inputRef, pillsRef, famRef, cbRef, xferRef, listHost, countHost, suggestHost;
  ...
  // ── theme (self-contained; scoped to .cipx so the dashboard is untouched) ────
  ...
  // the "closest matches" strip only helps for descriptive text — skip it for
  // a bare code lookup (where the live list filter is the answer).
  function renderFinder(q) {
    ...
    hits.forEach(function (h) {
      ...
      if (h.matched.length) card.appendChild(el("div", { class: "cipx-sug-why" }, ["matched: " + h.matched.join(", ")]));
  ...
  function header() {
    ...
    // light/dark toggle (top-right) — themes ONLY this tab (scoped .cipx tokens)
  ...
  // ONE consolidated control panel: the search/finder input + filters + chips +
  // the count/CSV, all in a single light rounded box (Sam's consolidation).
  function buildPanel() {
  ...
  // ── CSS (scoped under .cipx; the tab carries its OWN light/dark tokens, no :root) ─
  function ensureCss() {
    if (document.getElementById(CSS_ID)) return;
    var css = [
      // ── light tokens (default) live ON .cipx so the toggle can flip them without
      //    touching the dashboard's :root ──
      ".cipx{" +
      ...
      // the single consolidated control panel (light bg, rounded box)
      ".cipx-panel{background:var(--cipx-surface-2);border:1px solid var(--cipx-border);border-radius:14px;padding:16px 18px;margin:16px 0 14px;}",
      ...
      // suggestions strip (the "closest matches" — plain-English finder output)
      ...
      ".cipx-sug-why{font-size:.74rem;color:var(--cipx-muted);padding:0 14px 9px 122px;}",
      ".cipx-sug-card .cipx-detail{padding:0 14px 16px 122px;}",
      ...
      // muted / earthy category badges — additive, not attention-grabbing
      ...
      "@media (max-width:640px){.cipx{padding:6px 12px 24px;}.cipx-row,.cipx-sug-crow{grid-template-columns:14px 66px 1fr auto;gap:8px;}" +
        ".cipx-detail,.cipx-sug-why,.cipx-sug-card .cipx-detail{padding-left:14px;}}",
    ].join("");
