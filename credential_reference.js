...
(function () {
  ...
  function ensureCerScopeCss() {
    ...
    st.textContent =
      ...
      "#tab-credential-reference .cr-wl-toggle-on{background:var(--navy-primary);color:#fff;border-color:var(--navy-primary);font-weight:600;}" +
  ...
  function openWorklist() {
    ...
    if (!state.unclassified && !state.unclassLoading) {
      ...
      Promise.all([fetchUnclassified(), fetchUnclassOverlay(), fetchUnclassSuggestions(), fetchUnclassPreseed()]).then(function (parts) {
  ...
  function renderWorklist() {
    ...
    // ⚡ staged pre-seeds among the SHOWN, still-unassigned rows.
    var preseeded = shown.filter(function (it) {
      var a = state.unclassAssign[it.raw_title];
      return !(a && a.title) && state.unclassPreseed[it.raw_title];
    });
    if (preseeded.length) {
      ...
      if (state.sess) {
        var saveAll = el("button", { type: "button", class: "cr-wl-saveall" },
          ["💾 Save all pre-filled shown (" + preseeded.length + ")"]);
        saveAll.onclick = function () { bulkSavePreseeded(saveAll); };
  ...
  // Bulk-save the currently SHOWN pre-filled rows. Reads each row's INPUTS
  // (what-you-see-is-what-saves — a hand-edited prefill saves as edited);
  // skips rows already assigned or with an emptied title.
  function bulkSavePreseeded(btn) {
    var rows = Array.prototype.slice.call(
      document.querySelectorAll(".cr-wl-row.cr-wl-preseeded"));
    var todo = [];
    ...
    if (!window.confirm("Save " + todo.length + " pre-filled assignment"
  ...
  function renderWorklistRow(it) {
    ...
    var titleInp = el("input", { class: "cr-wl-input cr-wl-title-input", type: "text",
      ...
      value: cur.title || (ps && ps.title) || "", autocomplete: "off" });
    ...
    var issInp = el("input", { class: "cr-wl-input cr-wl-iss-input", type: "text",
      ...
      value: cur.issuer || (ps && ps.issuer) || "", autocomplete: "off" });
