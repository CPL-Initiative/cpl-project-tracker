...
/* The exhibit universe answers to its own switches: the six CPL types (and the
 * seventh state, none recorded — the same shape as `unrec`), the articulation
 * pair, statewide or not, and the ring of local exhibits. The two lists share
 * `show`, so a switch set in one universe is still set in the other. */
var EX_TYPE_KEYS=["cbe","icert","port","stdz","mil","oth"];
var SHOW_KEYS_EX=EX_TYPE_KEYS.concat(["tna","arty","noart","sw","nsw","members"]);
var ALL_SHOW_KEYS=SHOW_KEYS.concat(SHOW_KEYS_EX.filter(function(k){ return SHOW_KEYS.indexOf(k)<0; }));
function showKeys(){ return isExhibits() ? SHOW_KEYS_EX : SHOW_KEYS; }
var show={}; ALL_SHOW_KEYS.forEach(function(k){ show[k]=true; });
/* One set of switches PER UNIVERSE. `members`, `arty` and `noart` are keys in
 * both menus, so a shared object would let Deselect all on the CPL map turn
 * the course map's college courses off behind the reader's back. bindUniverse
 * parks the leaving universe's switches here and restores the arriving one's. */
var showMemo={}, litMemo={};
function freshShow(){ var o={}; ALL_SHOW_KEYS.forEach(function(k){ o[k]=true; }); return o; }
...
var SHOW_WORDS={cr:"CR \u2014 credit", nc:"NC \u2014 noncredit", nce:"NCE \u2014 noncredit enhanced", unrec:"Credit status not recorded",
  ...
  cbe:"Credit By Exam", icert:"Industry Certification", port:"Portfolio Review", stdz:"Standardized Assessment",
  mil:"Military", oth:"Other", tna:"CPL type not recorded", sw:"Statewide", nsw:"Not statewide"};
...
/* ══ THE EXHIBIT UNIVERSE — "CPL mode" (Sam's ruling, 2026-09-10) ═════════════
 * "It's like another universe where the entities are exhibits rather than
 * courses, but just like in course view, circles around the exhibits can
 * indicate that courses are articulated to them." One point per curated
 * credential (a CER unified title), its local MAP exhibits as the stars that
 * ring it, its discipline as its island, the Articulations light on the ones
 * courses are articulated to, and a second ring on the statewide ones. The
 * word CPL in the row switches UNIVERSES: the two payloads
 * (kb/_build_ccr_cpl_universe.py, kb/_build_ccr_cpl_universe_members.py) are
 * fetched on demand and cached beside the course universe, so switching back
 * is a re-bind, never a reload.
 *
 * ⚠️ THIS SUPERSEDES THE CPL FACE OF S238 (course points renamed by the
 * credential that reaches them). Sam dismissed "courses colored by CPL type"
 * for this on 2026-09-10; the face's own helpers (cplLabelLines, cplTipHtml,
 * cplSuggestRows, cplIslandHtml) are retired from every path but kept in the
 * file for one round in case a ruling wants a piece of them back. What stays
 * live from that payload: the course card's credential block, the exhibit
 * names the chip prints, and the outline's CPL layer — all on the Courses map.
 *
 * ⚠️ ONE RENDERER, TWO SHAPES. The exhibit point keeps i/x/y/t/n so every
 * shared path (hit-testing, the halo, the batching, the label placer, the
 * ring of members) runs unchanged; what differs is WORDS — the card, the
 * hover, the label's second line, the legend, the Show menu — and every one
 * of those asks isExhibits() rather than reading a field that happens to be
 * absent. `nd.c` is a LIST of CPL type codes here, not a credit status. */
var EXU_URL = window.CPL_SKYVIEW_EXU_URL || "ccr_cpl_universe.json";
var EXM_URL = window.CPL_SKYVIEW_EXM_URL || "ccr_cpl_universe_members.json";
var universe="courses";                     // which universe U is: "courses" | "cpl" (exhibits)
var UNIVERSES={courses:null, cpl:null};     // {u, mem} per universe, once bound
var exState="", exWaiters=[], exCourses=null, rebound=false, turnBefore=null;   // rebound: a payload swap that the next __ccrUniverse must render; turnBefore: whether the sky was turning when it swapped  // exCourses: identity -> the course identities articulated to it
var pendingOpen=null;                       // an identity to open once the other universe is drawn (the course jump)
function isExhibits(){ return universe==="cpl"; }
function exTypeWord(i){ var T=(U&&U.cpl_types)||[]; return T[i]||"CPL type not recorded"; }
function loadExhibits(then){
  if(exState==="ok"||exState==="missing"||exState==="blocked") return then&&then();
  if(window.CPL_CCR_EXHIBITS && window.CPL_CCR_EXHIBITS.islands){   // a harness inlines the payloads
    UNIVERSES.cpl={u:window.CPL_CCR_EXHIBITS, mem:window.CPL_CCR_EXHIBITS_MEMBERS||null};
    exState="ok"; return then&&then();
  }
  if(then) exWaiters.push(then);
  if(exState==="loading") return;
  var flush=function(){ var w=exWaiters; exWaiters=[]; w.forEach(function(f){ try{ f(); }catch(e){} }); };
  if(typeof fetch!=="function"){ exState="missing"; flush(); return; }
  exState="loading";
  var get=function(u){ return fetch(u).then(function(r){ if(!r.ok) throw new Error("http "+r.status); return r.json(); }); };
  /* The members are a second file so the layout stays small; a universe with
   * no members still draws, and its card says the payload is missing rather
   * than claiming a credential folds nothing. */
  get(EXU_URL).then(function(u){
    return get(EXM_URL).then(function(m){ return {u:u, mem:m}; }, function(){ return {u:u, mem:null}; });
  }).then(function(pair){ UNIVERSES.cpl=pair; exState="ok"; flush(); })
    .catch(function(){ exState=(location.protocol==="file:")?"blocked":"missing"; flush(); });
}
/* The pure swap: which payload U is, and every memo that belonged to the
 * universe leaving. No rendering — setUniverse() re-enters __ccrUniverse, and
 * the workspace binds Courses back before it paints its tables. */
function bindUniverse(kind){
  var pick=UNIVERSES[kind]; if(!pick) return false;
  showMemo[universe]=show; show=showMemo[kind]||freshShow();
  litMemo[universe]=lit; if(!(kind in litMemo)) litMemo[kind]=(kind==="cpl"); lit=litMemo[kind];   // CPL opens lit (Sam: "circles around the exhibits"); Courses keeps the reader's choice
  universe=kind; face=kind; rebound=true; turnBefore=rotating;
  window.CPL_CCR_UNIVERSE=pick.u; window.CPL_CCR_UNIVERSE_MEMBERS=pick.mem;
  U=pick.u; A=window.CPL_ATLAS_DATA||null; spreadUniverse(U);
  exCourses=(kind==="cpl" && pick.mem && pick.mem.courses) ? pick.mem.courses : null;
  selNode=null; selIsl=null; hoverNode=null; hoverIsl=null; searchHits=[]; searchTerm="";
  tokens=[]; drag=null; memFilter=""; parkedCam=null; isolate=false;
  roster=null; nodeIdx=null; orbitIdx=null; subjIdx=null; wsPaint=null; islSphere=null;
  U.islands.forEach(function(I){ I._s=null; });
  isoDirty();
  return true;
}
function exWords(){
  if(exState==="ok" && isExhibits()) return "<strong>CPL</strong>: "+exLineText()+
    " Search finds credentials and the local exhibits folded into them; the light marks the ones that carry an articulation.";
  if(exState==="loading"||exState==="") return "Loading the exhibit universe\u2026";
  if(exState==="blocked") return "The exhibit universe cannot be loaded from a file:// page \u2014 serve the page to open CPL.";
  return "The exhibit universe could not be loaded (ccr_cpl_universe.json), so the courses stay on the map.";
}
function exLineText(){
  if(!isExhibits() || !U || !U.counts) return exWords();
  var c=U.counts;
  return num(c.identities)+" credential"+(c.identities===1?"":"s")+" folding "+num(c.members)+" local MAP exhibit"+(c.members===1?"":"s")+" across "+num(c.disciplines)+
    " discipline"+(c.disciplines===1?"":"s")+"; "+num(c.articulated)+(c.articulated===1?" carries":" carry")+" an articulation and "+num(c.statewide)+(c.statewide===1?" is":" are")+" statewide"+
    (c.no_discipline?"; "+num(c.no_discipline)+" wait for a discipline":"")+".";
}
function exLineHtml(){
  if(exState!=="ok" || !isExhibits() || !U || !U.counts) return esc(exWords());
  var c=U.counts;
  return "<strong>"+num(c.identities)+"</strong> credential"+(c.identities===1?"":"s")+" folding "+num(c.members)+" local MAP exhibit"+(c.members===1?"":"s")+" across "+
    num(c.disciplines)+" discipline"+(c.disciplines===1?"":"s")+"; <strong>"+num(c.articulated)+"</strong>"+(c.articulated===1?" carries":" carry")+" an articulation and <strong>"+
    num(c.statewide)+"</strong>"+(c.statewide===1?" is":" are")+" statewide"+(c.no_discipline?"; "+num(c.no_discipline)+" wait for a discipline":"")+".";
}
/* The hash follows the WORD, not the payload: a reader who pressed CPL and
 * reloads while it loads should land on CPL. syncHash is a no-op off the map. */
function syncFaceHash(){
  if(curView!=="skyview" && curView!=="comprehensive") return;
  curArg=(face==="cpl")?"cpl":""; syncHash(curArg||undefined);
}
function setUniverse(kind, quiet, opts){
  kind = kind==="cpl" ? "cpl" : "courses";
  if(kind===universe){ face=kind; paintFace(); syncFaceHash(); return; }
  if(!UNIVERSES.courses) UNIVERSES.courses={u:window.CPL_CCR_UNIVERSE, mem:window.CPL_CCR_UNIVERSE_MEMBERS||null};
  if(kind==="cpl" && !UNIVERSES.cpl){
    /* ⚠️ A ROUTE MUST STILL DRAW. __ccrUniverse returns to us before it renders
     * when the face is not the bound universe, so a #skyview/cpl arrival whose
     * payload never comes has to be handed back as the courses, with the
     * route's other options intact — or the page is blank with a hint. */
    var fallBack=function(){
      face="courses"; paintFace(); syncFaceHash(); setHint(exWords());
      if(opts) window.__ccrUniverse(Object.assign({}, opts, {face:"courses"}));
    };
    if(exState==="missing"||exState==="blocked"){ fallBack(); return; }
    if(!quiet) setHint(exWords());
    face="cpl"; paintFace(); syncFaceHash();  // the word reads pressed while the payload is on its way
    loadExhibits(function(){
      if(UNIVERSES.cpl) setUniverse("cpl", quiet, opts);
      else fallBack();
    });
    return;
  }
  bindUniverse(kind);
  if(opts || (cvs && document.getElementById("u-cvs")===cvs)){
    window.__ccrUniverse(Object.assign({}, opts||{}, {face:kind}));
    renderTokens();                          // the chip host lives in the borrowed form and outlives a render
    if(!quiet) setHint(kind==="cpl" ? exWords()
      : "<strong>Courses</strong>: the course universe again. Switch to <strong>CPL</strong> for the credentials and their exhibits.");
  }
}
window.__ccrSetUniverse=function(k, quiet){ setUniverse(k, quiet==null?true:!!quiet); };
/* The legend says what the universe on screen draws. Both keep #u-lg-lit, so
 * the light's row folds and unfolds the same way in each. */
function legendHtml(){
  if(isExhibits()) return (   // ⚠️ the paren: `return` then a newline returns undefined (ASI), and the legend read "undefined"
    '<span title="A curated credential from the Credential Reference: one unified title folding every local MAP exhibit that names it."><i class="u-sw s0"></i>credential \u2014 a curated CER title; bigger with more local exhibits folded in</span>'+
    '<span id="u-lg-lit" hidden><i class="u-sw lit"></i>lit \u2014 a gold glow on a credential that courses are articulated to; the rest are drawn as they are</span>'+
    '<span title="MAP marks the exhibit as statewide."><i class="u-sw sw"></i>statewide \u2014 a second ring around the credential</span>'+
    '<span><i class="u-sw member"></i>local MAP exhibit under a credential \u2014 a small star; click or hover a credential to open it</span>');
  return '<span title="'+esc(SYSWHY[0])+'"><i class="u-sw s0"></i>M-ID, our working label</span>'+
...
/* What an exhibit label says at this zoom: the credential, "statewide" where it
 * is, and on the full band the issuer with the counts. */
function exLabelLines(nd, k){
  var head=trunc(nd.t||nd.i, k>TITLE_ZOOM?44:28)+(nd.sw?" \u00b7 statewide":"");
  if(k>FULL_ZOOM)
    return {band:"full", lines:[head, trunc(nd.g||"issuing agency not recorded", 48)+" \u00b7 "+num(nd.n)+" local exhibit"+(nd.n===1?"":"s")+
      (nd.ar?" \u00b7 "+num(nd.ar)+" articulation"+(nd.ar===1?"":"s"):"")]};
  if(k>TITLE_ZOOM) return {band:"titled", lines:[head]};
  return {band:"brief", lines:[head]};
}
function exTipHtml(hit){
  var nd=hit.nd, types=(nd.c||[]).map(exTypeWord);
  return '<b>'+esc(nd.t||nd.i)+'</b>'+(nd.sw?' <span class="sub">\u00b7 statewide</span>':'')+
    '<br><span class="sub">'+esc(nd.g||"Issuing agency not recorded")+'</span>'+
    '<br><span class="sub">'+(types.length?esc(types.join(" \u00b7 "))+' \u00b7 ':'')+num(nd.n)+' local exhibit'+(nd.n===1?'':'s')+
      (nd.ar?' \u00b7 '+num(nd.ar)+' articulation'+(nd.ar===1?'':'s'):' \u00b7 no articulation yet')+
      (nd.ss?' \u00b7 '+num(nd.ss)+' students served':'')+(hit.isl?' \u00b7 '+esc(hit.isl.d):'')+'</span>';
}
function exMemberTipHtml(hit){
  var m=hit.mem;
  return '<b>'+esc(m.n)+'</b><br><span class="sub">a local MAP exhibit folded into '+esc(hit.nd.t||hit.nd.i)+
    (m.conf!=null?' \u00b7 confidence '+Math.round(m.conf*100)+'%':'')+(m.q?' \u00b7 '+esc(String(m.q).replace(/_/g," ")):'')+'</span>';
}
/* The exhibit card: the credential, its issuer and types, the COURSES
 * articulated to it (Sam: "show the course(s), perhaps on the exhibit cards,
 * that have CPL based on the exhibit"), each a door onto the Courses map, and
 * the local MAP exhibits the Credential Reference folds into it. Nothing here
 * moves: exhibits are curated in the CER, not on this map. */
function renderExhibitNode(){
  var nd=selNode, isl=selIsl, el=document.getElementById("u-detail"); if(!nd||!el) return;
  var mine=membersOf(nd.i), types=(nd.c||[]).map(exTypeWord), crs=(exCourses&&exCourses[nd.i])||[];
  var h=(isl?'<p class="sub" style="margin:0 0 .4em"><button type="button" class="linkish" id="u-back-isl">Back to '+esc(isl.d)+'</button></p>':"")+
    "<h3>"+esc(nd.t||nd.i)+"</h3>"+
    "<p>"+chipFor(nd)+(isl?" \u00b7 "+esc(isl.d):"")+" \u00b7 "+num(nd.n)+" local exhibit"+(nd.n===1?"":"s")+
      (nd.ar?" \u00b7 "+num(nd.ar)+" articulation"+(nd.ar===1?"":"s"):"")+"</p>"+
    '<p class="sub">'+(nd.g?'Issued by <strong>'+esc(nd.g)+'</strong>':'Issuing agency not recorded')+
      (types.length?' \u00b7 '+esc(types.join(" \u00b7 ")):'')+(nd.ss?' \u00b7 '+num(nd.ss)+' students served':'')+'</p>';
  h+='<h4 style="margin:.9em 0 .3em">Courses articulated to it ('+num(crs.length)+' course identit'+(crs.length===1?'y':'ies')+
    (nd.ar?' \u00b7 '+num(nd.ar)+' receiving college course'+(nd.ar===1?'':'s'):'')+')</h4>';
  if(crs.length){
    h+='<p class="sub">Course identities on the Courses map that carry an articulation from this credential \u2014 the ring counts the receiving college courses under them. Open one to see it there.</p>'+
      '<ul class="idlist">'+crs.slice(0,40).map(function(c){
        var loc=c[4]||[];
        return '<li><button type="button" class="ttl linkish" data-course="'+esc(c[0])+'" title="Open this course on the Courses map">'+esc(c[2]||c[0])+'</button> '+
          '<span class="chip '+(c[1]==="M-ID"?"gen":"cid")+'">'+esc(c[1]||"")+'</span>'+
          '<div class="sub">'+esc(c[0])+(c[3]?" \u00b7 "+esc(c[3]):"")+" \u00b7 "+num(loc.length)+" receiving college course"+(loc.length===1?"":"s")+
          (loc.length?": "+esc(loc.slice(0,3).map(function(l){ return l[0]+(l[2]&&l[2].length?" ("+shortCollege(l[2][0])+(l[2].length>1?" +"+(l[2].length-1):"")+")":""); }).join(", "))+(loc.length>3?" \u2026":""):"")+'</div></li>';
      }).join("")+'</ul>'+(crs.length>40?'<p class="sub">Showing 40 of '+num(crs.length)+'.</p>':"");
  } else if(exCourses) {
    h+='<p class="empty">No course is articulated to this credential yet \u2014 a gap in MAP\u2019s record, not a finding about the credential.</p>';
  } else {
    h+='<p class="empty">The articulated courses cannot be listed \u2014 ccr_cpl_universe_members.json did not load. This is not the same as a credential no course is articulated to.</p>';
  }
  h+='<h4 style="margin:.9em 0 .3em">Local MAP exhibits folded in ('+num(mine.length)+')</h4>';
  if(!roster || !Object.keys(roster).length)
    h+='<p class="empty">No member payload loaded \u2014 ccr_cpl_universe_members.json is missing, so the local exhibits cannot be listed. This is not the same as a credential folding none.</p>';
  else if(!mine.length) h+='<p class="empty">No local exhibit is carried under this credential.</p>';
  else h+='<p class="sub">The titles colleges typed into MAP that the Credential Reference folds into this one credential, best confidence first.</p>'+
    '<ul class="mlist">'+mine.slice(0,MEMBER_PAGE).map(function(m){
      return '<li><span class="cd" title="A local MAP exhibit">'+esc(m.n)+'</span>'+
        (m.conf!=null?'<span class="un" title="How confidently the Credential Reference folds this title into the credential">'+Math.round(m.conf*100)+'%</span>':'')+
        (m.q?' <span class="chip mut" title="A quality flag from the Credential Reference">'+esc(String(m.q).replace(/_/g," "))+'</span>':'')+'</li>';
    }).join("")+'</ul>'+(mine.length>MEMBER_PAGE?'<p class="sub">Showing '+MEMBER_PAGE+' of '+num(mine.length)+'.</p>':"");
  h+='<p class="empty" style="margin-top:.6em">Exhibits are curated in the Credential Reference; nothing is moved from this map.</p>';
  el.innerHTML=h;
  var bk=document.getElementById("u-back-isl");
  if(bk) bk.addEventListener("click", function(){ selNode=null; showIsland(isl); draw(); });
  Array.prototype.forEach.call(el.querySelectorAll("[data-course]"), function(b){
    b.addEventListener("click", function(){ jumpToCourse(b.dataset.course, nd); });
  });
}
function jumpToCourse(cid, fromEx){
  pendingOpen={id:cid, words:"Opened <strong>"+esc(cid)+"</strong> on the Courses map \u2014 one of the courses articulated to <strong>"+
    esc(fromEx.t||fromEx.i)+"</strong>. Press <strong>CPL</strong> to return to the credentials."};
  setUniverse("courses", true, {solo:solo});
}
...
function sysPal(nd){
  /* A credential carries no identity system; it takes the working-label
   * color, the way the CER is our working vocabulary for exhibits. */
  var i=isExhibits() ? 0 : (nd.s===0||nd.s===1||nd.s===2)?nd.s:3, w=SYS[i];
...
/* An exhibit passes when one of its CPL types is on (none recorded takes its
 * own switch), its articulation state is on, and its statewide state is on. */
function exTypeOK(nd){
  var c=nd.c; if(!c || !c.length) return !!show.tna;
  for(var i=0;i<c.length;i++) if(show[EX_TYPE_KEYS[c[i]]||"oth"]) return true;
  return false;
}
function swOK(nd){ return nd.sw ? !!show.sw : !!show.nsw; }
function creditShown(nd){
  if(isExhibits()) return exTypeOK(nd) && artOK(nd) && swOK(nd) && isoNodeOK(nd);
  return creditOK(nd) && systemOK(nd) && kindOK(nd) && artOK(nd) && cteOK(nd) && isoNodeOK(nd);
}
 ...
 * on a frame. */
function showSig(){
  var K=showKeys(), t=universe+":"; for(var i=0;i<K.length;i++) t+=show[K[i]]?"1":"0";
...
function showNeeds(nd){
  var need={};
  if(isExhibits()){
    var c=nd.c; need[(c&&c.length)?(EX_TYPE_KEYS[c[0]]||"oth"):"tna"]=true;
    need[nd.sw?"sw":"nsw"]=true; need[nd.ar>0?"arty":"noart"]=true; need.members=true;
    return need;
  }
...
function healShow(nd){
  ...
  showKeys().forEach(function(k){ if(need[k] && !show[k]){ patch[k]=true; turned.push(k); } });
  showHealed=turned;
...
function healUnion(nodes){
  ...
  var patch={}, turned=[];
  showKeys().forEach(function(key){ if(need[key] && !show[key]){ patch[key]=true; turned.push(key); } });
  showHealed=turned;
  if(turned.length && typeof window.__ccrSetShow==="function") window.__ccrSetShow(patch, true);
  return turned;
}
...
function kindShort(kind, nd){
  if(isExhibits()) return kind==="subject" ? "DISC" : kind==="member" ? "LOCAL EXHIBIT" : kind==="term" ? "SEARCH" : "EXHIBIT";
...
function buildMemberIndex(){
  ...
  if(MEM && MEM.m && isExhibits()){
    /* The exhibit universe's members are LOCAL MAP EXHIBITS — [title,
     * confidence, quality flag] — with no control number and no college. The
     * key is minted here per identity so the ring, the hover and the search
     * index work unchanged; canMove refuses every one of them, so nothing
     * written here can ever read as a CN: row. */
    Object.keys(MEM.m).forEach(function(id){
      roster[id]=MEM.m[id].map(function(r, k){
        var rec={cn:"EX:"+id+"#"+k, d:"", n:r[0]||"", c:"", conf:r[1], q:r[2]||"", ex:true};
        noteCourse(rec.cn, rec);
        byCn[rec.cn]=rec; cnHome[rec.cn]=id;
        memIndex.push({id:id, cn:rec.cn, d:"", code:rec.n, lc:rec.n.toLowerCase(), c:"local exhibit"});
        return rec;
      });
    });
    exCourses=MEM.courses||null;
    memberSource="exhibits";
    return;
  }
  if(MEM && MEM.m){
...
function sharedKeyReason(cn, code, others){
  if(isExhibits()) return "Exhibits are curated in the Credential Reference; nothing is moved from the CPL map. "+
    "Switch to <strong>Courses</strong> to move a course.";
...
function canMove(cn){
  if(isExhibits()) return {ok:false, others:[], exhibits:true};
  var l=coursesOn(cn);
...
function drawFrame(){
  ...
  var starBatches={}, starOrder=[], allShown=showKeys().every(function(kk){ return show[kk]; });
  ...
  U.islands.forEach(function(isl){
    ...
    if(showNodes){
      ...
      isl.p.forEach(function(nd){
        ...
        /* A statewide exhibit wears a second ring (Sam, 2026-09-10: "shown
         * visibly on the sky") — a mark, not a color, so it reads by day and by
         * night; the word rides the label and the card. */
        if(isExhibits() && nd.sw && dr>1.8){
          ctx.beginPath(); ctx.arc(p[0],p[1],dr+3.2,0,6.2832);
          ctx.lineWidth=1.1; ctx.strokeStyle=s[1]; ctx.stroke();
        }
...
function drawMembers(nd, isl, p, rad, k, queue, focus){
  ...
  if(rest>0){
    ...
    var more="and "+num(rest)+" more "+(isExhibits()?"local exhibit":"college course")+(rest===1?"":"s")+" — see the details panel";
...
function labelLines(nd, k){
  ...
  if(isExhibits()) return exLabelLines(nd, k);
...
window.__ccrUniverse = function(opts){
  ...
  /* CPL is a UNIVERSE, not a lens (2026-09-10): a face the page is not on means
   * the other payload has to be bound first, and setUniverse re-enters here
   * once it is — with universe===wantFace, so this cannot loop. */
  if(wantFace!==universe){ setUniverse(wantFace, true, opts); return; }
  ...
  /* ⚠️ `U===window.CPL_CCR_UNIVERSE` is TRUE right after a payload swap too —
   * bindUniverse points both at the new one — so the swap carries its own
   * flag, or the legend, the Show menu, the corpus and the course jump all
   * stay the old universe's while the canvas draws the new. */
  if(!rebound && cvs && document.getElementById("u-cvs")===cvs && U && U===window.CPL_CCR_UNIVERSE){
  ...
  view_el.innerHTML =
    '<section class="u-full" id="u-full" aria-label="SkyView — the Common Course Reference as a map">'+
      ...
      '<div class="u-foot" id="u-foot">'+
        '<div class="u-legend" aria-label="How to read the map">'+legendHtml()+'</div>'+
  ...
  roster=null; nodeIdx=null; ensureCorpus();   // the map re-binds after a payload swap (rebound clears below, after the turn decision)
  ...
  wire(); draw();
  /* The sky turns when it OPENS (ruling 4). A universe swap is a re-render, not
   * an opening: a reader who had stopped the turn keeps it stopped, or the
   * point they were about to click walks away under the new payload. */
  if(sphereOn() && !(rebound && turnBefore===false)) startTurn();
  rebound=false;
  restoreTokens();              // a selection parked by a trip off the map comes back
  drawWrites();                 // the staged moves survive every re-render (a trip off the map, a universe swap) — the pane is seeded empty
  /* The course jump from an exhibit card: the other universe is drawn now, so
   * open the identity it named and say where the reader came from. */
  if(pendingOpen){ var po=pendingOpen; pendingOpen=null; if(nodeById(po.id)){ goNode(po.id); if(po.words) setHint(po.words); } }
...
function suggest(raw, limit, order){
  ...
  var cplRows = null;   // the CPL face's vocabulary rows retired 2026-09-10: the exhibit universe searches credentials directly
  if(cplRows) pts=[];
  ...
  if(memIndex && memIndex.length){
    ...
    pre.concat(inn).slice(0,limit).forEach(function(r){
      ...
      mems.push(isExhibits()
        ? {kind:"member", kindWord:"local exhibit", kindShort:"LOCAL EXHIBIT", label:r.code,
           sub:"folded into "+(h.nd.t||h.nd.i)+" · "+h.isl.d, isl:h.isl, nd:h.nd, cn:r.cn, code:r.code}
        : {kind:"member", kindWord:"college course", kindShort:"COLLEGE CRSE", label:r.code+" · "+r.c,
  ...
  if(cplRows){
    ...
    out.push({kind:"course", kindWord:isExhibits()?"credential":p.nd.a?"stand-alone course":"course identity",
              kindShort:kindShort("course", p.nd), label:p.nd.t||p.nd.i,
              sub:isExhibits() ? (p.nd.g||"issuing agency not recorded")+" · "+p.isl.d+(p.nd.sw?" · statewide":"")
                               : p.nd.i+" · "+p.isl.d+" · "+creditShort(p.nd),
              credit:isExhibits()?"":creditWord(p.nd),
              isl:p.isl, nd:p.nd});
  });
...
window.__ccrUniverseState = function(){
  ...
  return {view:view, moves:moves, sel:selNode?selNode.i:null,
          ...
          tokens:tokens.map(function(t){ return t.label; }),
          show:(function(){ var o={}; showKeys().forEach(function(k){ o[k]=!!show[k]; }); return o; })(),   // this universe's switches
          ...
          face:face, lit:lit, cpl:cplState, cplLine:(face==="cpl"?exLineText():null),
          universe:universe, exhibits:exState, exCourses:exCourses?Object.keys(exCourses).length:0,
...
function tipHtml(hit){
  if(isExhibits() && hit.nd){ return hit.mem ? exMemberTipHtml(hit) : exTipHtml(hit); }
  if(hit.ghost){
  ...
  if(hit.mem){
    ...
    var body = info && info.desc ? esc(trunc(info.desc, 260))
             ...
             : "";
    var artic=lit ? ((cplCollegesOf(hit.nd)||{})[m.c]||null) : null;
  ...
  if(isExhibits()){
    var folded=hit.isl.p.reduce(function(a,p){ return a+(p.n||0); }, 0), lit2=hit.isl.p.filter(function(p){ return p.ar>0; }).length;
    return '<b>'+esc(hit.isl.d)+'</b><br><span class="sub">'+num(hit.isl.n)+' credential'+(hit.isl.n===1?'':'s')+' folding '+
           num(folded)+' local MAP exhibit'+(folded===1?'':'s')+' · '+num(lit2)+' with an articulation</span>';
  }
...
function wire(){
  ...
  function paintShow(){
    var K=showKeys(), on=0; K.forEach(function(k){ if(show[k]) on++; });
    var w=document.getElementById("u-show-word"), sm=document.getElementById("u-show-sum");
    if(w) w.textContent = on===K.length ? "All" : (on+" of "+K.length);
    // The count alone says nothing about WHAT is off; the tooltip names it.
    var offWords=K.filter(function(k){ return !show[k]; }).map(function(k){ return SHOW_WORDS[k].replace(/ \u2014 .*$/, ""); });
  ...
  function applyShow(quiet){
    ...
    var off=showKeys().filter(function(k){ return !show[k]; }), noun=isExhibits()?"credential":"course";
    ...
    setHint(off.length===0
      ? (isExhibits()
          ? "Showing every credential."+wide
          : "Showing every course. <strong>"+num(unrec)+"</strong> have no recorded credit status; "+
            "they appear here and nowhere else."+wide)
      : "Showing <strong>"+num(n)+"</strong> of "+num(n+hidden)+" "+noun+"s; "+num(hidden)+" hidden"+
        (!isExhibits() && unrecHidden?" ("+num(unrecHidden)+" with no recorded credit status)":"")+
        (show.members?"":isExhibits()?"; the local exhibits under a credential are not drawn":"; the college courses under an identity are not drawn")+
        (isExhibits()?".":". Noncredit is drawn with a broken ring.")+wide);
    draw();
  }
  function setShow(patch, quiet){
    Object.keys(patch||{}).forEach(function(k){ if(ALL_SHOW_KEYS.indexOf(k)>=0) show[k]=!!patch[k]; });
    applyShow(quiet);
  }
  ...
  if(every) every.onclick=function(){ var p={}; showKeys().forEach(function(k){ p[k]=true; }); setShow(p); };
  var none=document.getElementById("u-show-none");
  if(none) none.onclick=function(){ var p={}; showKeys().forEach(function(k){ p[k]=false; }); setShow(p); };
  ...
  cvs.addEventListener("dblclick", function(e){
    ...
    if(hit.nd){
      if(isExhibits()){ selNode=hit.nd; selIsl=hit.isl; showNode(hit.nd, hit.isl); draw(); return; }
      window.__ccrOutline(hit.nd.i); return;
    }
    var d=!isExhibits() && hasWorkSurface(hit.isl);
...
function searchOne(raw){
  ...
  var head="<strong>"+num(searchHits.length)+"</strong>"+
    ...
    (names.length>4?" · …":"")+"."+
    (mh.length?" "+num(mh.length)+" of them "+(isExhibits()?"fold a local exhibit named like that.":"carry a college course numbered like that."):"");
...
function showMenuHtml(){
  var W=function(k){ return [k, (isExhibits() && k==="members") ? "Local exhibits" : SHOW_WORDS[k]]; };
  var groups=isExhibits() ? [
    ["CPL type", EX_TYPE_KEYS.concat(["tna"]).map(W)],
    ["Articulations", ["arty","noart"].map(W)],
    ["Statewide", ["sw","nsw"].map(W)],
    ["Under a credential", ["members"].map(W)]
  ] : [
...
/* ── panels ─────────────────────────────────────────────────────────────── */
function chipFor(nd){
  if(isExhibits()) return '<span class="chip mut" title="A credential from the Credential Reference: one curated title folding every local MAP exhibit that names it. Curated by the MAP team, never re-keyed from this map.">credential \u2014 CER unified title</span>'+
    (nd.sw?' <span class="chip cid" title="MAP marks this exhibit as statewide.">statewide</span>':'');
...
function showIsland(isl){
  ...
  var ex=isExhibits();
  var exFolded=ex ? isl.p.reduce(function(a,p){ return a+(p.n||0); }, 0) : 0;
  var exLit=ex ? isl.p.filter(function(p){ return p.ar>0; }).length : 0;
  el.innerHTML="<h3>"+esc(isl.d)+"</h3>"+
    (authLine?'<p class="sub u-auth">'+authLine+"</p>":"")+
    (ex
      ? "<p>"+num(isl.n)+" credential"+(isl.n===1?"":"s")+" folding "+num(exFolded)+" local MAP exhibit"+(exFolded===1?"":"s")+
        " \u00b7 "+num(exLit)+" with an articulation"+(isl.p.some(function(p){ return p.sw; })?" \u00b7 "+num(isl.p.filter(function(p){ return p.sw; }).length)+" statewide":"")+".</p>"
    ...
    (top.length?'<p class="sub">Biggest first — pick one to open it:</p><ul class="idlist">'+top.map(function(nd){
      return '<li><button type="button" class="ttl linkish" data-go="'+esc(nd.i)+'">'+esc(nd.t||nd.i)+"</button> "+
        chipFor(nd)+
        '<div class="sub">'+(ex
          ? esc(trunc(nd.g||"issuing agency not recorded", 44))+" · "+num(nd.n)+" local exhibit"+(nd.n===1?"":"s")+
            (nd.ar?" · "+num(nd.ar)+" articulation"+(nd.ar===1?"":"s"):"")
          : esc(nd.i)+" · "+num(nd.n)+" member"+(nd.n===1?"":"s")+
            (nd.k?" · "+num(nd.k)+" in orbit":"")+(nd.u!=null?" · "+esc(unitsWord(nd.u)):"")+
            (nd.ar?" · "+num(nd.ar)+" articulation"+(nd.ar===1?"":"s"):""))+"</div></li>";
    }).join("")+"</ul>":'<p class="empty">'+(ex?'No credential in this discipline yet.':'No clustered identity in this discipline yet — every course here is a stand-alone.')+'</p>')+
    (ex ? '<p class="empty" style="margin-top:.6em">Exhibits are curated in the Credential Reference; nothing is moved from this map.</p>'
...
function showNode(nd, isl, keepFilter){
  ...
  if(isExhibits()){ renderExhibitNode(); resetPanelScroll(); return; }
  renderNode();
  resetPanelScroll();
...
function memberRow(m, isl, nd, moved){
  ...
  if(open){
    if(info && info.desc) h+='<div class="mdesc">'+
      (olIsPlaceholder(info.desc)
        ? '<span class="chip mut" title="Text where the description goes that says the field is blank. Printed as the college published it; the quoted description and the imputed skills are not drawn from it.">placeholder</span> '
        : '')+esc(info.desc)+"</div>";
...
/* ── the exhibit names, where the reader already is (Sam, 2026-09-10) ───────
 * "Add the name of any articulated exhibits to the course description card for
 * the MID as well" — and on the decision sheet, in his own words: "I would like
 * the exhibits list on the course cards in course view (not just CPL view) when
 * the Articulations chip is selected."
 *
 * ⚠️ THEY WERE ALREADY ON THE CARD, AND THAT IS THE POINT. `cplListHtml` prints
 * every exhibit title — as the last line of a nested block, in muted type,
 * below the description and the full member list. On FIRE 110 X that member
 * list is 55 rows. Present and unreadable is the same as absent to the person
 * reading, which is the second time this lane has learned it (S250: an answer
 * printed 850px from the hand). So this names them ABOVE the fold and leaves
 * the full block below untouched — one line, not a second copy of the record.
 *
 * ⚠️ NOTHING RENDERS FOR A COURSE WITH NO EXHIBIT, deliberately. This feed
 * cannot tell a course nobody articulated from a course nobody has looked at
 * (the same reason the light marks presence and never absence), so a line
 * reading "no exhibits" would state a finding the data does not carry. */
function cplExhibitNames(nd){
  var b=(cplState==="ok")?cplOf(nd):null, out=[], seen={};
  if(!b) return out;
  b.forEach(function(e){
    (e[1]||[]).forEach(function(r){
      var t=r[1]||r[0];
...
/* Median 1 name, p95 six, one identity carries 21 (measured against
 * ccr_cpl.json, 2026-09-10) — so six in full covers 95% and the tail says how
 * many it is holding back and where the rest are. */
function cplNamesHtml(nd, cls){
  /* Two conditions, each load-bearing where it stands. While the record is in
   * flight only a course the MAP data already calls articulated says anything —
   * `ar` is the one signal we hold before the join lands, and it agrees with the
   * join exactly (1,490 = 1,490, measured 2026-09-10), so it is the honest
   * stand-in for that second. Once the record is here the JOIN decides, and a
   * course it reaches with nothing prints nothing.
   * ⚠️ The loading branch is verified in Chromium against the served page, not
   * here: this suite binds the payload on the window, so `cplState` is "ok"
   * before the first paint and jsdom never sees the in-flight second. */
  if(cplState==="loading"||cplState===""){
    return (nd.ar>0)
      ? '<p class="'+cls+'"><b>Articulated exhibits</b> <span class="sub">· loading MAP’s articulation record…</span></p>'
      : "";
  }
  var names=cplExhibitNames(nd), cap=6;
  if(!names.length) return "";
  return '<p class="'+cls+'"><b>Articulated exhibit'+(names.length===1?"":"s")+
    ' ('+num(names.length)+')</b> '+esc(names.slice(0,cap).join(" · "))+
    (names.length>cap
      ? ' <span class="sub">and '+num(names.length-cap)+' more — all of them, with what each earns, under '+
        '<em>Credit for prior learning</em> below.</span>'
      : '')+'</p>';
}
...
function renderNode(){
  ...
  if(nd.ar>0 && (cplState===""||cplState==="loading"))
  ...
  // The Articulations chip names the exhibits up here, since the block itself
  // sits below 55 member rows on a course like FIRE 110 X.
  if(lit) h+=cplNamesHtml(nd, "u-exl");
  ...
  if(nd.ar>0) h+=cplPanelHtml(nd, isl);
...
function paintFace(){
  ...
  if(line){ line.hidden = face!=="cpl"; if(face==="cpl") line.innerHTML=exLineHtml(); }
  var lg=document.querySelector("#u-foot .u-legend"); if(lg) lg.innerHTML=legendHtml();
  paintLegendLit();
}
function paintLit(){
...
/* The CPL word switches UNIVERSES (2026-09-10); `face` tracks `universe` so
 * the hash, the pressed state and the coverage line follow as they always did. */
function setFace(f, quiet){ setUniverse(f==="cpl"?"cpl":"courses", quiet); }
function setLit(on, quiet){
  lit=!!on; paintLit();
  /* ⚠️ REPAINT THE PANEL, NOT ONLY THE CANVAS. The chip now decides whether an
   * open course card names its exhibits, so a chip that repainted only the sky
   * would put the line on the NEXT course opened — the reader turns it on to
   * see exhibits on the course in front of them and sees nothing. Both finish
   * points: the record may still be loading when the chip goes on, and the
   * line says so until it lands. setFace has always repainted for this reason. */
  if(lit && cplState==="") loadCpl(function(){
    if(cvs && document.getElementById("u-cvs")===cvs) draw();
    repaintSelection();
  });
  if(!quiet){
    ...
    setHint(lit
      ? (isExhibits()
          ? "Lighting <strong>"+num(n)+"</strong> credentials that courses are articulated to ("+num(recs)+" articulation lines). The rest are drawn as they are."+wide
      ...
      : "Light out. Every "+(isExhibits()?"credential":"course")+" is drawn as it was.");
  }
  if(cvs && document.getElementById("u-cvs")===cvs) draw();
  repaintSelection();
}
...
window.__ccrWorkspace=function(mode, opts){
  opts=opts||{};
  if(isExhibits() && UNIVERSES.courses) bindUniverse("courses");   // the tables are the course universe's
...
/* ── a placeholder is not a description (Sam, 2026-09-10) ───────────────────
 * "Some of the course descriptions have boilerplate test for descriptions."
 * He was right, and the shape is narrower than it looks: literal reuse is a bad
 * detector here, because the MOST reused strings in the corpus are the C-ID
 * descriptors for statistics, psychology and government — good descriptions
 * that many colleges share on purpose. What is actually junk is text that
 * announces it is not a description.
 *
 * Measured against unified_courses_member_desc.js on 2026-09-10: 247 of the
 * 127,266 non-empty rows (0.19%), 171 identities, 107 distinct strings — every one of them
 * read by hand before this shipped. 65 identities go from "quotes a stub" to
 * "says it has none", which is the honest answer; 106 keep a real description
 * and stop losing the medoid vote to stubs beside it.
 *
 * ⚠️ ANCHORED, AND NEVER BY LENGTH. "Study of selected works of Shakespeare."
 * is 38 characters and a real description; a length rule kills it. Each pattern
 * below matches the WHOLE trimmed string, except the March-2012 migration
 * boilerplate, which is 345 characters of prose whose entire content is that
 * the field is blank.
 *
 * ⚠️ NOTHING IS HIDDEN — IT IS LABELLED. The map panel's member list still
 * prints the stub verbatim when a curator opens that college's course: that
 * list is each college's own record, and suppressing what a college published
 * would launder the data. The outline's member list, which prints no
 * description text at all, marks the row with the word "placeholder" beside the
 * "no description" chip it already had. Only the DERIVED layers — the quoted
 * description and the imputed skills — refuse to build on text that says
 * nothing. */
var OL_STUB = /text field is blank due to the .{0,40}data migration/i;
var OL_EMPTY = new RegExp("^(?:n/?a|tbd|tba|none|null|xx+|[.\\-–—\\s]*"
  /* ⚠️ NO COMMA IN THE SUBJECT CLAUSE, AND A BOUND ON IT. With `,` allowed and
   * the clause unbounded, "Experimental course in advanced welding techniques,
   * covering plate and pipe." matched — a real sentence, thrown away. No such
   * row is in the corpus today, so the measurement did not catch it; the
   * predicate's own check did. The longest real placeholder clause is
   * "Computer Information Science - Applications" at 43. */
  + "|experimental(?:\\s+(?:course|offering))?(?:\\s+in\\s+[\\w &/'-]{1,50})?"
  + "|see\\s+experimental\\s+offerings?)[.\\s]*$", "i");
function olIsPlaceholder(d){
  var t=String(d==null?"":d).trim();
  return !t || OL_STUB.test(t) || OL_EMPTY.test(t);
}
/* The real descriptions, with a count of what was set aside — the empty state
 * says which of the two silences it is looking at, because "nobody wrote one"
 * and "somebody wrote a placeholder" call for different work. */
function olDescs(nd, isl){
  var out=[], stubs=0;
  membersOf(nd.i).forEach(function(m){
    var info=courseInfo(isl, m);
    if(!info || !info.desc) return;
    if(olIsPlaceholder(info.desc)){ stubs++; return; }
    out.push({college:m.c, code:m.n, desc:info.desc, title:info.title, units:info.units});
  });
  out.stubs=stubs;
  return out;
}
...
window.__ccrOutline=function(id){
  if(isExhibits() && UNIVERSES.courses) bindUniverse("courses");   // an outline of record is a course's
  if(!ensureCorpus()){ if(typeof window.__ccrForest==="function") window.__ccrForest(); return; }
  var hit=nodeById(id);
...
function olHtml(nd, isl){
  ...
  /* The Articulations chip names the exhibits here too — the outline's own CPL
   * layer is the fourth section down and collapsible, so the card could name a
   * course and no exhibit until it was opened. Same gate as the map's panel:
   * the chip is the lens, and turning it on names exhibits wherever a course is
   * named. */
  if(lit) h+=cplNamesHtml(nd, "u-exl ol-exl");
  ...
  if(loading) dbody='<p class="empty">Loading the catalog descriptions for '+esc(isl.d)+'…</p>';
    ...
    (descs.stubs
      ? ' '+num(descs.stubs)+' publish'+(descs.stubs===1?"es":"")+' a placeholder instead — the March 2012 migration '+
  ...
  h+=olLayer("members","The college courses under this identity",
    'The rows every layer above was drawn from \u2014 and any it set aside, marked.',
    total
      ? '<ul class="ol-mem">'+mine.slice(0,40).map(function(m){
          ...
          return '<li><span class="ol-mem-c">'+esc(m.n)+'</span>'+
            ...
            (info&&info.desc
              ? (olIsPlaceholder(info.desc)
                  ? '<span class="chip mut" title="This college publishes text where the description goes, but the text says the field is blank — the March 2012 migration notice, or “Experimental course”. The layers above are not drawn from it.">placeholder</span>'
                  : '')
              : '<span class="chip mut" title="This college publishes no catalog description for the course.">no description</span>')+
          '</li>';
