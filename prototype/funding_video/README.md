...
A narrated cut is in draft beside the introductions (Sam, 2026-09-26: "a natural feminine voice-over that follows a script your write... tone down the music to just background level"). The script and the build plan are in `20260926_Narration_Script_Draft.md`. The spoken form the voice reads is `narration_s1.json`, and `narrate.py` reads it locally with Kokoro-82M's Heart voice (`af_heart`, Apache-2.0):
    ...
    python3 prototype/funding_video/narrate.py           # about a minute: .narration/s1/ gets one WAV per scene, timing.json and a dated MP3

The first run downloads the model (about 350 MB) from huggingface.co into `~/.cache/kokoro` (`KOKORO_DIR`), so the environment must allow huggingface.co. The spoken form departs from the written script where the voice needs it to: acronyms are unspaced (`CPL`, `EDD`, `FTES`) so each reads as one quick word, MAP is written `map` because capitals spell it out, and years are words because digits read as "two thousand". `phoneme_fixes` corrects what spelling cannot: unspaced `FTES` reads as the word "eftess", so it takes the four letters' phonemes, and each year takes a lighter stress on its twenties. Every fix must fire, and `never` lists the readings Sam rejected in v2 ("stilted, especially when sounding out C-P-L... same with sounding out the year numbers"); a run whose phonemes still carry one fails.

To check a read before sending it, transcribe it locally (`faster-whisper`, model `small`, as the video-context skill does). v3 is heard as written: "CPL", "FTES", "EDD", "map", "2026 through 28", "the 26, 27 year". Two earlier wordings failed that check: without its comma the budget year was heard as one number ("2627"), and "due November first" as "do November first", so v3 says "by November first", as the scene's headline does. Word timings put each "CPL" at 570 ms, against 702 ms for v2's spaced letters (12 occurrences). v3 runs 2:49: 158.8 seconds of speech and 1.1 seconds between scenes.
