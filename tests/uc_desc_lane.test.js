...
// PRESENCE-CONDITIONAL (2026-07-10, the issuer-preseed verifier precedent):
// the receipt is regenerated by every post-apply chain, and merges CONSUME
// family members — once fewer than 4 loose infant/toddler ECED rows remain,
// the fragmentation has been worked off and the marquee assertion retires
// to a consolidation-sanity check instead of failing on success.
const looseInfTod = dg.reduce((n, g) =>
  n + g.members.filter((m) => infTod(m.t) && /^ECED /.test(m.id || "")).length, 0);
if (looseInfTod >= 4) {
  ...
  check("ECED infant/toddler fragmentation consolidated below grouping threshold ("
    + looseInfTod + " loose members remain)", true);
}
