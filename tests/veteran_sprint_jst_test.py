#!/usr/bin/env python3
"""Veteran Sprint card — apply_veteran_jst() (Session 88).
...
"Service Members (MIL)" line + the 75% star rule. Guards:

  1. "JSTs Uploaded" becomes the real VeteransWithJSTs / 30,000 goal.
  2. "Service Members (MIL)" is inserted right after, with EnrolledMilitaryStudents.
  3. The 75% star rule + the statewide ratio land in the card footnote.
  4. Idempotent — a second apply doesn't duplicate the MIL line or footnote.
...
VJ = {"statewide": {"mil": 34135, "jst": 23543, "star_colleges": 50}}
...
mil_bd = _bd(card, "service members")
check("JSTs Uploaded shows real JST / goal", jst_bd["value"], "23,543 / 30,000")
check("Service Members (MIL) breakdown added", mil_bd is not None, True)
check("Service Members (MIL) shows reported MIL", mil_bd and mil_bd["value"], "34,135")
# MIL line sits immediately after the JSTs Uploaded line.
labels = [b["label"] for b in card["breakdowns"]]
check("MIL line is right after JSTs Uploaded",
      labels.index("Service Members (MIL)"), labels.index("JSTs Uploaded") + 1)
fn = " ".join(card.get("footnote", []))
check("footnote carries the 75% star rule", "&ge;75%" in fn, True)
check("footnote carries the statewide ratio (69%)", "(69%)" in fn, True)
check("card flagged live", card.get("live"), True)

# ── 4: idempotent ──
gen.apply_veteran_jst(kpis, VJ)
card = kpis["veteran_sprint"]
check("MIL line not duplicated on re-apply",
      sum(1 for b in card["breakdowns"] if b["label"] == "Service Members (MIL)"), 1)
check("footnote rule not duplicated on re-apply",
      len(card.get("footnote", [])), 1)
check("JSTs Uploaded still the real value", _bd(card, "jsts uploaded")["value"], "23,543 / 30,000")
...
check("None vj → no Service Members line", _bd(k2["veteran_sprint"], "service members"), None)
gen.apply_veteran_jst(k2, {"statewide": {"mil": 0, "jst": 0}})
check("empty statewide → no-op", _bd(k2["veteran_sprint"], "service members"), None)
