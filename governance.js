...
(function () {
  ...
  // Drift candidates, regenerated by kb/_build_governance_candidates.py in the
  // daily cron. Static file: the browser cannot read .github/workflows or the
  // schema, so repo-side detection HAS to happen on the runner. "Refresh" here
  // re-fetches this and re-runs the live reads — it does not re-scan the repo,
  // and the button says so rather than implying otherwise.
  var CANDIDATES_URL = "kb/governance_candidates.json";
  ...
  function ensureCss() {
    ...
    var css = [
      ...
      // Drift candidates
      ".gov-cand { border:1px solid var(--border); border-radius:8px; padding:10px 12px; margin:0 0 18px;"
        + " background: var(--surface-subtle); }",
      ".gov-cand h4 { margin:0 0 2px; font-size:.9rem; color: var(--navy-primary); }",
      ".gov-cand .sub { font-size:.75rem; color: var(--text-muted); margin:0 0 8px; }",
      ".gov-cand .grp { font-size:.7rem; text-transform:uppercase; letter-spacing:.04em;"
        + " color: var(--text-muted); margin:8px 0 3px; }",
      ".gov-cand ul { margin:0; padding-left:18px; }",
      ".gov-cand li { font-size:.8rem; margin:2px 0; }",
      ".gov-cand li .w { color: var(--text-muted); font-size:.74rem; }",
      ".gov-cand .none { font-size:.8rem; color: var(--text-muted); }",
      // Owner curation
  ...
  var CAND_GROUPS = [
    ["stale_row", "\u26A0 Register rows pointing at something that no longer exists"],
    ["cadence", "Recurring jobs with no cadence row"],
    ["decision_right", "Surfaces a person changes, with no decision right"],
  ];
  ...
  function renderCandidates() {
    ...
    var c = d._counts || {};
    var h = '<div class="gov-cand"><h4>\u{1F50E} Not on the register (' + list.length + ")</h4>";
    h += '<p class="sub">Detected by scanning the repo — <b>proposals only</b>, never added automatically, '
      + "because who-decides and is-this-load-bearing are judgment calls. "
      + esc(String(c.already_mapped || 0)) + " more surface(s) already map to a row and "
      + esc(String(c.dismissed || 0)) + " are dismissed with a reason in "
      + "<code>kb/governance_surface_map.json</code>.</p>";
    if (!list.length) {
      h += '<p class="none">Nothing unaccounted for. Every scanned surface maps to a row or has a '
        + "recorded reason not to.</p>";
    } else {
      CAND_GROUPS.forEach(function (g) {
        var rows = list.filter(function (x) { return x.kind === g[0]; });
        if (!rows.length) return;
        h += '<div class="grp">' + esc(g[1]) + " — " + rows.length + "</div><ul>";
        rows.forEach(function (x) {
          h += "<li><b>" + esc(x.label) + "</b> <span class=\"w\">" + esc(x.detail) + "</span></li>";
        });
        h += "</ul>";
      });
    }
    h += '<div class="gov-toolbar" style="margin:10px 0 0"><button class="gov-btn" data-gov-refresh>'
      + (state.refreshing ? "Refreshing\u2026" : "\u21BB Refresh") + "</button>"
      + '<span class="gov-live">Re-reads the committed scan and the live figures. The repo scan itself '
      + "runs in the daily job — this button cannot re-scan the codebase from a browser.</span></div>";
    return h + "</div>";
  }

  function loadCandidates() {
    return fetch(CANDIDATES_URL, { cache: "no-store" })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (d) { state.cand = d; return d; })
      .catch(function () { return null; });   // never take the register down
  }
  ...
  function render(root) {
    ...
    // ── Drift candidates ──────────────────────────────────────────────────
    // Surfaces that look governable and have no row. PROPOSALS: nothing here is
    // ever auto-added, because `decides` and `load_bearing` are judgment and a
    // register full of plausible rows nobody decided is exactly the theatre this
    // page exists to expose. Grouped and ranked so the first rows are the ones
    // worth acting on — an undifferentiated list of 39 was the first draft, and
    // it would have been read once and never again.
    h += renderCandidates();
  ...
  function wire(root) {
    ...
    var rf = root.querySelector("[data-gov-refresh]");
    if (rf) rf.addEventListener("click", function () {
      if (state.refreshing) return;
      state.refreshing = true; render(root);
      Promise.all([loadCandidates(), signedIn() ? loadLive() : Promise.resolve(null),
                   signedIn() ? loadOwners() : Promise.resolve(null)])
        .then(function (res) {
          if (res[1]) state.live = res[1];
          state.refreshing = false; render(root);
        })
        .catch(function () { state.refreshing = false; render(root); });
    });
  ...
  function activate() {
    ...
    Promise.all([
      ...
      loadCandidates(),   // ungated: the candidate scan carries no private data
    ]).then(function (res) {
