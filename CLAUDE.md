...
| **Admin tab / the side menu as data** | One place to manage the COBI side menu — order, grouping, naming, which sites show what, who sees it — beside what actually protects each tab. | ✅ **LIVE** (SkyGate #1193/#1195/#1196; SkyKey #1203; Sky159 #1209/#1210). The menu is `cobi_nav`, a curator overlay edited by drag and drop, **seeded EMPTY** — today it is exactly what the code ships. ⭐ **THE OVERLAY NEVER GATES THE MENU**: the page builds from code and paints, then applies the overlay if and when it arrives. Offline · HTTP error · malformed rows · a throwing `plan()` · a corrupt cache · a read that never resolves each land on the shipped menu, each tested — a nav that fails closed gives a site with no navigation, so the fail-safe carries more test surface than the feature. ⚠️ **LOCKOUT IS PREVENTED IN CODE, NOT THE TABLE** — `admin`/`dashboard` cannot be hidden, cannot lose their pin, refuse to be dragged into a group, and `admin` is LIFTED OUT of a group that gets hidden; enforced at plan, drag and render because the table is the thing being guarded. **Two protection lists, and the axis is RECOVERABILITY**: `PROTECTED` (never hidden) = admin+dashboard; `AUDIENCE_LOCKED` = dashboard ALONE, because an audience rule is recoverable (sign in and it returns) and `hidden` is not. ⚠️ **DISPLAY IS NOT SECURITY** and the tab is built around saying so: menu columns and the live RLS gate (`cobi_rls_gates()`) share ONE table. **73 tables + 6 views: 29 public-read · 24 team-phrase · 10 server-only · 5 reviewer-only · 4 Finance · 1 GR · 0 with RLS off**; five tabs remain genuinely unmapped and render **unknown with the reason**, never a clean bill of health. ⭐ **HIDE + AUDIENCE ARE ONE LADDER** (Everyone → signed-in → magic-link → nobody), stated in words on the row, and pressing it ASKS instead of toggling. **The merge is at the CONTROL, not the table** — `plan()` treats the two columns differently — and **`hidden` PRESERVES the audience underneath it**, or un-hiding silently widens a magic-link-only item to everyone. An option that cannot be honoured is not offered, re-checked at the write. ⭐ **THE PROTECTION CHIP IS ON EVERY ROW** — it covered public-read tabs alone, so a properly protected tab showed nothing, indistinguishable from unexamined (**1 of 7 → 7 of 7**). ⚠️ With no measurement everything classifies unknown, so chipping every row would report **"Not mapped" ×35** off one failed request; distinct `nodata`/`unread` states, resolved at display and never counted as gates. ✅ **CURATOR CATEGORIES** — `plan()` unions overlay-only groups (a tab parented to an unknown group used to degrade to top level SILENTLY, exactly what a curator-made group looked like). ⚠️ **Removing one is a DELETE, not an omission** — the save is an upsert, so an omitted row survives and the category returns out of position; a failed delete ABORTS the save. Guards: key unique vs draft AND code groups · blank rename falls back to the key · removal only while EMPTY. ⚠️ **Empty groups stay in `plan().groups` on purpose** — `makeGroup` already drops them from the rail and they are the editor's drop targets. ⚠️ **A FULL-REWRITE SAVE MUST ROUND-TRIP WHAT IT DID NOT TOUCH**, and **a bulk POST is ONE INSERT over the UNION of the array's keys** (that is why `cobi_nav` held zero rows for two days while the tab looked finished). **NEXT: nobody has dragged-and-SAVED in a real browser yet**, then create/delete a category, then fill owners on DR-13…DR-18. Story: `docs/admin_tab_lessons.md`; ADR [`adr-the-side-menu-as-an-overlay-over-code-defaults`](docs/kb-notes/adr-the-side-menu-as-an-overlay-over-code-defaults.md); durable [`methodology-a-bulk-post-is-one-statement-over-the-union-of-its-keys`](docs/kb-notes/methodology-a-bulk-post-is-one-statement-over-the-union-of-its-keys.md) · [`methodology-a-check-that-never-registers-can-never-fail`](docs/kb-notes/methodology-a-check-that-never-registers-can-never-fail.md). |
