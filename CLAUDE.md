...
| **Memory tab / Autogenerate + the Briefing** | Drafting a memory row from a typed topic, reading the entries back, and curating them. | ✅ **AUTOGENERATE + BRIEFING LIVE; BRIEFING CONFIRMED BY SAM IN A BROWSER (2026-08-25) — do not re-ask.** ✅ **CURATE ACTIONS LIVE** (SkyFixer #1330/#1331). ⭐ **A CAP THAT IS GENEROUS FOR ONE CALLER IS A SILENT CONTENT SWAP FOR ANOTHER** — `cpl-chat` capped `query` at 1,000 chars and the drafting envelope measured **984**, so an 870-char note reached the model as *"When responding "* and it wrote about the doctrine instead. Fixed with a **per-surface cap** (`queryCapFor`), an optional **`retrieval_query`** (search text ≠ sent text), and a `DRAFTING_BLOCK` on `volatile` — never `stable`, the prompt-cache breakpoint. **The topic LEADS**, so anything a cap eats is instruction text and fails loudly. ⭐ **THE BRIEFING IS AN AGENT READ-BACK, NOT A SIERRA ANSWER** — `cpl-chat` references `cpl_memory` **nowhere**. Every claim cites its entry; a citation to a non-entry renders FLAGGED. ⭐ **THE WRITE KEY NAMED NOTHING** — `slug` is UNIQUE but **NULLABLE** and the display handle falls back to the uuid, so **6 of 572 rows** took a PATCH matching zero rows while the page blamed the team phrase. Writes key on `id`; a keyless row is REFUSED with the reason. ⭐ **THE ✎ CHIP WAS A CYCLE THAT WROTE WHAT IT PASSED THROUGH** — reaching `stale` from `proposed` transited `verified`, every step a real PATCH plus an audit row; **Sam's two clicks sit in `cpl_memory_log` 15s apart**. Now a MENU (one click, one write, each status carrying what it means); `setStatus` **clears the stamp** whenever the status is not `verified`; a verified write **names the signed-in person**. ⚠️ **Delete is reachable from the list and does not delete from it** — it opens the entry's confirm, where the referencing-entry count and the reviewer-only warning fit. ⚠️ **ONE ROW STILL CARRIES THE FALSE STAMP** (`stale` + `verified_by='curator'`) — awaiting Sam's go; the **11 `proposed` rows with `verified_by='Sam Lee'/'Jenni'` are real attribution, never sweep those**. ⚠️ **HALF A TWO-HALF FEATURE DEPLOYS ITSELF** — the client is a Pages static file, the Edge Function needs a dispatch, and `cpl-chat-smoke` tests the DEPLOYED function so nothing notices the gap. ⭐ **THE BRIEFING READ 34 OF 188 AND PICKED THEM BY "MOST RECENTLY EDITED"** (Sky196, 2026-08-26). The corpus budget is **17,951 chars** (20,000 server cap minus a MEASURED 2,049-char envelope) and the verified set digests to **83,058**, so only ~18% fits — while rows arrive `updated_at` descending and `briefDigest` drops from the END. Every entry the model saw was dated 2026-08-05 or later; a typo fix outranked a standing rule untouched since June. **Disclosure and selection are different problems** — this panel already said "Read 34 of 188" correctly and nobody had asked WHICH 34. ⚠️ **A STRICT LADDER SORT IS THE WRONG FIX AND ONLY LIVE DATA SHOWS IT**: 82 of 188 verified rows are procedure+decision, so ordering strictly by band spends the whole budget inside band 0 — **38 decisions and ZERO facts, pitfalls, risks or milestones**, worse than the recency order it replaced, because the traps live in the bands it never reaches. ⭐ **THE LADDER ORDERS, A PROPORTIONAL SHARE SELECTS**: each row carries `(j+0.5)/bandSize`, its position WITHIN its band, and the sort runs on that fraction first and band second — so any prefix holds the same FRACTION of every band while each slice still runs down the ladder, oldest slices first. Live result: **49 entries, all seven kinds** (12 fact · 12 procedure · 10 pitfall · 9 decision · 2 risk · 2 milestone · 2 opportunity). A band of *n* rows first appears at `1/(2n)`, so at a 20% cut **every band of 3+ rows is guaranteed** — the test asserts that, not "every band always", which the maths does not support. ⭐ **AND THE BRIEFING WAS READING `summary`, NOT `plain`** — the Report renders `reportProse()` (plain-preferred) while `briefRow()` sent the curator's summary, so screen and model read DIFFERENT words and **every plain-language pass this table has ever had never reached the model**. The file's own "what is briefed is what is shown" invariant was true of the SET and false of the TEXT. `detail` still rides along as the evidence. ⭐ **`title` IS READER-FACING** (Sam, 2026-08-26) — the Report renders it bold above every paragraph. `plain` was already good (565 of 589); the engineer-speak was in `title` (63 of 382 proposed, 18 of 188 verified). **202 rows rewritten**; after: **0 rows without plain text, 0 with developer jargon, 0 British spellings across all 573 non-superseded rows**. `summary`/`detail` untouched — the curator record. Receipt `kb/cpl_memory_plain_seed_2026-08-26.json`. ⚠️ **GUARDS ON THE ORDERING FUNCTION ALL PASSED WITH THE WIRING DELETED** — removing `rows = briefOrder(rows)` from `renderBriefingPanel` scored 74/74. The guard that works drives the real panel and reads the body that was SENT. Five perturbations, each a named FAIL with the count still registering. ⚠️ **A case-INSENSITIVE detector with a case-SENSITIVE fix leaves the capitals behind** (`Labelling` survived a sweep that caught `labelling`). **Corpus:** 386 proposed / 188 verified, **26 verified rows name no verifier**. **Free rider:** 8 British spellings in Sierra's own prompt text. **NEXT:** ① Sam's go on the false stamp; ② work the 26 unattributed `verified` rows; ③ **Sam works the hopper** — it is now readable end to end; ④ the 20,000 cap is the real ceiling, and raising it needs a `cpl-chat` deploy (his call). Story: [`docs/cobi_memory_tab_lessons.md`](docs/cobi_memory_tab_lessons.md); durable [`a-silent-input-cap-is-a-content-swap`](docs/kb-notes/methodology-a-silent-input-cap-is-a-content-swap.md) · [`name-the-credential-that-actually-failed`](docs/kb-notes/methodology-name-the-credential-that-actually-failed.md) · [`when-a-corpus-does-not-fit-the-order-is-the-selection`](docs/kb-notes/methodology-when-a-corpus-does-not-fit-the-order-is-the-selection.md). |
