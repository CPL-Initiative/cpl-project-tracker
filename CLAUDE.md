...
### SkyView S187 — the queue cannot reach the goal, and the fold landed (2026-08-24)
...
17,321 on canvas, precomputed stable layout, draggable islands) is the only place that repair
happens. ⚠️ Labels had to be **collected then collision-rejected** — drawing each inline
reproduced the unreadable pile of his own Obsidian screenshot.
⭐ **THE ESL FOLD IS APPLIED: seven comprehensives, 1,997 rows.** Mechanism is **survivors,
not new ids** (the code decides: `UC-CUR-*` is the only self-trusted prefix and Session 56
re-minted all 4,053 away). ⚠️ **`ON CONFLICT` blocked three of seven RENAMES — 1,994 landed
where 1,997 was planned, and that gap was the whole story**; one blocker was Sam's own row,
superseded explicitly per Rule 8. ⚠️ **Automotive VESL does not exist** — his own example had
zero rows, and 78% of the lane is generic workplace English.
⚠️ **The fold-scoped-list trap fired THREE times** (carve-out card, survivor check, and the
identity lint before it): *ask whether the list you read can contain what you are counting.*
