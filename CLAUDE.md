...
| **Activity↔Project PR-C/D** | PR-C: editor + add-flow modal (Activity/Project radio + ladder fields + N-to-N multi-select); reuses the magic-link auth + the new `data-kind` scoping. PR-D (optional): split Workplan Goals into its own top-level tab if the page gets dense (Sam's prior preference: one page with two sections). | queued (next session) |
