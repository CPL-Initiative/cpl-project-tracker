...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **LIVE, TWO LANES, ONE SOLVER.** ⭐ **NEVER READ THE CONFIG — CALL `_effective()` / `_alloc()` / `_nc()` / `_prios()`** (#1359). Baked defaults are **STALE BY DESIGN**, `mirrorYears` makes the stored Year-2 block **INERT**, `yearPriorities` is SPARSE. Dump the live dials with `scripts/funding_effective.js --config <file>` — never transcribe a snapshot here. ✅ **Sam's dial RULINGS** (values: read the config): factor **0.5 in BOTH lanes**, no NC discount · **$60k NC floor INFEASIBLE**; at $50k **30 of 33 floored**. ⭐ **BOTH BOUNDS SOLVE TOGETHER** (`solveBounded()`). **A ceiling cannot lift a floored college — the floor is the only real equity lever**, priced in the earn rate of the middle. ⚠️ **A floor the pool cannot honor SAYS SO** (`floorInfeasible`). ⚠️ **NEVER RE-DERIVE AN ALLOCATION — call the model, in BOTH lanes** ([`a-total-that-balances-is-not-a-total-that-is-right`](docs/kb-notes/methodology-a-total-that-balances-is-not-a-total-that-is-right.md)). ⚠️ **NC money never sums into the credit total** (Sam: *"the neglected step child"*). ⭐ **ROUTE, DON'T SPLIT**: a unit earns in ONE lane by origin; **`share` splits the MONEY, not the FTES**. ✅ **DRAFT-MODEL POSTURE (Sam):** a missing field is not a blocker — an ABSENT number is fine, a **plausible wrong one never is**. ✅ **A priority PINS its measure** with `metric_src` (unknown pin → loud `bad_src`, **$0, never a full-cap advance**; declared-but-undelivered → `undelivered`, $0). Agreement checked on **UNIT and MILESTONE**. ⚠️ **A pin belongs with its metric, NOT in the bake.** Access is measured on **`ppa_u`** (`pa` is the **DISJOINT** half). ⏭ **MAP is shipping `Origin` + `LocID2`** — one builder change then, no consumer change. ✅ **THE NC LANE EARNS (Sky200, #1367)** — a second NC row per college, shares **34/33/33** (Sam) and factor 0.5 inherited from credit, with an `ncPriorities` override layer. **Live: $0 earned across all 33, exactly ONE earning proportionally.** ⭐ **THE PIN IS COMPULSORY HERE** — NC inherits credit's WORDING, so an unpinned NC priority prose-matches a **CREDIT** source and scores noncredit money on credit performance; the coupling runs both ways, so the `ppa_u` pin is load-bearing for BOTH lanes. ⚠️ **NC MUST NEVER ADVANCE** — the artifact-not-loaded branch pays the FULL CAP, so it is lane-guarded (else a slow load disburses **$1.8M**). ⚠️ **A lane normalizes on its OWN share sum**, and **a share set MULTIPLIES the pot rather than normalizing it** (1.30 places 1.30 × W — in credit too). ⚠️ **`fmtCountK` misstates an NC target** (1–25 FTES; 1.4 → "1"), and **the credit lane has the same latent case**. ⭐ **EVERY college with noncredit gets a row, including the 78 below threshold** — the threshold is a DIAL, and listing only qualifiers meant moving it changed WHICH ROWS EXIST. Their money cells are **em-dashes, never $0** ("never eligible" ≠ "posted nothing"). ⚠️ **ZEBRA IS PER COLLEGE, NOT PER ROW** — `nth-child` parity puts a college's two rows on opposite stripes and the pair reads as two colleges. ✅ **"Confirm participation", not "opt in"** (Sam) — opt-in presumes a default of OUT; nothing here turns on a choice, only on results. ⚠️ **`partLabel` was rendering "by by"** — `partReqText()` is the one joiner now. ⛔ **THE THRESHOLD CANNOT GO BELOW 400 AT THE $50k FLOOR** — swept through the model: 400 is the last feasible step, **350 already demands $1.90M against $1.8M**. **Threshold and floor are ONE decision**; below 500 the $100k ceiling stops binding at all. **NEXT: ① the ⬆/⬇ glyph → the words `min`/`max` (Sam had to ask what it meant; his own rule is plain words over glyphs); ② his call on the threshold, given the floor coupling; ③ the credit-lane rounding variant; ④ NC's own strategy text — Year-1 P-Success still lists "noncredit mirror courses" among CREDIT strategies.** Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) ([archive](docs/cpl_funding_lessons_archive.md)). |
