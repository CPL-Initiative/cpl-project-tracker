...
| **CPL clean-up worklist** | What to fix in the CPL data, in what order, and who fixes it. | ✅ **LIVE — `map_cleanup_worklist`**, a TABLE rebuilt inside the nightly promotion (SkyLoad #1256/#1257). Authority: [`docs/map_cleanup_worklist.md`](docs/map_cleanup_worklist.md). ⭐ **BUILT FOR THE CUSTOMER SUCCESS TEAM** — **Natalie Powell** (lead), **Chelsea Mirada**, **Ally Barker** (Sam, 2026-08-19). Carries the **college contact for that class of work** (AO/faculty for the approval queue, CPL coordinator otherwise). ⚠️ **THEY NEED THE TEAM PHRASE, NOT REVIEWER ACCESS** — none is on the roster and they should not be added for this: reviewer is all-or-nothing and also reaches `map_student_credit` (591,820 student-grain rows), `kb_curation`, `gr_*`, and **`team_access` itself, so a reviewer can rotate every phrase**. ⭐ **The gate dropped by MATERIALISING, not by weakening** — as a view it was permanently reviewer-only, and relaxing `security_invoker` would have silently bypassed the student-grain gate; the two look alike in a diff and are opposite in effect. Every row is a per-college AGGREGATE, so it sits at `is_allowed_reviewer() OR team_pass_ok()` like `map_college_cr_unit`. ⚠️ **`G9` refuses to finish the promotion if it loses that gate** (rebuilt by DROP/CREATE nightly, so a policy mistake would be SILENT). ⚠️ **No k-anonymity — internal tool, NEVER a public surface.** ⚠️ **12 colleges have work and NO contact to call** (Citrus 109 · Allan Hancock 99 · Saddleback 63 · Hartnell 53 …) — corroborates the MAP Users gap. ⭐ **RANKED BY DECISIONS, NOT ROWS** — an `effort_shape` per class (`one rule` / `per row` / `upstream`), because sorting by row count inverts the order. Same trap the Common CR Reference hit. **P1 recommendations that CANNOT yield credit — 17,594 rows, ~100 colleges, ZERO units** (ACE no-credit 11,926 · individualized 3,955 · zero-hour 1,713): one rule, the same rule everywhere. ⚠️ Clearing it **raises every disposition rate without awarding a unit** — say so when the rate moves. **P2 plan says Transcribed, no units — 14,348 rows / 4,196 students.** ⚠️ **Biggest by rows, smallest by effort: 94% is TWO colleges** — LA Pierce 1,842 (**73% of its own students**) + Merced 1,787 — and it is an **ingest gap** (bare `Transcribed`, `MAPSAS-*` AP exhibits; credit recorded, unit amounts never landed), not curation. 2b is 568 students on `MAPCXH-*` HS articulation, mostly MVC. **P3 Applied with zero applied units — 413 rows.** **P4 articulations in the approval cascade — 2,225 at ARTICULATION grain** (Faculty 1,055/45 colleges · **Initiator 1,026 across only NINE** — a stalled queue, not a statewide condition · AO 144); not a defect, and the only class naming its own owner. ⚠️ **`cpl_plan_status` is a STUDENT attribute** (constant for 47,804 of 47,804) — measure it per student or a grain artefact reports as a defect (56,614 rows → 4,196 students). ⚠️ **`applied_credits > 0` is NOT "credit was awarded"** — it is **identical to `articulated_credits` on all 462,355 Needs Action rows**. **NEXT:** ① P1 as one instruction to ~100 colleges; ② P2 to Pierce + Merced directly; ③ ask the nine Initiator colleges what is stalling; ④ Sam on the applied-fork conflict. |
