...
| **PR-Sidebar-A** | Replace top tab nav with a fixed left rail sidebar. CSS Grid layout (`grid-template-columns: 220px 1fr`). Each tab as a list item in the sidebar; sign-in widget moves into the sidebar footer; URL-hash routing unchanged (sidebar wires up the same `activate()`). Hamburger toggle for narrow screens. **User-confirmed scope**: top bar removed entirely (not adaptive); sidebar is THE navigation. **User-flagged Session 10 priority** (2026-05-26): the curator explicitly asked for it as the next priority but it was punted due to scope — Session 11's lead-off lane. | **queued — flagged Session 11 priority** |
