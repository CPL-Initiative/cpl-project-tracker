...
| **Fact Sheet (public) — curate access + accessibility** | Who can edit the public Fact Sheet, and whether the page actually works for everyone who lands on it. | ✅ **BOTH SHIPPED** (SkyCurate, #1269). ⭐ **THE CURATE BUTTON WAS NEVER THE GATE** — writes are RLS'd to `is_allowed_reviewer()`, the anon key can read and never write, and `factsheet_edit.js` is **served publicly**, so any reveal switch is discoverable. Hiding it is **presentation**: a visitor stops being offered a control they cannot use. The module says so, and a test pins that the reveal flag is never consulted by the auth helpers. ⚠️ **HIDING THE BUTTON ALSO HIDES THE WAY IN** — it *was* the sign-in entry point, so gating on `isReviewer()` alone strands a curator on a new laptop or past the keeper's 12h cap. Two doors: ① the page now loads **`cpl_session.js`**, so a COBI sign-in reaches it across browser tabs (this also fixed a latent gap — it previously saw a session ONLY if the magic link opened in that same tab); ② **`?curate=1`**, stripped from the address bar on read, remembered per browser, `?curate=0` forgets. **The bookmark is `fact-sheet/?curate=1`.** ⚠️ **`hidden` in the markup is decorative when a class sets `display`** — `.btn{display:inline-flex}` outranks the UA `[hidden]` rule, so the CSS half is load-bearing. ⭐ **THE ACCESSIBILITY AUDIT FOUND MORE THAN THE FEATURE DID, and NONE of it was reachable by the nine existing jsdom suites.** ① The statewide grid needs **368px** of fixed track: at 360px the page scrolled sideways **31px**, the flex column collapsed so the program name printed **ON TOP of its own figure**, and **"Could adopt" was clipped out of existence** by `overflow:hidden`. **A page that silently drops a column looks complete** — the worst of the three. Now stacks below 560px with labels carried as `::before` (out of `textContent`, so the Word export is untouched). ② **No skip link** — ~25 tab stops before any content. ③ The 674px funding table's scroll container is why the page never broke **and** why it was **keyboard-unreachable**; now a labelled region focusable **only while it overflows**. ④ Skipped heading levels, fixed by moving the LEVEL and keeping the LOOK on a tag-qualified utility. ⚠️ **THAT FIX BROKE THE CONTENTS HEADING AND NO ASSERTION SAW IT** — `h-sub` on an `h2` matches `h3.h-sub` never; a **before/after pixel diff** caught it. A change claiming "semantics moved, appearance did not" is only checkable in pixels. ✅ **Contrast: all 31 painted pairs pass AA**, composites included — the palette was already sound. `--mustard-fill` decorative rules are 1.95:1, the same documented exemption class as `--border-strong` (1.92:1); a test pins that **no TEXT uses it**. **Verification is split by instrument:** `tests/factsheet_a11y.test.js` (69 checks, CI, structure + computed contrast — jsdom has no layout engine) and **`fact-sheet/check_mobile_layout.js`** (Chromium, on demand, deliberately NOT in `npm test` — CI has jsdom only; 9 viewports + keyboard + reduced motion). Desktop pixel diff **0px**. Full suite **232/232**. ⚠️ **No session can verify the live page** — the sandbox is egress-blocked from `cpl-initiative.github.io`. ✅ **BOTH VERIFICATION ITEMS CLOSED BY EXERCISE (Sky175, #1271)** — the 9-viewport harness passes on current `main`, and the Curate reveal was **clicked end to end in Chromium over http://** (a `file://` origin cannot hold `localStorage`): public hidden · `?curate=1` reveals + strips the param + is remembered · `?curate=0` forgets · **a COBI sign-in in another tab reveals it with no param**. 10/10. **NEXT: ① Sam opens it on his phone** — still the one part no session can do. Story: [`docs/fact_sheet_lessons.md`](docs/fact_sheet_lessons.md); durable [`hiding-a-control-also-hides-the-way-in`](docs/kb-notes/methodology-hiding-a-control-also-hides-the-way-in.md) · [`verify-with-the-instrument-that-can-see-the-defect`](docs/kb-notes/methodology-verify-with-the-instrument-that-can-see-the-defect.md). |
...
⭐ **Chromium 127+ makes an overflowing scroller focusable with NO tabindex, so both behavioural checks passed against the UNFIXED page.** The measuring browser can hide the defect: assert the attribute, label the behaviour a regression guard. Three more of my own checks were wrong before the code was.
