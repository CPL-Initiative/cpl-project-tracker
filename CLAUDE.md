...
2. **Course-identity layer** (which common course a local course is) — staging
   ...
   - **Staging (operational, machine-built from the COCI course universe):**
     ...
     the payoff layer), `coci_curation.json` (curation state).
...
- The **Unified Courses** tab lets allowed reviewers curate courses.
  `unified_courses.js` is a **static asset** — edit it directly; it is NOT
  regenerated by `excel_to_dashboard.py`.
- Auth is **Supabase GoTrue magic-link** sign-in gated by an `allowed_reviewers`
  list. Sign-in works today; the magic link's redirect must be passed as a
  **`?redirect_to=` query param** (not a body `options` object) and must match
  the Supabase **Site URL / allowed Redirect URLs**. Don't re-break that config.
  Schema setup: `kb/supabase_curation_setup.sql`.
- Supabase is **live and shared**: only the unified-courses curation tables are
  in scope. The projects/budget/personnel/workplan tables (§8) and the
  auth/Redirect-URL config are off-limits without explicit confirmation, and no
  destructive migrations without sign-off.
...
merging + subject canonicalization, singleton minting, discipline completion,
and the `suspect_course_as_exhibit` triage (raise the Modesto pattern with the
college).
