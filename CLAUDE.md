...
| **Governance & team enablement** | Decision rights (who decides what), acceptance standards per input, and which cadences actually run — plus onboarding as the team grows past Sam. | ✅ **Starter LIVE** (SkyMail, #997/#998) — team-gated ⚖️ Governance tab: 10 decision rights · 8 acceptance standards · 5 cadences · 6 open questions. **Every `owner` is deliberately unset — filling them IS the review (OQ-01).** ⚠ It measures itself: the contact-refresh cadence was **decided in June and has never run once** (0 rows in `map_college_nudges`). **Owner column is EDITABLE** (#1000 — Sam: *"how do I add an owner? Doesn't appear to be editable"*; I shipped the review without the pen). Owners live in a separate gated `governance_owners` table overlaying the register by row id, so a session regenerating the JSON can never wipe an assignment; no delete policy. **Next:** ① fill the owner column; ② run that cadence once end-to-end with a named owner; ③ decide CIP's promotion criteria BEFORE the fall-2026 cutover (OQ-03); ④ cut the load-bearing list — 8 of 10 is too many. Team guide: `docs/working_with_claude_code.md` + the CLAUDE.md §"Working with the MAP team" obligations. **Agents: recommended NOT yet** — an agent must be invoked, so it fails exactly when a new user forgets; standing instructions can't be. Build a cross-impact reviewer agent when concurrency makes collisions real. ✅ **SIERRA IS ON THE REGISTER + THE DRIFT DETECTOR IS LIVE (SkyMiner, #1031/#1034/#1036).** Sam: *"I don't have the MAP/CO test invitation on my list of governance items"* — he was right; Sierra was absent entirely. Added **DR-11** (what Sierra tells the public — `decides` records honestly that it has been Sam personally, four times in two days), **CA-06** (Sierra feedback triage, `never-run`, and it **measures itself** — a `live:"feedback"` lens showing *21 of 25 untriaged*, CI rows excluded), **OQ-07** (what must be true before inviting MAP/CO — suggests staging it) and **OQ-08** (register drift). **Detector built + wired to the cron** (step 4a0): pure static analysis over 4 surfaces, **39 → 15 candidates** (6 mapped, 14 dismissed with reasons in a committed file), **0 stale rows**, top six are recurring jobs nobody listed. **Proposes, never auto-adds.** ⚠️ **Owner flow had 3 defects** (#1032/#1033) — first-click save, a failed read rendering as "nobody has an owner", and sign-in never re-reading; **7 more reported, unfixed** — the likeliest to be mistaken for a regression is `Clear owner` being a no-op on the 3 cadences carrying a register-file owner. `governance.test.js` 44 → **90**. **Promote-from-candidate NOT built** (needs judgment fields typed by a human; today you edit the surface map). ✅ **`sierra_rules` IS LIVE (Sky155, #1186)** — Priority 1 of [`adr-judgment-in-tables-mechanism-in-code`](docs/kb-notes/adr-judgment-in-tables-mechanism-in-code.md). The ~10 built-in prompt rules that **outrank team guidance** are now a registry (`sort_order` + `applies_when`) overlaid by a reviewer-only table that **never replaces** code defaults — a failed read costs the *edits*, never the *governance*. Proven **byte-identical** to the old chain across all 16 context combinations. **Protected set** (`portal`, `landing_page`, `volume`, `credit_status`) always ships its code body and ignores `active=false`; a curator body is APPENDED. `sierra_rules_log` is trigger-written. **Seeded EMPTY on purpose.** ✅ **THE CURATOR UI IS LIVE (SkyGate, #1190)** — on **Sierra Training**, not Admin (Sam's call; it puts the whole instruction stack on one screen in precedence order). Reviewer-only for read AND write, honouring the gating recorded in `supabase_sierra_rules.sql`. The pane renders the **generated** defaults (`kb/_build_sierra_rule_defaults.py` → `sierra_rule_defaults.js`) with the overlay painted on top, because a table seeded empty would otherwise show a curator nothing; a test lifts the real `assembleRules` and proves the display merge equals it across **40 overlay×context combinations**. *Restore the built-in wording* WRITES THE DEFAULT BACK (no DELETE policy, by design) and says so; a protected rule offers no Switch off, because `active=false` is ignored there and the control would be a no-op. ⚠️ **The "which rules were in play" view** (`chat_interactions.rules_fired`) is still unbuilt — the ADR says it is worth more than editability. `AUDIENCE_RULES` deliberately deferred. ✅ **REGISTER EXPANDED 12 → 18 (SkyGate).** The drift detector's queue was 26 and the `<25` guard had been red for two days; **19 were genuinely resolvable** — 4 mapped to existing rows, 15 belonged to six unrecorded decision rights: **DR-13 the workplan itself** (the most public artifact the project has, four tables editable in-page, no named owner), **DR-14** phrase rotation, **DR-15** contracts, **DR-16** CPL News, **DR-17** the Common CR Reference, **DR-18** TMC submissions. All `owner: null` — filling them IS the review (OQ-01). ⚠️ **A false stale flag was a DETECTOR BUG**: the path regex anchored on `\b`, which cannot match before a dot, so `.github/...` was captured as `github/...` and reported missing — the exact failure its own docstring warns about. Fixed with a negative lookbehind. **Queue now 7**, all scheduled workflows nobody has listed as cadences; each needs a row or a reasoned dismissal. **Do not bulk-dismiss** — the reason is the point. |
