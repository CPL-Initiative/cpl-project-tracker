...
| **Exhibit-canonicalization revisit** | Audit + bring current the `kb/unified_titles.json` + `kb/credentials.json` exhibit-canonicalization layer (the credential-identity layer per `kb/README.md`, separate from course-identity). Phase 1e + the CSC tab work shipped a lot of infrastructure (canonical SUBJ4, validate workflow, TOP/CTE/CIP, local-variants aggregation) that may inform the exhibit layer too. See `.claude/skills/exhibit-canonicalization/SKILL.md` + `docs/exhibit_unification_vision.md`. | **NEXT SESSION** (user-flagged 2026-05-23) |
