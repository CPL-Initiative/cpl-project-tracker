...
| `tmc_ge_patterns.js` | The **GE Breadth patterns** (`window.CPL_TMC_GE_PATTERNS`) for the full-ADT companion panel (Session 60): **Cal-GETC** (the single statewide ADT GE pattern as of Fall 2025, AB 928; primary) + legacy **IGETC** and **CSU GE Breadth**. Each modeled as `sections[].slots[]` like a TMC but `ge:true`+`noncid:true` (college-certified GE areas, no C-ID auto-match; `units` = per-course minimum). **DRAFT** — encoded from public ASCCC/CCC standards (CCCCO Breadth Form PDFs bot-block the agent env), verify against the official forms — **except Cal-GETC, now VERIFIED** against the official **Cal-GETC Standards v1.3** (PR #457: Area 3 Arts & Humanities → 2 courses; pattern sums to **34 units**). Lazy-loaded by `tmc_builder.js`. Static — NOT a daily-cron artifact. |
...
> **Session 57 + 58 narratives archived** → `docs/roadmap_archive.md` (Bruh
> Skydriver — worklist popup polish + "(NC)" title cleanup #441 + the
> CONSOLIDATION LOOSENING #442, `_sug_sig` level-SAFE → level-COLLAPSING;
> Bruh Skyleader — override-rename + segment-fold + completion note #445,
> synonym map + keyword-gather #446, the 🏷 looseness slider).


### Session 59 — Bruh Star Navicus: the TMC Builder tab (2026-06-16)

Sam's detour from the data lane into the **real ASCCC transfer process**: a new
**TMC Builder** tab (`#tmc-builder`, §7d) shipped end-to-end across PRs
#450/#451/#452 (all merged + LIVE). Pick a College + a Transfer Model Curriculum →
LEFT column = the fixed ASCCC C-ID course list, RIGHT auto-fills the college's own
COCI course carrying each slot's C-ID; Total Units, export (.docx/print/JSON),
Supabase Save/Resume. **All 45 official TMCs** encoded by `tmc/_parse_tmc_pdfs.py`
(PyMuPDF, from PDFs Sam fetched in his browser + uploaded — c-idsystem.org 403s the
agent env; 756 slots, all `draft`, 25 `cid_unverified` discrepancy flags). Plus a
full **curator layer**: Status filter + CO-review queue, 📤 Submit-for-CO-review,
magic-link login (shared `cpl_sb`, `allowed_reviewers`) gating per-row
`tmc_curator_notes`, committed PDF artifacts (§8 tables). Full story:
`docs/tmc_builder_lessons.md`; data model `docs/kb-notes/reference-tmc-adt-data-model.md`.


### Session 60 — Bruh Momentus: TMC Builder list-first + the full ADT (GE Breadth) (2026-06-17)

Sam-interactive polish + the next horizon. **#454 list-first redesign:** the tab
lands on a filterable **directory of all 45 TMCs** (click a row to build, ← All
TMCs to return — the Program/Discipline dropdown dropped); "All colleges" = a
review view; the empty `planned`/"Coming soon" bucket retired (status = Official |
Draft); one consolidated filter block. **#455** pinned the global "What are you
working on" quick-start bar to the header (`quickstart.js`). **#456 the GE Breadth
companion — the full ADT = TMC major + GE pattern + electives:** new
`tmc_ge_patterns.js` (Cal-GETC primary per AB 928 + legacy IGETC/CSU-GE), rendered
by `renderGeInto` as a **manual** picker (GE areas are college-certified —
`ge:true`+`noncid:true`, no C-ID auto-match), combined Full-ADT total, saved in the
same `alignments` jsonb (no schema change). **#457** verified **Cal-GETC against
the official Standards v1.3** + fixed Area 3 (Arts & Humanities → 2 courses; pattern
now sums to **34 units**). Tests 53→55 files green. Lessons:
`docs/tmc_builder_lessons.md` (3 Session-60 sections). **NEXT:
`docs/session_61_handoff.md`** — verify IGETC/CSU-GE; the CPL-native TMC-slot wiring.
