...
| `tmc_college_courses.js` | Per-college course index (`window.CPL_TMC_COLLEGE_COURSES`: 120 colleges, 141,699 courses, 7.9 MB) powering the right-side pickers. Built one-shot by `tmc/_build_college_courses.py`. **Session 90 unioned the c-id.net authority** (`kb/reference/cid_articulations.json`) with COCI's `CIDNumber` — roughly DOUBLING C-ID coverage (10.6k→21.3k college×C-ID pairs; +9,924 courses gained a C-ID). A course can now carry MULTIPLE C-IDs: rows are `[subj,num,title,units,cid]` or `[…,cid,xcid[]]` (`cid`=primary/display, `xcid[]`=additional c-id.net C-IDs); the consumer matches a slot against `{cid}∪xcid` (`courseCids()`/`matchedCid()`) and `autoMatch` used-tracks so one course can't fill two slots. Join = `(college,subject,number)` exact + leading-zero fallback; `sequence:true` rows excluded; soft-fails if the c-id.net file is absent. Static — rebuild only on a fresh COCI/c-id.net extract; NOT in the daily `git add` list. |
...
- Auto-match is **C-ID-exact only** (course C-ID == slot C-ID/alt); never
  ...
  official c-id.net authority (`cid_articulations.json`, Session 90) — the union
  roughly doubles coverage, so far fewer colleges pick manually. A course can
  carry multiple C-IDs (`{cid}∪xcid`); `autoMatch` used-tracks to avoid filling
  two slots with one course. No contact-hours in COCI → legitimacy = units + C-ID
  for now.
