...
| **Activity↔Project PR-B/C/D** | PR-B: generator + renderer update — render Activities section + Projects section separately; project rows show "Contributes to: Activity N" chips; retire the hardcoded `activity_labels` dict as live source (keep as fallback). PR-C: editor + add-flow modal (Activity/Project radio + ladder fields + N-to-N multi-select). PR-D (optional): split into own top-level tab if the page gets dense (Sam's prior preference: one page with two sections). | queued (next session) |
