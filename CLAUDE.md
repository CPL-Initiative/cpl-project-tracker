...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **LIVE, REORDERABLE, AND NOW TWO-SIDED.** ⚠️ **The baked defaults in `cpl_funding_data.js` are STALE by design** — the live model is the Supabase overlay (`config.projects."cpl-implementation".scenarios."Scenario 1"`, camelCase). ✅ **SAM'S SAVED CONFIG (2026-08-21): `priorityOrder [2,0,1]` — Access 34%, Outreach 33%, Success 33%; factors all 1.0; front-load ON; Year-2 mirror ON; deadline 2026-11-01; all three metrics in CPL FTES.** ⭐ **The reorder is a PERMUTATION stored BESIDE the config**, never a rewrite (the live overrides are PARTIAL and `yearPriorities[slot]` is an object keyed by index string, so a rewrite that forgets a field re-points a priority at another identity's default). ONE translation seam: above it DISPLAY index, below it SOURCE index. ⚠️ **The order is WINDOW-LEVEL** (the years are deliberately identical); Year-2 sync is a reversible MIRROR, default OFF. 🆕 **A MAXIMUM ALLOCATION BESIDE THE MINIMUM** (SkyBound, #1293) — `pool.cap_window`, editable exactly like the floor, **$400,000**, `0` disables it. ⭐ **IT IS CLOSE TO A NO-OP FOR EQUITY AND THAT IS THE FINDING**: 6 colleges held, **$262,241 (1.1% of the pool)** moved to the colleges ranked ~7–70, and **the 45 at the minimum gain nothing** — a college pinned to the floor cannot be lifted by a ceiling. Range 3.48x → 2.67x. ⭐ **THE FLOOR IS THE LEVER** — a **$200,000 minimum reaches 1.88x alone** and makes a $400K ceiling non-binding; the floor's own limit is ~$210,785, the average award. ⭐ **THE TWO BOUNDS ARE SOLVED TOGETHER** (bisect `lambda` in `clamp(lambda*size, floor, cap)`), because a floor is monotone and a ceiling is not: pinning at the ceiling RELEASES money and lifts **5 of 50** floored colleges back off the floor, which a pin-as-you-go loop strands **with the pool still balancing**. Ceiling OFF reproduces the old loop bit-for-bit. ⚠️ **A BOUND ON THE MONEY IS A BOUND ON THE BAR** — a capped college's targets scale to **ceiling ÷ `plainRatio`**, not the bare ceiling, or the six largest get the only unsubsidized rate in a state where every unbound college pays a ~9% discount funding the floor; **cap ÷ target is ONE rate above the minimum**, and the clamp reaches **both** target paths (a student-unit priority never touches `prioEntitlement`). ⭐ **THE EXPLAINER IS AUDIENCE-FACING** (CO, possibly CA Finance): [How this funding model works](prototype/funding_model_explainer.html). ⚠️ **It is a SNAPSHOT — re-run `prototype/build_funding_model_explainer.js` and re-publish to the SAME artifact URL whenever the shares, factors, order or bounds change**; every figure is generated by the tab's own engine, never retyped, and its worked example is the largest **UNBOUND** college. ⭐ **Sam's framing rules, both earned the hard way:** never *"a ceiling, not a check"* — say **what a college receives is driven by its own CPL results, as they happen**; and **never frame the CO's share as withholding** — name what each amount BUYS, no crimson on money meant to be spent. **NEXT: ① Sam picks the ceiling number — $400,000 as shipped, or the floor raise that does more with one lever; ② Sam reads the explainer end to end in a browser** (no session can — egress-blocked from the Pages host); ③ whether the Year-2 mirror should be on for Scenario 2. Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) (2026-06→07 in [`..._archive.md`](docs/cpl_funding_lessons_archive.md)); durable [`a-second-bound-breaks-a-pin-as-you-go-solver`](docs/kb-notes/methodology-a-second-bound-breaks-a-pin-as-you-go-solver.md). |
...
| **College & district identity** | One taxonomy: every college/district name variant resolving to MAP's authoritative `college_id`, the CCCCO MIS district code, and every spelling any system uses. | ✅ **WRITTEN TO SUPABASE AND SURFACED AS A TAB** (SkyLink built it #1131–#1133; SkyVouch landed it #1278). `map_colleges` gains `district` · `mis_district_code` · `mis_college_code` · `district_type` · `mis_absent_why`, and **`variants` is populated for the first time: 0 → 118 of 128 rows** · 118 with a district · **73 districts** · 0 test rows touched. ⭐ **SCOPE IS EVERY ENTITY WE HOST A LANDING PAGE FOR** (Sam, 2026-08-21), not just credit colleges — and the exclusion was never in the builder, it was in the `--map-json` EXPORT filtered to `entity_kind='college'`. ⭐ **TWO NC INSTITUTIONS HAD REAL MIS IDENTITIES NOBODY HAD JOINED** — North Orange Continuing Education is `NORTH ORANGE ADULT` (863 under 860), SDCCE is `SAN DIEGO ADULT` (076 under 070); both pass the district-prefix invariant, so not lucky name matches. Partners (Futuro Health, Launch) carry **no** code and a stated **reason** — ⚠️ a missing MIS code is a finding only for a COLLEGE, or four permanent blanks push `unresolved` off zero for ever. ⭐ **THE LINT IS THE POINT, NOT THE MAPPING** — `--observed-json` feeds every college-name STRING in a live table; **13 resolved to no identity**. ⚠️ **IT FOUND A LIVE DEFECT: `map_college_contacts` holds `"Cypress College "` and `"San Jose City College "` WITH A TRAILING SPACE**, both carrying a real `primary_contact_email` and a named coordinator, neither exact-matching `map_colleges` — so both rendered as having NO CPL contact, silently. Fixed in the JOIN (`college_briefing.js` resolves through `variants`), **never by trimming the table** — it rebuilds from MAP nightly and a load must reproduce its source. ⚠️ **A VARIANT MUST NEVER SHADOW A CANONICAL NAME** ("Mission College" is a real college AND a plausible variant of "Los Angeles Mission College") — second pass, 0 collisions today, so the guard is PREVENTIVE. ✅ **SAM'S RULING (2026-08-21):** *"Calbright and LAUNCH get 2 entities—one credit, one noncredit. San Diego and North Orange are one entity."* Recorded as attributed DATA in `kb/reference/college_identity_rulings.json`, not hard-coded — a judgment a human made must survive a rebuild. ⚠️ **NEEDS MAP: `college_id` for `Calbright College Credit` + `Launch Apprenticeship Non-Credit`** — in neither `map_colleges` nor `map_college_users`; `college_id` is MAP's to assign, so minting one would fabricate an identity the whole system trusts. Reported `awaiting_map_id`. 🆕 **College Identity TAB live** — a LINT SURFACE, not a viewer: it makes ABSENCE visible (`variants` was empty on 128 rows for months and nobody could SEE it), labels every figure live / gated / snapshot, **checks its own snapshot against live counts**, and offers the shared `unlockRow` for the gated half. **NEXT:** Sam looks at the tab; MAP supplies the two ids; open question — district columns (done) vs its own `districts` table. Story: `docs/college_identity_lessons.md`. |
...
### SkyBound — the ceiling is real, and it is not the equity lever (2026-08-22, Session 184)

**Sam: move the explainer link to the title row; add a Max Funding factor beside the
$150K minimum, editable, set to $400,000; recalculate; *"push back and simpler
recommendations welcome."*** PR **#1293**.
⭐ **MEASURED BEFORE BUILDING, AND IT CHANGED WHAT THERE WAS TO SAY.** A $400,000
ceiling holds **6 colleges** and moves **$262,241 — 1.1% of the pool**, all of it to the
colleges ranked ~7–70. **The 45 at the minimum gain nothing**, because a college pinned
to the floor cannot be lifted by a ceiling; biggest gainer is Santa Monica at **+$7,079**.
Range compresses 3.48x → 2.67x. ⭐ **THE FLOOR IS THE LEVER**: a $200,000 minimum reaches
**1.88x on its own**, and at that floor a $400,000 ceiling never binds — one dial, not
two. (Floor's own limit ~$210,785, the average award.) Shipped at $400K as asked; the
number is a live dial and Sam's to move.
⭐ **THE SOLVER HAD TO CHANGE SHAPE.** A floor is **monotone** — pinning pushes everyone
else DOWN, so pin-as-you-go is safe forever. A ceiling runs the **other way**: pinning
RELEASES money and pushes everyone else UP, which lifts colleges back **off** the floor
(**5 of 50** at $400K). A second `if` in the old loop would have stranded them at
$150,000 **with the pool still balancing and every row inside both bounds** — invisible.
`allocModel()` bisects `lambda` in `clamp(lambda*size, floor, cap)`, then computes the
free rows with the waterfall's own arithmetic, so with the ceiling OFF it reproduces the
old loop **bit-for-bit** (`0.000e+0`), asserted against a transcription of it.
⚠️ **A BOUND ON THE MONEY IS A BOUND ON THE BAR** — capping the allocation and not the
target asks the capped college for ~40% more CPL per dollar and has the state asking for
more CPL than it funds. ⚠️ **Scale to ceiling ÷ `plainRatio`, not the ceiling**: every
unbound college already pays a ~9% discount funding the floor, so a bare scale would hand
the six largest the only unsubsidized rate in the state. **cap ÷ target is now ONE rate
for every college above the minimum** (`1.000000000000`, both bases). ⚠️ **The clamp must
reach BOTH target paths** — a student-unit priority never touches `prioEntitlement`.
⚠️ **A new check could not fail** (no rural college comes near $400K, so the rural bound
was vacuous — it passed against a deliberately broken `capFor()`); it runs at $160K now.
⚠️ Two amount inputs in one box shared **one accessible name**. ⚠️ **The explainer's
worked example had to move** to the largest **unbound** college — the largest is now
pinned to a round number its own share cannot explain; its typed `$5,060` rate is
computed now and rises to **$5,159**, the clearest statement of what the ceiling buys
everyone else. Explainer regenerated from the live config and **re-published to the same
artifact URL**.
Durable: [`a-second-bound-breaks-a-pin-as-you-go-solver`](docs/kb-notes/methodology-a-second-bound-breaks-a-pin-as-you-go-solver.md) · story `docs/cpl_funding_lessons.md` · handoff `docs/session_185_handoff.md`.
