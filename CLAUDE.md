...
| **MAP Custom Reports (3 new) / ITPI automation** | Wire the three new MAP Custom Reports — exhibit credit recommendations, the same **by Catalog Year**, and student details — and decide how they stay fresh. | ✅ **ALL THREE SERVED BY THE API AND WIRED** (SkyFetch, #1246/#1247/#1248). `View_CollegeExhibitCR_APIDataset` 174,223 · `View_CollegeExhibitCRByCatalogYear_APIDataset` 211,005 · `View_StudentDetailsCredits_APIDataset` 591,820 — API `dataCount` matches the builder **exactly** on all three, so builder and API are ONE cut. `REQUEST_PAYLOAD` is now **10 datasets**. ⭐ **THESE ARE OUR OWN SPEC COMING BACK** — the 13-column report IS Dataset A of [`docs/map_dataset_sql_for_malone.md`](docs/map_dataset_sql_for_malone.md), same columns same order, already loaded as `map_college_cr_unit`; so this was a RECONCILIATION, not a discovery. All three deltas run one way (+1.46% · +3.07% · +10.02%) = **our staleness resolving**, as `cpl_memory` predicted — confirm, don't file. ⚠️ **"Catalog year is the genuinely new dimension" was WRONG** — `map_student_credit.catalog_year` is **100% filled**, 9 values, 2019-20→2026-27. What was missing is catalog year at the ARTICULATION grain. ⭐ **THE 11-COLUMN REPORT IS DELIBERATELY NOT FETCHED** — it is the 13-column one with the grain collapsed and is derivable. Which to keep was MEASURED: dropping `course_type` costs **4 rows of 204,714**; dropping `catalog_year` costs **32,990 (16.1%)**. ⚠️ The spec warns loudest about `CourseType` ("~8% of rows") — that was a NARROWER key and does not hold. ✅ **StudentMAPID is SALT-HASHED** — Pedro Campos (ITPI) via Sam 2026-08-19, corroborated by enumeration (5M decimals + 8 variants ≠ a sampled hash). `Notes` stays held (free text, student grain, no consumer). ⚠️ **THREE status-shaped fields** in that view: `Status` = workflow stage · `CPLStatusPlan` = what the college DECIDED (the point) · `CPLPlanStatus` = **not a status**, a pipe-delimited checklist. ⚠️ **The payload IS the PII boundary** — minimisation is the ABSENCE of entries, one edit from being undone, so `tests/custom_report_payload_test.py` pins it (verified to FAIL, not just pass). ⭐ **ITPI push DECLINED on the merits** — three config lines beat a second writer, an issued credential and invisible failures. **OPEN:** does the salt ROTATE? (Sam asked Pedro; a rotating salt leaks nothing but silently makes student counts incomparable across runs — build the key-set overlap check at LOAD time regardless). **NEXT:** load the two new views into Supabase and reconcile per-college. Story: [`docs/map_custom_reports_lessons.md`](docs/map_custom_reports_lessons.md). |
