...
| **Local course ↔ CR alignment** | "Which of MY courses should I articulate against this credit recommendation, and how did other colleges do it?" — so faculty don't guess. | 🔬 **PROVEN OFFLINE, NOT BUILT (SkyBridge, 2026-08-13).** Sam's acceptance case — Cerritos on `ASME BPVC Section IX — FCAW Welder Qualification` (recs: Introduction / Advanced FCAW): **`WELD 214L` "Flux Cored Arc Welding (FCAW) Certification Laboratory"** tops both by a **2–3× gap**, corroborated by Barstow's near-identical `WELD 54B`. ⭐ **TWO SIGNALS, NEITHER SUFFICIENT ALONE** — Santa Ana mapped `WELD 240 Structural Welding SMAW` + `WELD 244 D1.1 Code Clinic` to FCAW recs; **neither title contains "FCAW"**, so title similarity alone would never propose them. Title finds the obvious match, **peer precedent finds the broader-course pattern**. Ships as a **proposal with its evidence attached**, never a determination. ⚠️ Candidates are **NOT scoped by TOP** — that would gate on TOP (Rule 7). **Three independent gaps:** ① M-ID leverage answers a narrower question and omits Cerritos entirely (its 12 welding courses sit in 12 *other* clusters); ② peer articulations with exact course numbers/titles are in `kb/coci_articulations.json` — **repo-only**; ③ **no per-course title table in Supabase**. ⚠️ **`coci_college_offerings` has TWO silent caps** — `titles_text` at **900 chars** (801 rows exactly there) and `sample_courses` at **8** (5,077 rows have more), so `search_college_offerings` **is blind to every title past the cap on its 801 largest rows** — a hole beyond this feature. **NEXT:** per-course table from the 141k COCI list, a peer-articulation table keyed by credential+rec, one RPC for both signals, cpl-chat wiring. Story: `docs/local_course_alignment_lessons.md`. |
...
⭐ **Alignment proven, and it needs TWO signals** — Santa Ana mapped courses whose titles contain no "FCAW".
Durable: `methodology-a-summary-field-can-be-a-retrieval-gate`, `methodology-two-signals-for-a-judgment-proposal`,
`methodology-an-unadopted-item-is-a-shelf-not-an-absence`.
