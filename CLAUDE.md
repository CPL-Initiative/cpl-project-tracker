...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **REORDERABLE, AND THE REORDER REWRITES NOTHING** (SkySort, #1268) — the order is a **PERMUTATION stored BESIDE the config**, never a rewrite of it, because the live overrides are **PARTIAL** (Scenario 2 sets `metric`/`share` on two and neither `factor` nor `title`) and `yearPriorities[slot]` is an **object keyed by index string**, not an array, so a rewrite that forgets a field silently re-points a priority at another identity's baked default. **ONE translation seam** (`prioField`/`prioMetricSource`/`prioUnit`/`setPrio` + `priorities()`): above it DISPLAY index, below it SOURCE index — `label` is positional, `key`/`src` are the identity. ⚠️ **The order is WINDOW-LEVEL** (Sam, 2026-08-09: the years are deliberately identical). ⭐ **Year-2 sync is a MIRROR, not a copy** — reversible, writes nothing, **default OFF**; an explicit *Copy Year 1 → Year 2* asks first. ⚠️ **The baked defaults in `cpl_funding_data.js` are STALE by design** — the live model is the Supabase overlay (`config.projects."cpl-implementation".scenarios."Scenario 1"`, camelCase). ✅ **SAM'S SAVED CONFIG (read 2026-08-21): `priorityOrder [2,0,1]` — Access first at 34%, Outreach 33%, Success 33%; all three funding factors 1.0; front-load ON; Year-2 mirror ON; deadline 2026-11-01; all three metrics in CPL FTES.** ⭐ **THE EXPLAINER IS AUDIENCE-FACING AND HAS BEEN TONE-REWORKED FOR IT** — [How this funding model works](prototype/funding_model_explainer.html), for CO and possibly CA Finance staff. ⚠️ **It is a SNAPSHOT — re-run `prototype/build_funding_model_explainer.js` and re-publish to the SAME artifact URL whenever the shares, factors or order change**; every figure is generated by this tab's own engine, never retyped. ⭐ **It states one thing the tab does not**: the $150k minimum is funded from inside the pot, so a college above it is effectively funded at **~$5,060/FTES rather than $5,649.63** (a floored small college reaches its target on far less — Lassen ~$23,900/FTES). ⭐ **EVERY FIGURE WAS TRUE AND IT STILL READ AS THE CO WITHHOLDING MONEY** (Sam, 2026-08-22, #1285–#1289) — fixed by **naming the beneficiary** of each amount, then **retiring the waterfall** (it reconciles an account, so it argues spending is loss — and it was crimson's only consumer on the page). It now opens on **$35,000,000** and three destinations: **$25,240,308** to the 115 colleges AND the four noncredit campuses · $8,959,692 statewide projects and technology · $800,000 two CO posts. ⭐ **Sam's earning-language ruling:** never *"a ceiling, not a check"* — say **what a college receives is driven by its own CPL results, as they happen**. ⚠️ **Retiring a section nearly deleted a figure** ($800,000 lived only in the waterfall) and **the boxes must SUM** — the residual is now *defined* as the appropriation minus the rest. ⚠️ **"Also say the noncredit campuses" is an ALLOCATION rule, not a blanket one** — they are correctly not in the credit-FTES split. **NEXT: Sam reads the explainer end to end in a browser before sending it out** (no session can — egress-blocked from the Pages host); then whether the Year-2 mirror should be on for Scenario 2. Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) (2026-06→07 in [`..._archive.md`](docs/cpl_funding_lessons_archive.md)). |
