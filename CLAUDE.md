...
| **SkyView / the CCR curation interface** | An interactive view of the Common Course Reference — common courses by discipline, their constituent local courses, and moving a course to where it belongs. | ✅ **PROTOTYPE LIVE, THE DRAG WORKS, AND THE ARITHMETIC THAT GOVERNS THE LANE** (SkyView #1309–#1312; SkyCal #1317). ⭐ **GRINDING THE WHOLE QUEUE PERFECTLY LANDS AT 35,937 — 14.4× SHORT OF 2,500.** Merging only compares what exists; **packaging** is the only mechanism with the right shape (ESL proved it at **85:1** — [`measure-your-mechanism-ceiling`](docs/kb-notes/methodology-measure-your-mechanism-ceiling-before-working-the-queue.md)). Target **≈17 per discipline**. ⭐ **THE CORPUS IS ~5,700 DECISIONS, NOT 17,321 ROWS** — **97.1% are ≤12 identities, modal 2**; median 12 per discipline, **118 of 144 have ≤40**. ⚠️ **3,001 have NO discipline** (8,065 identities) — a *different* job (assign one first) and half the queue; kept separate, never blended. ✅ **THE DRAG WORKS END TO END (SkyCal, #1317)** — member re-home (Session 54) writes `CN:<control number>`, honored in `excel_to_dashboard.py`, **never re-mints** (Rule 7 not engaged); still **zero `CN:` rows**, nothing written to Supabase. **101,063 member courses over 16,240 identities** now ship in a SECOND 2.5 MB payload (`ccr_universe_members.json`; record `[control_number, code, college index]`, **no title** — 5.5 MB with, 2.5 without). ⚠️ **The verb had also never WORKED: `pointerdown` clobbered the carried course**, so Drag… → click-the-destination moved nothing — unobservable with no course on screen, which is the other half of what zero `CN:` rows meant. ⚠️ **`nd.n` is NOT a college count** (`ESOL M9168` read *"1,152 colleges"* against 123 colleges; it disagrees with the carried members on **3,399 of 16,242** rows) — both displayed, carried count leading. ⚠️ **1,122 control numbers sit under MORE THAN ONE identity**, so a move is a GLOBAL statement and the course leaves **every** card; 2 carry no control number and are **dropped and counted**, never coerced to zero. ⚠️ 850 members render **200 with a filter and the denominator** — a capped list must not read as a census. Chromium harness + `tests/ccr_universe_members_test.py` (CI); every fix perturbed red, and **three of the harness's own first four failures were the harness** ([`a-blocked-path…`](docs/kb-notes/methodology-a-blocked-path-hides-the-defects-behind-it.md)). ⭐ **FOREST IS A MAP, TREES ARE GRAPHS** — a canvas with a **precomputed stable layout**, keyword fly-to, **draggable islands**, cross-area moves. Sam's reason for reversing the per-decision design was right: **a per-decision view structurally cannot show a cross-area move.** ⭐ **COLOR CARRIES THE IDENTITY SYSTEM** (Sam): ★ C-ID · ◆ CCN · ✳ M-ID · ○ unified, **state on a separate rim dot**. ⭐ **MERGE CHAINS RESOLVE TRANSITIVELY** (#1312) — a `merge_into` is a DECISION and sameness composes; *grouping is by KEY, never transitive* governs SIMILARITY edges ([note](docs/kb-notes/methodology-transitive-closure-is-right-for-decisions-and-wrong-for-similarity.md)). ⚠️ **A cross-discipline merge is ambiguous between a wrong MERGE and a wrong DISCIPLINE, and the repairs are opposite** (2,731 of 10,170; top pairs are siblings = a VOCABULARY signal). **The view can move a course and cannot relabel an island.** **NEXT:** ① **Sam drives the drag in a browser** — no session can (egress-blocked); density and the drop affordance are his calls. ② **Step 2 of the scope: the QUEUE** — a drag leaving the destination's SUBJ4 inconsistent with its corroborated discipline queues a re-mint candidate; **proposes, never auto-adds**. ③ The **1,122 duplicate-claim courses** are a worklist of their own. ④ The 67 Z-scheme `ESOL Z####` rows the fold never touched; re-home `FIMS M1018` (still blocked — it does not render, so it needs the un-merge verb); the one-college-many-numbers rule; a tool for the 3,001. ✅ **The Beginning spot-check and the survivor-member audit are DONE** — see the ESL packaging row. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md). |
| **ESL packaging (the first fold)** | Collapse the ESL discipline to comprehensives + carve-outs — the proof that packaging reaches the target. | ✅ **APPLIED, PUBLISHED AND SPOT-CHECKED — ESL went 2,300 identities → 27** (SkyView #1311/#1312; spot-check Sky188 #1315). Cohort `package-esl-s187@bot`, 1,997 rows, receipts `kb/esl_package_out/2026-08-24/` + `kb/esl_fold_spotcheck/2026-08-24/`. Survivors: Beginning `ESOL M9168` · Intermediate `ESOL M9256` · Advanced `ESOL M1141` · Vocational `ESOL M9023` · Civic `ESOL M9177` · Enrichment `ESOL M1152` · Voc—Healthcare `ESOL M91IL`. ⭐ **THE MECHANISM IS SURVIVORS, NOT NEW IDS** (Sam) — `merge_into_orphan` self-trusts only `UC-CUR-*` and Session 56 re-minted all 4,053 away, so a Z-scheme target would flag as an orphan forever. ⭐ **CALIBRATE THE SIGNAL, DON'T RANK BY INTUITION** — the fold read only the modal TITLE while the COCI export describes **96%** of members in prose. Wrong rate over rows a description can CHECK: `default-beginning/medium` **76.7%** · `numeric/medium` **49.2%** · `combo/high` 12.5% · `word/high` **6.2%**. **`numeric` is a coin flip and had been ranked BELOW `default-beginning`** — 94 rows were about to be skipped. ⚠️ **The denominator is rows the source can DECIDE** — 1,217 folds assert nothing either way and are EXCLUDED, never counted as agreement. ✅ **SAM RATIFIED NEW BANDS 2026-08-24 — `0-2 Beginning / 3-5 Intermediate / 6-10 Advanced`, superseding the P-4 pinning** (*"Level 6 ESL can go in Advanced"*). Dry-run `kb/_esl_relevel_dryrun.py` → **32 re-levels** (31 Adv→Int, 1 Beg→Adv); six of the nine over-claims resolve, all agreeing with the college's catalog, and `ESOL M1050` stays Advanced per his call. ⚠️ **His bands measure WORSE against local catalogs (49.2% → 54.5%) and that is NOT an objection** — the catalogs are ~116 colleges disagreeing with each other (rung 1 splits 47/44), and a statewide mapping exists to override local variance; the number is **blast radius**, not a verdict. ⚠️ **Extending the reader to 0-10 needs THREE guards, each from live data**: a level WORD beats a number (`Beginning Skills 9`), a grade range is not a level (`K-12`), roman numerals stop at VII (`Beginning Skills 2 X` reads the trailing X as 10) — `tests/esl_relevel_bands_test.py`, all three proven to fail when broken. **THE APPLY IS NOT RUN — dry-run only, nothing in Supabase.** ⭐ **DIRECTIONAL ERROR BEATS AGGREGATE: 85 under-claim, 9 over-claim**, and the SMALL half is the one to work — all 9 are the `5+ → Advanced` cut sitting ONE RUNG too low for 6-rung ladders (LACCD *College ESL V*, 8 + 7 colleges). ⚠️ **A local course NUMBER is not a level ordinal** (parallel credit/noncredit schemes; a ladder was built and rejected at 325 false proposals). ⚠️ **A purpose bucket is not a level bucket** — 45 rows name a level inside Enrichment/Civic/Vocational; re-pointing strips the carve-out. ✅ **Survivor-member audit clean** (live-verified): 7 pre-existing members, 1 non-ESL (`FIMS M1018`), the four big level survivors at **zero**. **NEEDS SAM: ① the 9 over-claims, or move the cut to `6+` (fixes 6 by rule); ② does the numeric pinning survive at all.** 222 proposals staged, **nothing written to Supabase**. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md); durable [`calibrate-a-signal-before-you-rank-the-queue`](docs/kb-notes/methodology-calibrate-a-signal-before-you-rank-the-queue.md). |
