...
| CSC-E | Common Subject Code tab — local-variants data refresh. Variants column currently shows MID-aggregated SUBJ4 (uninformative post-canonicalization). Walk `kb/coci_minted_memberships.json` raw local college Subject codes, aggregate per discipline as `local_subject_variants` in seed. | queued (Bruh Quad / next) |
| CSC-F | Global column centering across dashboard tables (CCR, CSC, KPI cards, projects grid, exhibit analysis). Center H+V except first column. Prototype on CSC first; sweep after eyeball review (some tables have asymmetric column intent that may break). | queued (Bruh Quad / next) |
| **SEC-baseline** | Security tooling baseline — Dependabot (pip + github-actions, weekly), CodeQL (Python + JS, push/PR/weekly), TruffleHog secret scan (`--results=verified` to avoid false-positives on public Supabase anon keys). All free for public repos. 5 initial Dependabot PRs all green; stage 1 (pip bumps #105/#106) merged this session. | **DONE** 2026-05-23 (PR #101, Bruh Quad) |
