...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **LIVE, TWO LANES, ONE SOLVER.** ⭐ **NEVER READ THE CONFIG — CALL `_effective()` / `_alloc()` / `_nc()` / `_prios()`** (#1359). Baked defaults in `cpl_funding_data.js` are **STALE BY DESIGN**, `mirrorYears` makes the stored Year-2 block **INERT** (says factor 1, model uses **0.5**), and `yearPriorities` is SPARSE. Dials live in Supabase `cpl_funding_config`; dump them with `scripts/funding_effective.js --config <file>` — do not transcribe a snapshot here, it only goes stale. ✅ **Sam's dial RULINGS** (values themselves: read the config): factor **0.5 in BOTH lanes**, no discount for NC · **$60k NC floor is INFEASIBLE**; at $50k, 30 of 33 floored, break-even 3,909 FTES (`--nc-sweep`). ⭐ **BOTH BOUNDS SOLVE TOGETHER** — `solveBounded()` bisects lambda in `clamp(lambda*size, floor, cap)`; `cpl_funding_cap.test.js` guards it. **A ceiling cannot lift a floored college — the floor is the only real equity lever**, priced in the earn rate of the middle. ⚠️ **A floor the pool cannot honor SAYS SO** (`floorInfeasible`, #1302). ⚠️ **NEVER RE-DERIVE AN ALLOCATION — call the model, in BOTH lanes** ([`a-total-that-balances-is-not-a-total-that-is-right`](docs/kb-notes/methodology-a-total-that-balances-is-not-a-total-that-is-right.md)). ⚠️ **NC money never sums into the credit total** (Sam: *"the neglected step child"*). ⚠️ **A DEDUP HAS A SCOPE**: Mt. SAC NC's FTES is the duplicate, the institution is not. ⭐ **EXPLAINER LIVE** at **`/funding-model/`**. ✅ **THE NC LANE EARNS LIKE CREDIT** (Sam, 2026-08-26) — a CAP earned against the CR three, origin-filtered. ⭐ **ROUTE, DON'T SPLIT**: a unit earns in ONE lane by origin and counts three times INSIDE it; **`share` splits the MONEY, not the FTES**. Leakage UNDERcounts NC rather than double-paying — the incentive to use the NC landing page. ✅ **DRAFT-MODEL POSTURE (Sam, 2026-08-27):** a missing upstream field is NOT a blocker and NOT a design flaw — *"freedom to play to pay"* — but values must **calculate correctly on available data**, so an ABSENT number is fine and a **plausible wrong one never is**. ✅ **STEP 1 SHIPPED (#1363/#1364):** a priority pins its measure with **`metric_src`** (registry `METRIC_SOURCES`; `""` un-pins; an unknown pin is a loud `bad_src` earning **$0, never a full-cap advance**; a declared-but-undelivered source is `undelivered` — $0, not an advance). **Two** agreement checks now: UNIT and **MILESTONE**. ⚠️ **A pin belongs with its metric, NOT in the bake** — the bake's slot 2 is a *headcount/transcribed* metric, so pinning there turned 9 assertions red. ⛔ **The live Access metric was measuring the wrong people — FIXED.** It asks for APPLIED units from portal/landing page/batch upload; prose-matching sent it to `pp_u` (portal-origin **TRANSCRIBED**, 25 units on 3 colleges), so **all 115 colleges read $0** on the largest share (0.34 = **$7,969,705**). ⚠️ **And `pa` was the WRONG HALF** — pe/pa/p2/p3 all carry `and not is_potential`, so they **EXCLUDE** portal students; `pa` and `ppa` are **DISJOINT**. New **`ppa`/`ppa_u`** (applied units where `Potential Student = Yes`, Sam's temporary origin flag) is live: **108 students / 661.5 units / 52 colleges**; Access now **12 earning, 0 "no feed"**. ⏭ **MAP is shipping `Origin`** (Student Portal / Landing Page / Batch / College Entered) **+ `LocID2`** for the NC location; `ppa_u` then narrows to the named origins and the `nc_*` measures become the same rung cut by LocID2 — one builder change, no consumer change. Sam: **"all three count the NC FTES only if they have a Yes."** ✅ **ROW SHAPE RULED (Sam, 2026-08-27): Option A — a second NC row per college**, CR/NC chips, one grid per cell so Tgt/Now align exactly, abbreviated `$99.7K`, bold only on Total. **NEXT: the NC shares (nothing ruled — credit's 34/33/33 is the default), then NC earning + the Option-A row.** Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) ([archive](docs/cpl_funding_lessons_archive.md)); durable notes in [`docs/INDEX.md`](docs/INDEX.md). |
