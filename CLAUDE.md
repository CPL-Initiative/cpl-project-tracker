...
| **GR register / CO policy & regulation review** | One place where every Chancellor's Office priority area's regulatory and Ed. Code revisions under consideration live, with the source artifacts informing them — pointed at the whole CO, not just CPL. | 🔨 **BUILT, NOT YET DEMOED** (Sky168). Sam is showing it to **CO General Counsel** as a possible instrument for all their policy/reg reviews. ⭐ **A DOCUMENT BECAME A REGISTER** — it was ONE hardcoded jsonb blob (`gr_content`, `DOC_ID='cpl-t5-priorities'`), one topic, `writes: []`. Now `gr_areas → gr_revisions → gr_artifacts`; every ask falls out as a column. `gr_content` LEFT IN PLACE as the rollback copy — the migration copied its 16 priorities out and deleted nothing. **2 areas · 20 revisions · 2 artifacts · 14 sections across 3 codes**; `dual-enrollment` is a clearly-marked **SAMPLE** (neutral review prompts, never positions) so the demo need not show MAP's advocacy aimed AT the CO. ⭐ **CITATIONS ARE DATA — that is the load-bearing change.** A section dropdown cannot exist while §55050 lives only inside a sentence; *filter / facet / group-by* are all one request: **make the attribute addressable**, which is a schema change, not a UI task. ⚠️ **§11342.2 is GOVERNMENT Code** (the APA definition of *regulation*) — a `5xxxx→T5 / else→EC` rule would have fabricated a citation **in front of lawyers**; codes are assigned by EXPLICIT range and anything unmatched is left unassigned. Backfilled citations carry `citations_derived` and render dashed until a human edits them. Dropdowns are built from what is **actually cited** — a code with nothing cited gets NO dropdown, never an empty one. ⭐ **THE CROSS-AREA SECTION INDEX is the reason GC would adopt this** (not asked for): once citations are data, *which sections do two priority areas both propose to amend?* is one query — dual enrollment, CPL, SCFF and baccalaureate all edit overlapping Title 5, and nobody sees the collision until rulemaking. No full-text search over the same prose could produce it. ⚠️ **"Guidance Memo / Title 5 Rulemaking" are NOT artifacts** — they are the three PATHWAY tiers (how a change gets made); artifacts are a new axis, kept separate. ⚠️ **The Knowledge Base already exists and is NOT this tab's** — `cpl_documents` (62) + `cpl_document_sections` (525, pgvector) is **Sierra's** RAG corpus, fed by an EXTERNAL indexer from Sam's private vault, last indexed **2026-07-19**, and the GR tab reads none of it; there is no browser path into it, so artifacts are scoped to link + division + a human `why` and the form says nothing is auto-analysed. 🔒 **WRITES ARE REVIEWER-ONLY** — a shared phrase is a bearer credential with no identity, no per-person revocation and no answer to *who changed this row*. ⚠️ **BLOCKED ON SAM — the GR phrase opens EVERY shared team tab** (`team_pass_check()` matches ANY secret), so inviting GC today also hands them the Workplan, Budget, Memory, MAP Users and Governance. Same defect as Finance (§11 *Org & phrase scope*), now on the critical path — and the tab was **built to lock the CO out**. ⚠️ `cobi_admin_surface.js` is GENERATED and its scanner **reads comments** — spelling the URL pattern out in prose invented a table called `"table"` in the RLS inventory; a phantom row in a security view is the same defect as a missing one. **NEXT:** ① the phrase-scope fix (gates the invite); ② **per-field sensitivity** — which sections are under review is public law, the POSITION is what is sensitive, so a `sensitivity` column + view lets the collision index go CO-wide while positions stay restricted (this is what turns a MAP tool into a CO tool); ③ edit + version history, which a legal audience needs; ④ load the authoritative CO priority-area list. Story: [`docs/gr_register_lessons.md`](docs/gr_register_lessons.md); durable [`methodology-a-filter-needs-a-field`](docs/kb-notes/methodology-a-filter-needs-a-field.md). |
