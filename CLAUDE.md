...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **LIVE, TWO LANES, ONE SOLVER.** ⭐ **NEVER READ THE CONFIG — CALL `_effective()` / `_alloc()` / `_nc()` / `_prios()`** (#1359). Baked defaults in `cpl_funding_data.js` are **STALE BY DESIGN**, `mirrorYears` makes the stored Year-2 block **INERT**, and `yearPriorities` is SPARSE. Dials live in Supabase `cpl_funding_config`; dump them with `scripts/funding_effective.js --config <file>` — never transcribe a snapshot here, it only goes stale. ✅ **Sam's dial RULINGS** (values: read the config): factor **0.5 in BOTH lanes**, no NC discount · **$60k NC floor INFEASIBLE**; at $50k **30 of 33 are floored**, break-even 3,909 FTES (`--nc-sweep`). ⭐ **BOTH BOUNDS SOLVE TOGETHER** (`solveBounded()`; `cpl_funding_cap.test.js` guards it). **A ceiling cannot lift a floored college — the floor is the only real equity lever**, priced in the earn rate of the middle. ⚠️ **A floor the pool cannot honor SAYS SO** (`floorInfeasible`, #1302). ⚠️ **NEVER RE-DERIVE AN ALLOCATION — call the model, in BOTH lanes** ([`a-total-that-balances-is-not-a-total-that-is-right`](docs/kb-notes/methodology-a-total-that-balances-is-not-a-total-that-is-right.md)). ⚠️ **NC money never sums into the credit total** (Sam: *"the neglected step child"*). ⚠️ **A DEDUP HAS A SCOPE**: Mt. SAC NC's FTES is the duplicate, the institution is not. ⭐ **EXPLAINER LIVE** at **`/funding-model/`**. ⭐ **ROUTE, DON'T SPLIT**: a unit earns in ONE lane by origin and counts three times INSIDE it; **`share` splits the MONEY, not the FTES**. Leakage UNDERcounts NC rather than double-paying. ✅ **DRAFT-MODEL POSTURE (Sam):** a missing upstream field is NOT a blocker and NOT a design flaw — *"freedom to play to pay"* — but an ABSENT number is fine and a **plausible wrong one never is**. ✅ **A priority PINS its measure** with **`metric_src`** (registry `METRIC_SOURCES`; `""` un-pins; an unknown pin is a loud `bad_src` — **$0, never a full-cap advance**; declared-but-undelivered is `undelivered` — $0, not an advance). Agreement checked on **two** axes: UNIT and **MILESTONE**. ⚠️ **A pin belongs with its metric, NOT in the bake.** ✅ **Access is FIXED, measured on `ppa_u`** (#1363/#1364 — applied units for portal-origin students; `pa` was the **DISJOINT** half, not a superset). ⏭ **MAP is shipping `Origin` + `LocID2`**; `ppa_u` then narrows to the named origins and the `nc_*` measures become the same rung cut by LocID2 — one builder change, no consumer change. ✅ **THE NC LANE EARNS — BUILT (Sky200).** A **second NC row per college** (Option A: CR/NC chips, Tgt/Now aligned, bold only on Total). NC priorities are credit's three re-pointed at `nc_pe_u`/`nc_pa_u`/`nc_pt_u`, **shares 34/33/33 (Sam, 2026-08-27 — same as credit)**, factor 0.5 inherited, with an `ncPriorities` override layer. ⭐ **THE PIN IS COMPULSORY HERE** — NC inherits credit's WORDING, so an unpinned NC priority prose-matches a **CREDIT** source and scores noncredit money on credit performance. **The coupling runs both ways**: the NC rung comes from the CREDIT resolution, so the `ppa_u` pin is load-bearing for BOTH lanes. ⚠️ **NC MUST NEVER ADVANCE** — the artifact-not-loaded branch pays the FULL CAP, so it is lane-guarded (without it a slow load disburses the whole **$1.8M**). ⚠️ **A lane normalizes on its OWN share sum**, and **a share set MULTIPLIES the pot rather than normalizing it** (1.30 places 1.30 × W — in credit too). ⚠️ **`fmtCountK` misstates an NC target** (1–25 FTES; 1.4 → "1") — **caught in Chromium, not jsdom**. ⭐ **Live: exactly ONE of the 33 institutions earns proportionally, and $0 is earned across all 33.** **NEXT: ① Sam looks at the row (density + tint are his); ② the credit-lane variant of the rounding; ③ NC's own strategy text — Year-1 P-Success still lists "noncredit mirror courses" among CREDIT strategies.** Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) ([archive](docs/cpl_funding_lessons_archive.md)); durable notes in [`docs/INDEX.md`](docs/INDEX.md). |
