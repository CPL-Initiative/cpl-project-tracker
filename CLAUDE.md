...
| **CCR Atlas / the curation interface** | An interactive view of the Common Course Reference — common courses by discipline, their constituent local courses, and moving a course to where it belongs. | ✅ **PROTOTYPE LIVE + THE ARITHMETIC THAT GOVERNS THE LANE** (SkyView, #1309/#1310/#1311). ⭐ **GRINDING THE WHOLE QUEUE PERFECTLY LANDS AT 35,937 — 14.4× SHORT OF 2,500.** Merging only compares what exists; **packaging** is the only mechanism with the right shape, and the ESL dry-run already proved 10.7:1 inside one discipline. Design target: **2,500 ÷ 144 disciplines ≈ 17 common courses each** ([`measure-your-mechanism-ceiling`](docs/kb-notes/methodology-measure-your-mechanism-ceiling-before-working-the-queue.md)). ⭐ **THE CORPUS IS 6,056 DECISIONS, NOT 17,321 ROWS** — collapse the six lanes into connected components: **97.1% are ≤12 identities, modal size 2**, 144 disciplines carry work at **median 12 decisions**, **118 of 144 have ≤40**. Automotive Technology is 57; Welding 35. ⚠️ **3,001 of the 6,056 have NO discipline** (8,065 identities) — a *different* job (assign a discipline first) and half the queue; kept separate, never blended. ⭐ **THE DRAG VERB ALREADY EXISTED AND HAD NEVER BEEN USED** — member re-home (Session 54) writes `CN:<control number>`, is honored in `excel_to_dashboard.py`, tested, reversible, and **never re-mints an M-ID** (Rule 7 not engaged); **zero `CN:` rows** against 28,797 identity-level rows. You cannot drag a course into a cluster you cannot see. ⭐ **FOREST IS A MAP, TREES ARE GRAPHS** — a global force graph is a hairball (Sam's own Obsidian shot is illegible at ~500 nodes); 158 discipline cells → one discipline's decisions → one 2–12 node graph. ⚠️ **The graph must be on the FIRST screen** — it shipped two clicks down with the grid sorted by decisions, so the top cells had no data and the obvious click was a dead end. ⚠️ **A happy-path selector cannot see a dead end** (the check clicked `.cell.demo`). ⭐ **COLOR CARRIES THE IDENTITY SYSTEM** (Sam): ★ C-ID · ◆ CCN · ✳ M-ID · ○ unified, with **state on a separate rim dot** — folding state into the fill would make a flagged C-ID stop looking official. ⭐ **THE UNIVERSE VIEW IS BUILT (#1312), and Sam's REASON reversed the design call:** *"courses are mismatched in the wrong subject area and may need to be dragged to a course in another area… pull an area over closer to the clusters in another area."* **A per-decision view structurally cannot show a cross-area move**, so the universe view is not a nicety — it is the only place that repair happens. All **17,321 identities on canvas** (SVG would be ~70k DOM nodes), layout **precomputed and stable** (one that re-solves per load is unnavigable — you cannot learn where anything is), islands **draggable** so two distant subjects can be brought side by side. ⚠️ **Labels are COLLECTED then placed with collision rejection** — drawing each as its island drew reproduced the exact unreadable pile of Sam's Obsidian screenshot. ⭐ **THE OVERLAY STORES ONE HOP PER ROW, AND THE DISPLAY READ HOPS AS GROUPS (#1312).** A curator merges X into Y; a later pass merges Y into a comprehensive; Y is then a source AND a target, and the merge-target loop never skipped a target that was itself a source — so **340 folded identities rendered as their own rows** while their members stayed stranded mid-chain. **Only 96 are ESL-fold rows: 180 come from the title lane, 60 from Sam's own curation, and 248 of the 340 are not ESL at all** (Kinesiology 42, Fire Technology 14, English 14, Art 12). The fold did not cause it, it made it visible. `flatten_merge_chains()`: ESL **169 → 77**, all rows **16,824 → 16,484** (exactly the 340), **0 rows appeared**, comprehensives absorbed their stranded members. ⚠️ **This is NOT a violation of "grouping is by KEY, NEVER transitive"** — that rule governs SIMILARITY edges, which are measurements and do not compose; a `merge_into` is a DECISION, and sameness does ([`transitive-closure-is-right-for-decisions`](docs/kb-notes/methodology-transitive-closure-is-right-for-decisions-and-wrong-for-similarity.md)). ⚠️ **The first cut wrote `merge_into[src] = src` on a cycle** — a row that is a member of itself, worse than the stale edge because it is invisible; measured first (22,538 direct · 490 two-hop · 18 three-hop · **0 cycles**). ⚠️ **A cross-discipline merge is ambiguous between a wrong MERGE and a wrong DISCIPLINE, and the repairs are opposite** — 2,731 of 10,170 judgeable merges cross a discipline but the top pairs are siblings (CIS↔CS 107, Law↔Legal Assisting 67, Art↔Photography 53), a VOCABULARY signal; the one non-sibling pair, Ethnic Studies↔Kinesiology (73), turned out to be right merges under a wrong label (`ETHS M1227` is "Intercollegiate Women's Flag Football"). The universe view has a verb for moving a course and **none for relabeling an island**. **NEXT:** the 794-row spot-check; re-home `FIMS M1018` as the first real drag; a survivor-member audit before the next packaging pass; the one-college-many-numbers rule; the 3,001 no-discipline decisions need their own tool. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md). |
| **ESL packaging (the first fold)** | Collapse the ESL discipline to comprehensives + carve-outs — the proof that packaging reaches the target. | ✅ **APPLIED AND PUBLISHED 2026-08-24 — ESL went 2,300 identities → 27** (SkyView, #1311/#1312), cohort `package-esl-s187@bot`, 1,997 rows, receipt `kb/esl_package_out/2026-08-24/`. **Beginning `ESOL M9168` · Intermediate `ESOL M9256` · Advanced `ESOL M1141` · Vocational `ESOL M9023` · Civic `ESOL M9177` · Enrichment `ESOL M1152` · Vocational—Healthcare `ESOL M91IL`.** Far past the 10.7:1 the dry-run projected; against a target of ~17 comprehensives per discipline **ESL is essentially done.** ⭐ **THE MECHANISM IS SURVIVORS, NOT NEW IDS** (Sam): an existing identity is chosen and RENAMED. The code decides this, not taste — `merge_into_orphan` self-trusts exactly one prefix, `UC-CUR-*`, and Session 56 re-minted all 4,053 of those away (**live count: 0**), so a Z-scheme id would flag as an orphan on every audit run. Survivor choice is a STATED RULE: for a LEVEL, dominant SUBJ4 → title opening with the plain level word → most colleges; for a NEW comprehensive there is no level to encode and the title is replaced anyway, so **adoption decides** (ranking by the level word gave a 2-college anchor; by colleges, a 12-college one). ⚠️ **`ON CONFLICT DO NOTHING` BLOCKED THREE OF THE SEVEN RENAMES** — merges landed, NAMES did not. **1,994 landed where 1,997 was planned, and that three-row gap was the whole story**; a write reporting only "no error" would have hidden it. One blocker was **Sam's own row** (`ESOL M9177`), superseded EXPLICITLY per Rule 8, reported the same turn, and **confirmed by him the same day** — the Rule 8 round trip working end to end. ⚠️ **AUTOMOTIVE VESL DOES NOT EXIST** — Sam's own example for the subject split has **zero rows**; **78% of the Vocational lane (90 of 116) is generic workplace English**, residue 26 across eight subjects (largest Healthcare, 6). **Measure the example before building for it.** ⭐ **PACKAGING HIDES PRE-EXISTING OVER-MERGES** — `FIMS M1018` *"Film and American Culture"* (film studies, TOP 0612.00) was bot-merged into `ESOL M1152` *"American Culture and Film"* (credit ESL, TOP 4930.87), the same words reordered. It was 1 of 2 members and conspicuous; it is now **1 of 35** inside Enrichment ESL and looks like nothing. **Audit a survivor's EXISTING members before folding** — afterwards the evidence is diluted by design. ⚠️ **53% of the fold is medium confidence and concentrated: 794 Beginning rows got there by DEFAULT** (no level word) — **the open spot-check**, now correctable by dragging. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md). |
