...
| **CPL clean-up worklist** | What to fix in the CPL data, in what order, and who fixes it. | ✅ **LIVE — `map_cleanup_worklist`**, a TABLE rebuilt inside the nightly promotion (#1256/#1257). Authority: [`docs/map_cleanup_worklist.md`](docs/map_cleanup_worklist.md) — read it before acting. ⭐ **BUILT FOR THE CUSTOMER SUCCESS TEAM** — Natalie Powell (lead), Chelsea Mirada, Ally Barker; carries the **college contact for that class of work** (AO/faculty for the approval queue, CPL coordinator otherwise). ⚠️ **THEY NEED THE TEAM PHRASE, NOT REVIEWER ACCESS** — reviewer is all-or-nothing and also reaches `map_student_credit` (student grain), `kb_curation`, `gr_*` and **`team_access` itself, so a reviewer can rotate every phrase**. ⭐ **The gate dropped by MATERIALISING, not by weakening** — as a view it was permanently reviewer-only, and relaxing `security_invoker` looks identical in a diff and is opposite in effect; **`G9` refuses to finish the promotion if the rebuilt table loses that gate.** ⚠️ **No k-anonymity — internal tool, NEVER a public surface.** ⚠️ **12 colleges have work and NO contact to call** (Citrus 109 · Allan Hancock 99 · Saddleback 63 · Hartnell 53 …) — corroborates the MAP Users gap. ⭐ **RANKED BY DECISIONS, NOT ROWS** — an `effort_shape` per class (`one rule` / `per row` / `upstream` / `needs a ruling`), because sorting by row count inverts the order. ⚠️ **A class marked `one rule` must be checked against its own TEXT before one instruction is written for it** — grouping by a predicate and naming the group after its commonest reason is how **5,311 ACE deferrals got a refusal's instruction** ([`methodology-a-one-rule-class-must-be-checked-against-its-own-text`](docs/kb-notes/methodology-a-one-rule-class-must-be-checked-against-its-own-text.md)). **P1 cannot yield credit — 12,283 rows / ~100 colleges / ZERO units** (one rule everywhere). ⚠️ Clearing it **raises every disposition rate without awarding a unit** — say so when the rate moves. **P2 Transcribed with no units — 14,348 rows / 4,196 students**; ⚠️ biggest by rows, smallest by effort — **94% is LA Pierce + Merced**, and it is an **ingest gap**, not curation. **P3 Applied with zero applied units — 413 rows.** **P4 approval cascade — 2,225 articulations** (Faculty 1,055/45 colleges · **Initiator 1,026 across only NINE** — a stalled queue, not a statewide condition · AO 144); not a defect, and the only class naming its own owner. **P5 `credit MAY be available if the college evaluates` — 5,311 rows / 101 colleges / owner Sam, `needs a ruling`.** ACE is **deferring to the college, not refusing** ("credit may be granted on the basis of an individualized assessment / institutional evaluation"), so **NEVER bulk-rule it Not Applicable**; zero units is the MECHANISM of a deferral, not evidence of a denial. Rank = **READINESS**, not value — it is the only class here that could still become credit for a student. **NEXT:** ① P1 as one instruction to ~100 colleges — **safe to send now, was not before the split**; ② **Sam's ruling on P5** before anything about those rows reaches a college; ③ P2 to Pierce + Merced directly; ④ ask the nine Initiator colleges what is stalling. |
