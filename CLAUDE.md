...
| **CCR Atlas / the curation interface** | An interactive view of the Common Course Reference — common courses by discipline, their constituent local courses, and moving a course to where it belongs. | ✅ **PROTOTYPE LIVE + THE ARITHMETIC THAT GOVERNS THE LANE** (SkyView, #1309–#1312). ⭐ **GRINDING THE WHOLE QUEUE PERFECTLY LANDS AT 35,937 — 14.4× SHORT OF 2,500.** Merging only compares what exists; **packaging** is the only mechanism with the right shape, and ESL then proved it at **85:1** ([`measure-your-mechanism-ceiling`](docs/kb-notes/methodology-measure-your-mechanism-ceiling-before-working-the-queue.md)). Design target: **2,500 ÷ 144 disciplines ≈ 17 each**. ⭐ **THE CORPUS IS ~5,700 DECISIONS, NOT 17,321 ROWS** — collapse the six lanes into connected components: **97.1% are ≤12 identities, modal size 2**, median 12 per discipline, **118 of 144 have ≤40**. ⚠️ **3,001 have NO discipline** (8,065 identities) — a *different* job (assign one first) and half the queue; kept separate, never blended. ⭐ **THE DRAG VERB ALREADY EXISTED AND HAD NEVER BEEN USED** — member re-home (Session 54) writes `CN:<control number>`, is honored in `excel_to_dashboard.py`, tested, reversible, **never re-mints** (Rule 7 not engaged); **zero `CN:` rows**. You cannot drag a course into a cluster you cannot see. ⭐ **FOREST IS A MAP, TREES ARE GRAPHS** — a global force graph is a hairball; the universe view is a **canvas** (SVG would be ~70k DOM nodes) with a **precomputed stable layout** (one that re-solves per load is unnavigable), keyword fly-to, **draggable islands** and cross-area moves. Sam reversed the per-decision design and his reason was right: **a per-decision view structurally cannot show a cross-area move.** ⚠️ Labels are **collected then collision-rejected**; drawn inline they reproduce his own Obsidian screenshot. ⭐ **COLOR CARRIES THE IDENTITY SYSTEM** (Sam): ★ C-ID · ◆ CCN · ✳ M-ID · ○ unified, **state on a separate rim dot** — folding state into the fill would make a flagged C-ID stop looking official. ⭐ **THE OVERLAY STORES ONE HOP PER ROW AND THE DISPLAY READ HOPS AS GROUPS (#1312)** — an identity merged into and then merged onward is a source AND a target, so **340 folded identities rendered as their own rows** with members stranded mid-chain. **Only 96 are ESL-fold rows; 248 are not even ESL.** `flatten_merge_chains()`: ESL **169 → 77**, all rows **16,824 → 16,484**, **0 rows appeared**. ⚠️ **NOT a violation of "grouping is by KEY, NEVER transitive"** — that governs SIMILARITY edges, which are measurements; a `merge_into` is a DECISION, and sameness composes ([`transitive-closure-is-right-for-decisions`](docs/kb-notes/methodology-transitive-closure-is-right-for-decisions-and-wrong-for-similarity.md)). ⚠️ **A cross-discipline merge is ambiguous between a wrong MERGE and a wrong DISCIPLINE, and the repairs are opposite** — 2,731 of 10,170 judgeable merges cross one, but the top pairs are siblings (CIS↔CS 107, Law↔Legal Assisting 67), a VOCABULARY signal; the one non-sibling pair proved to be right merges under a wrong label. **The view can move a course and cannot relabel an island.** **NEXT:** the Beginning spot-check (**517 rows**, and it needs a worklist built first — merged-away identities do not render, so there is nothing to drag); re-home `FIMS M1018` as the first real drag; a survivor-member audit before the next packaging pass; the one-college-many-numbers rule; a tool for the 3,001. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md). |
| **ESL packaging (the first fold)** | Collapse the ESL discipline to comprehensives + carve-outs — the proof that packaging reaches the target. | ✅ **APPLIED AND PUBLISHED 2026-08-24 — ESL went 2,300 identities → 27** (SkyView, #1311/#1312), cohort `package-esl-s187@bot`, 1,997 rows, receipt `kb/esl_package_out/2026-08-24/`. **Beginning `ESOL M9168` · Intermediate `ESOL M9256` · Advanced `ESOL M1141` · Vocational `ESOL M9023` · Civic `ESOL M9177` · Enrichment `ESOL M1152` · Vocational—Healthcare `ESOL M91IL`.** Far past the 10.7:1 the dry-run projected; against a target of ~17 comprehensives per discipline **ESL is essentially done.** ⭐ **THE MECHANISM IS SURVIVORS, NOT NEW IDS** (Sam): an existing identity is chosen and RENAMED. The code decides this, not taste — `merge_into_orphan` self-trusts exactly one prefix, `UC-CUR-*`, and Session 56 re-minted all 4,053 of those away (**live count: 0**), so a Z-scheme id would flag as an orphan on every audit run. Survivor choice is a STATED RULE: for a LEVEL, dominant SUBJ4 → title opening with the plain level word → most colleges; for a NEW comprehensive there is no level to encode and the title is replaced anyway, so **adoption decides** (ranking by the level word gave a 2-college anchor; by colleges, a 12-college one). ⚠️ **`ON CONFLICT DO NOTHING` BLOCKED THREE OF THE SEVEN RENAMES** — merges landed, NAMES did not. **1,994 landed where 1,997 was planned, and that three-row gap was the whole story**; a write reporting only "no error" would have hidden it. One blocker was **Sam's own row** (`ESOL M9177`), superseded EXPLICITLY per Rule 8, reported the same turn, and **confirmed by him the same day** — the Rule 8 round trip working end to end. ⚠️ **AUTOMOTIVE VESL DOES NOT EXIST** — Sam's own example for the subject split has **zero rows**; **78% of the Vocational lane (90 of 116) is generic workplace English**, residue 26 across eight subjects (largest Healthcare, 6). **Measure the example before building for it.** ⭐ **PACKAGING HIDES PRE-EXISTING OVER-MERGES** — `FIMS M1018` *"Film and American Culture"* (film studies, TOP 0612.00) was bot-merged into `ESOL M1152` *"American Culture and Film"* (credit ESL, TOP 4930.87), the same words reordered. It was 1 of 2 members and conspicuous; it is now **1 of 35** inside Enrichment ESL and looks like nothing. **Audit a survivor's EXISTING members before folding** — afterwards the evidence is diluted by design. ⚠️ **THE OPEN SPOT-CHECK IS 517 ROWS, NOT 794** — of 1,079 landed Beginning folds, 765 are medium confidence, splitting into **517 `default-beginning`** (no level signal at all) + **248 `numeric`** (a number implying a level — weak evidence, but evidence). Work the 517. ⚠️ **It has NO SURFACE**: a merged-away identity does not render, so these cannot be dragged (the `FIMS M1018` lesson again), and the Atlas card shows a 90-row SAMPLE. A worklist is the prerequisite, not the spot-check itself. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md). |
