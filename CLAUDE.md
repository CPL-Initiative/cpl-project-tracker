...
| **CPL clean-up worklist** | What to fix in the CPL data, in what order, and who fixes it. | ✅ **LIVE — `map_cleanup_worklist`**, a TABLE rebuilt inside the nightly promotion (#1256/#1257). Authority: [`docs/map_cleanup_worklist.md`](docs/map_cleanup_worklist.md) — read it before acting. ⭐ **BUILT FOR THE CUSTOMER SUCCESS TEAM** — Natalie Powell (lead), Chelsea Mirada, Ally Barker; carries the **college contact for that class of work**. ⚠️ **THEY NEED THE TEAM PHRASE, NOT REVIEWER ACCESS** — reviewer is all-or-nothing and also reaches `map_student_credit` (student grain), `kb_curation`, `gr_*` and **`team_access` itself**. ⭐ **The gate dropped by MATERIALISING, not by weakening**; **`G9` refuses to finish the promotion if the rebuilt table loses that gate.** ⚠️ **No k-anonymity — internal tool, NEVER a public surface.** ⚠️ **12 colleges have work and NO contact to call** (Citrus 109 · Allan Hancock 99 · Saddleback 63 …). ⭐ **RANKED BY DECISIONS, NOT ROWS** — an `effort_shape` per class, because sorting by row count inverts the order. ⚠️ **A class marked `one rule` must be checked against its own TEXT before one instruction is written for it** ([`methodology-a-one-rule-class-must-be-checked-against-its-own-text`](docs/kb-notes/methodology-a-one-rule-class-must-be-checked-against-its-own-text.md)). **P1 cannot yield credit — 12,283 rows / ~100 colleges / ZERO units**, one rule everywhere; ⚠️ clearing it **raises every disposition rate without awarding a unit**. **P2 Transcribed with no units — 14,348 rows / 4,196 students**, ⚠️ **94% is LA Pierce + Merced** and it is an **ingest gap**, not curation. **P3 Applied with zero applied units — 413 rows.** **P4 approval cascade — 2,225 articulations** (**Initiator 1,026 across only NINE** — a stalled queue, not a statewide condition). ✅ **P5 RULED (Sam, 2026-08-20) — `Credit by Exam opportunities`, and SPLIT the same day on Sam's own challenge: `cx-course-named` **1,310 rows / 26 exhibits / 89 colleges, `one rule`, sendable** vs `cx-no-course-named` **4,001 / 225 / 95, `upstream`, NOT SENDABLE**.** *"Presented to students as Credit by Exam options for them — not ruled out by college staff (unless they don't permit Cx for that particular course)."* ⭐ **Not a disposition problem — an OFFER NOBODY WAS MAKING**, and it costs **nothing to build**: Cx is the mechanism CCCs already use for exactly this and the **largest CPL type in the corpus (798 credentials)**, while **all 5,311 rows carry an EMPTY `course_type`** — untyped, not refused, and an untyped row reads exactly like a closed one. ⭐ **One rule + one exception covers the class**: the college's own Cx policy for that course decides the swimming question nobody needed to pre-answer. ⚠️ **NEVER bulk-rule Not Applicable** — zero units is the MECHANISM of a deferral. ⚠️ **A Cx offer must NAME A COURSE TO CHALLENGE** — three quarters of the class names none (*"Credit may be granted on the basis of an individualized assessment of the student"* and nothing more), and there the offer collapses into *"you may request Cx"*, **which every student can already do**. Sam: *"if there is no course or discipline, it's meaningless and a copout on ACE's part."* ⭐ **The row is not empty — the RECOMMENDATION is**: each still carries its EXHIBIT (the training ACE reviewed), and `fetch_custom_report.py` **already asks for `AceID` + `Title`** and stores neither — a loader change, not an ITPI ask. ⚠️ **Join rate NOT yet measured** (the catalogue is fetched on the runner and never written down) — do not quote coverage until it is. ⚠️ `chatbox_exhibits` rescues 0 of 225, verified against a positive control (785 of 6,330 ids DO join). ⚠️ **Rank 5 UNDERSTATES it** (5 meant readiness, now resolved). **NEXT:** ① P1 as one instruction to ~100 colleges; ② the 1,310 `cx-course-named` to colleges as the Cx offer, then attach the exhibit title to unblock the 4,001, then Sam's CCRR normalisation; ③ P2 to Pierce + Merced; ④ ask the nine Initiator colleges what is stalling; ⑤ decide whether P5's rank moves. |
