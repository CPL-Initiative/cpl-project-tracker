...
| **EACR — Exhibit & CR Adoption** | One place to see every exhibit, its credit recommendations, and the colleges that could adopt it. | ✅ **REWORKED AND LIVE** (Sky162, #1221–#1223; 224/224 green). ⭐ **The College filter matched `adopters ∪ potentials` — 93.6% of hits were NOT adoptions** (Pasadena 1,790 cards / 44 adopted; median card **1 adopter, 41 potentials**), because `potential` = program-of-study under the same TOP ∪ matching C-ID, i.e. **TOP as a primary determination** (Rule 7). ⭐ **THREE SCOPES, NOT A TOGGLE** — `adopted` (default) · `likely` · `any` (*a lead, not a match*). The middle one is the prescriptive M-ID layer (**739 titles / 4,972 pairs**, 25× tighter, **names the local course**) which previously drove no filter at all. Adopted-default verified safe: all 137 statewide cards have adopters. ⚠️ **FILTER, COLUMN AND EXPORTS MUST SHARE ONE SCOPE** — #1221 fixed the first two; the exports still emitted the raw 41-college list, *the layer that reaches a college by email*. ✅ **CER wiring SOUND — 1,745/1,745 classified titles resolve.** Gaps fixed: **8 credentials rendered as TWO cards** (a blank-issuer unclassified twin sorting to the bottom) — **a blank issuer is UNKNOWN, not DIFFERENT**, so it folds while two *named* issuers never do; and **`exhibit_ids` was rendered NOWHERE** (5,135 ids → 2,673 cards). ✅ **Two sub-tabs**, the Student view folded in as a college-scoped mode. ✅ **A11Y + mobile (#1223)** — four defects in day-old work incl. a half-declared ARIA tab pattern and a **colour-only** WCAG 1.4.1 failure; plus **"+N more" had never worked** (`expanded[eid+"_pot"]` written, `expanded[eid]` read). **NEXT (curation, not code):** ① 4 titles existing only as unclassified cards the CER knows; ② 2 statewide cards matching no college at all; ③ sweep other `{0,N}` test bounds (#1224's class); ④ credential view caps at 50 groups. **NEEDS SAM:** confirm **Adopted** is the right default — a visible change; old counts look much smaller. Story: [`docs/eacr_scope_lessons.md`](docs/eacr_scope_lessons.md). |
