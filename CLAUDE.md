...
| **SkyView / the CCR curation interface** | An interactive view of the Common Course Reference — common courses by discipline, their constituent local courses, and moving a course to where it belongs. | ✅ **PROTOTYPE LIVE, THE DRAG WORKS, AND THE ARITHMETIC THAT GOVERNS THE LANE** (SkyView #1309–#1312; SkyCal #1317). ⭐ **GRINDING THE WHOLE QUEUE PERFECTLY LANDS AT 35,937 — 14.4× SHORT OF 2,500.** Merging only compares what exists; **packaging** is the only mechanism with the right shape (ESL proved it at **85:1** — [`measure-your-mechanism-ceiling`](docs/kb-notes/methodology-measure-your-mechanism-ceiling-before-working-the-queue.md)). Target **≈17 per discipline**. ⭐ **THE CORPUS IS ~5,700 DECISIONS, NOT 17,321 ROWS** — 97.1% are ≤12 identities, modal 2. ⚠️ **3,001 have NO discipline** (8,065 identities) — a *different* job (assign one first) and half the queue; kept separate, never blended. ✅ **THE DRAG WORKS END TO END (SkyCal, #1317)** — member re-home (Session 54) writes `CN:<control number>`, honored in `excel_to_dashboard.py`, **never re-mints** (Rule 7 not engaged); still **zero `CN:` rows**, nothing written to Supabase. **101,063 member courses over 16,240 identities** now ship in a SECOND 2.5 MB payload (`ccr_universe_members.json`; record `[control_number, code, college index]`, **no title** — 5.5 MB with, 2.5 without). ⭐ **FOREST IS A MAP, TREES ARE GRAPHS** — a canvas, precomputed stable layout, keyword fly-to, draggable islands. Sam's reason for reversing the per-decision design was right: **it structurally cannot show a cross-area move.** ⭐ **COLOR CARRIES THE IDENTITY SYSTEM** (Sam): ★ C-ID · ◆ CCN · ✳ M-ID · ○ unified, state on a rim dot. ⭐ **MERGE CHAINS RESOLVE TRANSITIVELY** (#1312) — a `merge_into` is a DECISION and sameness composes; *grouping is by KEY, never transitive* governs SIMILARITY edges ([note](docs/kb-notes/methodology-transitive-closure-is-right-for-decisions-and-wrong-for-similarity.md)). ⚠️ **A cross-discipline merge is ambiguous between a wrong MERGE and a wrong DISCIPLINE, and the repairs are opposite.** **The view can move a course and cannot relabel an island.** ✅ **STAND-ALONES + DESCRIPTIONS LANDED (#1319)** — 33,423 single-college rows as **144 per-discipline islands**, drawn **HOLLOW** (a stand-alone asserts no equivalence, so it must not read as a weaker claim); **127,523 course descriptions** in 302 per-subject shards fetched on demand. ⚠️ **THE PAGE MUST BE SERVED, NOT OPENED** (34.8 MB stored, 11.6 MB even at 120 chars, and `file://` blocks `fetch`): `python3 -m http.server 8000`. A shard that cannot load SAYS SO — an empty description is indistinguishable from a course that has none. `prototype/ccr_desc/` is **gitignored**. ⭐ **THE WRITE KEY DOES NOT NAME ONE COURSE** (SkyCruise, #1329) — `CN:<control number>` is all the re-home writes, and **1,814 of 139,834** control numbers name more than one course as the artifacts build them (**462** in the source once the declared institution fold is applied; audit `kb/_audit_control_number_claims.py`, receipt `kb/control_number_audit/`). Every layer below picked the FIRST match — generator via `cn_rows[cn][0]`, page via `byCn[cn]` — and `member_extract` keys on the bare number, so a drag moved whichever course was indexed first and took the other out of its own card. **3,634 draggable rows.** REFUSED now, with the reason, flagged before the click as well as at it; back-ported to the decision view, a second unguarded write path. ⚠️ **Zero `CN:` rows exist, which is the only window you get to find a latent key defect.** ⭐ **SPLIT BY REQUIRED REPAIR, NEVER ONE HEADLINE** — the real worklist is **73** two-real-courses rows across 12 institutions, **93 at San Jose City College**; the rest are 112 cross-institution, 132 the CCN cutover, 145 one course written two ways, and **1,352 one institution entered twice**. ⚠️ **A DECLARED FOLD REACHES ONLY THE ROSTER THAT CONSULTS IT** — `map_college_roster_rules.json` is applied where names enter the EACR payload and never by `mcolleges`, so `CaÃ±ada College` renders that way in the member list today; the raw COCI export carries **only** the broken spelling, 678×, so there is nothing to fold FROM. ⚠️ **TWO GUARDS WHERE ONE IS UNREACHABLE MEANS THE OTHER IS UNTESTED** — the first perturbation left every check green. ⭐ **A VIEW MUST NOT FLY WHERE IT CANNOT DRAW** — search reported *"19 match … Ringed in red"* at zoom 12% while `draw()` renders nodes only above 0.20; one constant read by both now, and removing the floor reproduces Sam's screenshot at exactly `0.120`. An EXACT discipline name wins the search outright (three ESL names contain one another); **one search box**, in the header. **344 course titles queued into a crowded island and 54 fit** — 290 were painted over each other, while island names had had collision rejection since they were written. ✅ **SKYVIEW IS THE LANDING VIEW** (Sam: *"more manageable and less intimidating"*; side-menu naming unchanged) — shipped WITH keyboard operation of the canvas, which **did not exist**: Tab through subjects, Enter into one, Tab through identities, Esc out. ⚠️ **THE WORK SURFACE BEHIND IT IS 1.2% BUILT — 5 of 159 subjects, 593 of 49,907 identities.** ~39 MB inline for all; the per-discipline description shards are the walked path. Decision view also: ranked (⚠️ neither available count can rank alone — the reported one understates, the carried one **saturates at the `--max-members` cap of 14**), card-to-card drag, pan/zoom, group review status. **NEXT:** ① **decision packs per discipline, fetched on demand** — the bottleneck behind every tweak; ② **Sam drives the flipped view** — density and the drop affordance are his calls; ③ the 73 two-real-course numbers; ④ the member-roster fold; ⑤ **Step 2 of the scope: the QUEUE** — a drag leaving the destination's SUBJ4 inconsistent with its corroborated discipline queues a re-mint candidate; **proposes, never auto-adds**. ④ The 67 Z-scheme `ESOL Z####` rows the fold never touched; re-home `FIMS M1018` (still blocked — it does not render, so it needs the un-merge verb); the one-college-many-numbers rule; a tool for the 3,001. ✅ **The Beginning spot-check and the survivor-member audit are DONE** — see the ESL packaging row. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md). |
...
| **Memory tab / Autogenerate + the Briefing** | Drafting a memory row from a typed topic, and reading the entries back so a misunderstanding is visible before it costs anything. | ✅ **BOTH SHIPPED AND DEPLOYED** (SkyRead #1320/#1321; deployed Sky190, 2026-08-25). ⭐ **AUTOGENERATE DREW THE WRONG SUBJECT BECAUSE THE TOPIC NEVER ARRIVED** — `cpl-chat` capped `query` at 1,000 chars server-side and the instruction envelope measured **984** of them, so Sam's 870-char note reached the model as *"When responding "*. Handed no subject and ~9 KB of answer doctrine it wrote about the doctrine, in `STATEWIDE_RULE`'s own words. ⚠️ **A cap that is generous for one caller is a silent content swap for another**, and the surviving prefix is still grammatical — no ragged edge to notice. Fixed with a **per-surface cap** (`queryCapFor`, 1,000 chat / 6,000 drafting / 20,000 briefing; an unknown surface → `null` → chat cap, so it is not a lever from outside), an optional **`retrieval_query`** (retrieval keyed on the envelope returned **99 keywords, one of them the curator's**, at a healthy-looking 0.86), and a **`DRAFTING_BLOCK`** appended to `volatile` that REPLACES the conversational doctrine — never `stable`, which is the prompt-cache breakpoint. ⚠️ **The topic LEADS the prompt** so anything a cap eats is instruction text and fails loudly. ⭐ **THE BRIEFING IS AN AGENT READ-BACK, NOT A SIERRA ANSWER** — `cpl-chat` references `cpl_memory` **nowhere**; she has never read this table, so a briefing dressed as her answer would prove nothing. ⭐ **Every claim cites its entry** as a superscript number with a hover/focus card + a numbered source list; a citation to a slug that is not an entry renders **FLAGGED**, counted in the status line. ⚠️ **Inline slugs were unreadable** — this table's slugs are whole sentences. ⚠️ **It briefs `reportFiltered()`, passed in not re-derived**, and states what it read (a capped read must not read as a census). **Three defects the verification caught, each the same lesson one level down:** the envelope was outside the corpus budget (overshot 1,392 chars); the surface vocabulary lives in **THREE** places and two were updated (CI found the curator picker — *naming the lists is not checking them*); and the tap-target exemption was **unearned**, because the source list justifying it measured 15px. ⚠️ **Three budget assertions passed VACUOUSLY** against a client declaring no budget (`indexOf("")` → 0). **Audit: Autogenerate's blast radius is NIL** (3 calls ever, no row carries the doctrine text) and the corpus is clean (**0 near-duplicate pairs** over 536 active rows) — but **26 of 177 `verified` rows have no `verified_by`**, 358 proposed vs 177 verified, and 3 sessions wrote under two author strings each. ⚠️ **HALF A TWO-HALF FEATURE DEPLOYS ITSELF** — the client is a repo-root static file (Pages, on merge), the Edge Function needs a dispatch, and `cpl-chat-smoke` tests the DEPLOYED function so nothing notices the gap. For a day the Briefing ran against a server that did not know its surface. ⚠️ **Two sessions described that wrongly in opposite directions** because neither could call the endpoint — measured, this envelope puts the **corpus FIRST**, so a truncation eats the *instructions*, not the corpus (the "sixteen characters" figure is Autogenerate's envelope-first shape). `cpl-chat-preview-ab.yml` exists to replace that guessing. ✅ `sierra_guidance` CHECK widened for `memory-briefing`; ✅ `plain` backfilled (live rows at zero). Story: [`docs/cobi_memory_tab_lessons.md`](docs/cobi_memory_tab_lessons.md). **NEXT: Sam reads a real briefing in a browser** — no session can. **Free rider:** 8 British spellings in Sierra's own prompt text. Story: [`docs/cobi_memory_tab_lessons.md`](docs/cobi_memory_tab_lessons.md); durable [`a-silent-input-cap-is-a-content-swap`](docs/kb-notes/methodology-a-silent-input-cap-is-a-content-swap.md). |
