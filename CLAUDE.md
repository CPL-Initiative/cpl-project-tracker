...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **LIVE, TWO-SIDED, DOWN TO TWO DIALS.** ⚠️ **The baked defaults in `cpl_funding_data.js` are STALE by design** — the live model is the Supabase overlay (`config.projects."cpl-implementation".scenarios."Scenario 1"`, camelCase; the reorder is a PERMUTATION stored BESIDE it — a partial rewrite re-points a priority at another default). ✅ **SAM'S SAVED CONFIG: `priorityOrder [2,0,1]` — Access 34%, Outreach 33%, Success 33%; factors 1.0; front-load ON; Year-2 mirror ON; deadline 2026-11-01; all three metrics in CPL FTES.** 🆕 **BOUNDS — minimum `pool.floor_window` $175,000 · maximum `pool.cap_window` $400,000**, `0` disables the ceiling (SkyBound, #1293/#1297). 🆕 **THE RURAL CARVE-OUT IS RETIRED** (#1297) — its $1,000,000 folded into the main proportional pool (**$24,240,308 across 115 colleges**); the `rural` flag survives as federal context ONLY — no glyph, section, card, CSV column or control. ⭐ **THE FLOOR DOES NEARLY ALL THE WORK**: **69 of 115** sit at the minimum, **the median college IS the minimum**, and the ceiling binds **2 / $82,815 / 0.34%**. ⭐ **A CEILING CANNOT LIFT A FLOORED COLLEGE**, so it was never the equity lever; **the floor is**, up to ~$210,785 (the average award). ⚠️ **A FLOOR IS NOT FREE — it is a transfer priced in the earn rate of the middle**: unbound colleges now earn **78.2% of base ($4,419 per CPL FTES vs $5,649.63)**. ⭐ **THE TWO BOUNDS ARE SOLVED TOGETHER** (bisect `lambda` in `clamp(lambda*size, floor, cap)`) because a floor is monotone and a ceiling is not: pinning at the ceiling RELEASES money and lifts colleges back off the floor, which a pin-as-you-go loop strands **with the pool still balancing**. Ceiling OFF reproduces the old loop bit-for-bit. ⚠️ **A BOUND ON THE MONEY IS A BOUND ON THE BAR** — a capped college's targets scale to **ceiling ÷ `plainRatio`**, never the bare ceiling (every unbound college already discounts to fund the floor), so **cap ÷ target is ONE rate above the minimum**; the clamp must reach **both** target paths (a student-unit priority never touches `prioEntitlement`). ⚠️ **A REDUNDANT-LOOKING MECHANISM MAY CARRY A SECOND JOB** — the rural allowance was the pool's ONLY unconditional money (the floor caps what a college can EARN, never what it receives), so the 13 went **$76,923 guaranteed → $0**. ⭐ **EXPLAINER, audience-facing** (CO, possibly CA Finance): [How this funding model works](prototype/funding_model_explainer.html). ⚠️ **It is a SNAPSHOT — re-run `prototype/build_funding_model_explainer.js` and re-publish to the SAME artifact URL whenever the shares, factors, order or bounds change**; every figure, worked-example cards included, comes from the tab's own engine; the example is the largest **UNBOUND** college. ⭐ **Sam's framing rules:** never *"a ceiling, not a check"* — say **what a college receives is driven by its own CPL results, as they happen**; and **never frame the CO's share as withholding** — name what each amount BUYS. ⚠️ **NC MONEY STAYS VISIBLY SEPARATE ON THE COLLEGE ROW** (Sam: *"the neglected step child"*) — an NC expansion is SCOPED, NOT BUILT: every NC institution on a $25K floor / $100K ceiling, same three priorities, counted only from NC landing pages. ⚠️ **Mt. SAC's NC FTES is DOUBLE-COUNTED** (feeder + credit row both carry 10,829.3) — harmless until NC money follows it. **NEXT: ① Sam picks the pair — $175K + $400K as shipped, or hold the floor at $150K and send the freed $1M to noncredit (same change, two dials: `floor_window` / `feeder_carveout`); ② Sam reads the explainer in a browser** (no session can — egress-blocked); ③ Year-2 mirror for Scenario 2. Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) ([archive](docs/cpl_funding_lessons_archive.md)); durable [`a-second-bound-breaks-a-pin-as-you-go-solver`](docs/kb-notes/methodology-a-second-bound-breaks-a-pin-as-you-go-solver.md) · [`a-mechanism-that-looks-redundant…`](docs/kb-notes/methodology-a-mechanism-that-looks-redundant-may-be-carrying-a-second-job.md). |
...
### SkyBound — a ceiling, then one dial fewer (2026-08-22, Session 184)
