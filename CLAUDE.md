...
| **Exhibit-canon PR-B** | **Credential Reference** tab — top-level dashboard tab modeled on the CSC tab. Lists every unified_title with raw-variant count, primary issuer, confidence band, quality_flag; filters by band / issuer / flagged-only. Read-only MVP, then two-stage curation (initiated → validated) writing to `kb_curation` via synthesized key namespace (`_UNIFIED_TITLE::<raw_title>` / `_CREDENTIAL::<unified_title>::<issuing_agency>` — no DDL). | queued — Session 6 |
