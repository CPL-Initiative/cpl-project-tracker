...
| **My College (college action page) / MAP-team queue** | One page (not 123) where a college picks itself and gets its stats, its opportunities against the goals, and concrete to-dos — plus the same engine pointed INWARD at the MAP team's own backlog. | ✅ **LIVE. The tab opens on a CHOICE, and always asks** (Sky167 #1232–#1234; SkyAsk #1274). Flow: scope question → curated list → "Welcome, X" → collapsible sections. ⭐ **A remembered choice is a NAMED SHORTCUT, never a destination** (Sam, 2026-08-21) — restoring it into state opened the tab populated for whoever was here last; a scope with no entity is not offered at all. Sierra is a **collapsible section, expanded by default**, closed by Collapse all like every other (Sam's ruling); her `<summary>` carries the SINGLE heading, and the header carries **no prose** — the struck sentence was #1231's duplicate description of Sierra growing back a THIRD time. ⭐ **ONE QUESTION CLUSTER, BELOW THE ROLE CHIPS** — two clusters straddling the role picker made `needAudience()`'s *"tap who you are above"* point the wrong way, and the generic starters name **Riverside City College** on every college's page. The tab hands its questions to the widget via `CPL_CHAT.setSuggestions()`; a group scope clears them; the fallback is gated on the questions **LANDING**, not on the mount. ⚠️ **THE PANE'S INLINE `text-align:center` OUTRANKS THE CSS THIS MODULE INJECTS** — every lazy COBI tab ships that loading placeholder, so capped prose renders CENTRED inside a left-anchored box and reads as a width bug. `shedPlaceholder()` clears it at render. **Standing invariants — do not break:** `buildQueue(sources, now)` is **pure, do not fork the ranking rules**; **measure at load, never carry a list** (a failed read is `unknown`, never 0); ⚠️ **NEVER re-derive an allocation — call `_alloc()`** (floor waterfall; Mt. SAC = **$522,239**) and **`_prios(name, slot)`** with an explicit slot or a Year-2 view renders $0; ⭐ **join BOTH sides through `cplCollegeShort()`**; ⭐ **a ROLL-UP sums UNSUPPRESSED rows only** or `total − visible` hands back the k=10 figure (withheld are COUNTED; ABSENT stays distinct from withheld); ⭐ **the briefing is a docx that READS THE RENDERED DOM**, so it cannot drift from the screen and inherits suppression by construction; ⚠️ **`askSierra()` must OPEN the section first** (#1166, re-armed); ⚠️ **`prefill()` must stay send-free**; ⚠️ an **ABSENT measurement must never render as an ACHIEVEMENT**; ⚠️ `safePct()` — never round UP into a claim; ⚠️ `map_college_cr_unit` has **no k-anonymity of its own**; ⚠️ `map_credential_student_rollup` is a **MATVIEW** (no RLS, `anon` holds the grant). The lead figure is **ONE decision, not 300** — 98.8% of the 64,074 waiting units is Credit for Basic Military Service (backlog **592 rows**, **33 of 106 have NONE**). ⚠️ **TWO OF FIVE SCOPES SHIP DISABLED WITH THEIR REASON** — SWP region and ASCCC region exist nowhere here, and **`college_geo.region` is a THIRD scheme** (~10-way proximity map; SWP has **8** consortia, ASCCC **4** areas). A test pins that it stays unwired. **HELD BY SAM:** MAP deep links, the `?college=` RLS decision, the MIS side-by-side (parked — our extract is stale pending the MAP reload; our aggregation is faithful). **NEXT:** Sam looks at the redesign in a browser; whether the public Sierra page + Fact Sheet drawer take the new intro wording; the region lists when he finds them. Story: `docs/college_action_page_lessons.md`. Durable: [`an-rls-filtered-read-is-not-an-error`](docs/kb-notes/methodology-an-rls-filtered-read-is-not-an-error.md) · [`a-report-must-read-the-screen-not-recompute-it`](docs/kb-notes/methodology-a-report-must-read-the-screen-not-recompute-it.md) · [`an-inline-placeholder-style-outranks-the-css-you-inject`](docs/kb-notes/methodology-an-inline-placeholder-style-outranks-the-css-you-inject.md). |
