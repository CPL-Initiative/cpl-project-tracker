...
| `tmc_templates.js` | The **45-TMC catalog** (`window.CPL_TMC_TEMPLATES`) — 8 **DRAFT** definitions (the fixed left-side C-ID course lists: Required Core / List A/B/C) + 37 `planned` stubs (the rest of the approved set), each with a per-TMC status (`draft`/`planned`/`official`) and an official-template URL in `_meta.sources`. Slot C-IDs are real (from `cid_descriptors.json`); section/unit/select details need faculty verification. Encode a planned TMC by adding its `sections[]`. Static. |
...
- **Per-TMC status indicator** (Session 59 follow-up): the picker is a **45-TMC
  catalog** (the full approved set, from Sam's TMC-download .docx), grouped into
  **Available now** (8 encoded, `draft`) and **Coming soon** (37 `planned` stubs).
  Each row shows a status chip — `✓ Official` / `⚠ Draft` / `◷ Coming soon` — and
  links to its **official template** (`_meta.sources[id]`, the c-idsystem.org PDF;
  the site Cloudflare-blocks automated fetch, so the 37 are encoded by hand as
  their PDFs are obtained). `tmcStatus()` infers `draft` when `sections` exist,
  `planned` otherwise; flip to `official` when faculty-verified.
...
- All-**STATIC**, all-**lazy**, NOT regenerated by `excel_to_dashboard.py` and NOT
  ...
  `tmc_templates.js` (45-TMC catalog: 8 **DRAFT** defs + 37 `planned` stubs +
  `_meta.sources`; faculty must verify, encode more by appending) → then
