...
| **CCR Atlas / the curation interface** | An interactive view of the Common Course Reference — common courses by discipline, their constituent local courses, and moving a course to where it belongs. | ✅ **PROTOTYPE LIVE + THE ARITHMETIC THAT GOVERNS THE LANE** (SkyView, #1309/#1310/#1311). ⭐ **GRINDING THE WHOLE QUEUE PERFECTLY LANDS AT 35,937 — 14.4× SHORT OF 2,500.** Merging only compares what exists; **packaging** is the only mechanism with the right shape, and the ESL dry-run already proved 10.7:1 inside one discipline. Design target: **2,500 ÷ 144 disciplines ≈ 17 common courses each** ([`measure-your-mechanism-ceiling`](docs/kb-notes/methodology-measure-your-mechanism-ceiling-before-working-the-queue.md)). ⭐ **THE CORPUS IS 6,056 DECISIONS, NOT 17,321 ROWS** — collapse the six lanes into connected components: **97.1% are ≤12 identities, modal size 2**, 144 disciplines carry work at **median 12 decisions**, **118 of 144 have ≤40**. Automotive Technology is 57; Welding 35. ⚠️ **3,001 of the 6,056 have NO discipline** (8,065 identities) — a *different* job (assign a discipline first) and half the queue; kept separate, never blended. ⭐ **THE DRAG VERB ALREADY EXISTED AND HAD NEVER BEEN USED** — member re-home (Session 54) writes `CN:<control number>`, is honored in `excel_to_dashboard.py`, tested, reversible, and **never re-mints an M-ID** (Rule 7 not engaged); **zero `CN:` rows** against 28,797 identity-level rows. You cannot drag a course into a cluster you cannot see. ⭐ **FOREST IS A MAP, TREES ARE GRAPHS** — a global force graph is a hairball (Sam's own Obsidian shot is illegible at ~500 nodes); 158 discipline cells → one discipline's decisions → one 2–12 node graph. ⚠️ **The graph must be on the FIRST screen** — it shipped two clicks down with the grid sorted by decisions, so the top cells had no data and the obvious click was a dead end. ⚠️ **A happy-path selector cannot see a dead end** (the check clicked `.cell.demo`). ⭐ **COLOR CARRIES THE IDENTITY SYSTEM** (Sam): ★ C-ID · ◆ CCN · ✳ M-ID · ○ unified, with **state on a separate rim dot** — folding state into the fill would make a flagged C-ID stop looking official. **NEXT:** the whole-universe view (precomputed layout + level-of-detail — a **map**, not a live force graph) and the one-college-many-numbers audit rule. Story: [`docs/ccr_atlas_lessons.md`](docs/ccr_atlas_lessons.md). |
| **ESL packaging (the first fold)** | Collapse the ESL discipline to three comprehensives + carve-outs — the proof that packaging reaches the target. | 🔨 **PROPOSAL RENDERED, BLOCKED ON SAM** (SkyView, #1311). **1,846 identities / 3,128 local courses** would fold today — **not the 2,149 the 2026-07-15 plan claims** (297 already curated, 39 of them Sam's own and protected by `ON CONFLICT`; 6 no longer rows). ⚠️ **THE DRY-RUN CANNOT BE RE-RUN FRESHER** — its inputs are the AI-drafted **baseline**, deliberately static at `_generated_at 2026-05-22`, because curation is an **overlay** applied at export time; re-running reproduces itself byte for byte. The gap is closed reproducibly by `kb/_esl_package_actionable.py`. ⚠️ **EVERY ROW'S `target` IS `null`** — the three comprehensive rows **do not exist**; minting them is NOT a Rule 7 re-mint but it is a naming decision that blocks the apply entirely. ⚠️ **53% of the write is medium confidence, and it is concentrated: 794 of the 1,110 Beginning rows (72%) got there by DEFAULT** (no level word in the title), so Beginning is the biggest bucket *and* the least certain. ⚠️ **THE BUCKET THE PLAN CALLED RISKIEST HAD ALREADY BEEN CONSUMED** — transfer-level ESL was flagged for individual confirmation because it awards *real transferable credit*; of 22, **only 8 still stand** (8 curated away, 6 vanished with no curation row). The review gate closed before anyone opened it. ⚠️ **A CLEAN BILL OF HEALTH FROM A QUESTION THE DATA CANNOT ANSWER** — the preview's first cut read fold-scoped skip lists for carve-outs and reported *"22 of 22 standing"*; a check now asserts that card reports what it LOST. **NEEDS SAM: ① name the three comprehensives ② work the 794-row Beginning spot-check.** Then apply under Rule 10. Receipts: `kb/esl_package_out/2026-08-24/`. |
...
### SkyView S187 — the queue cannot reach the goal, and the drag verb was already there (2026-08-24)

**Sam: *"a left turn over to the CCR … an interactive interface something like my graph
view in obsidian"*, then the frame: *"cluster 142k local courses down to 2k-2.5k."***
⭐ **The governing finding is arithmetic, not design: grinding all 6,056 decisions
perfectly lands at 35,937 — 14.4× short.** Merging compares what exists; only
**packaging** reaches the target. ⭐ **The corpus is 6,056 decisions, not 17,321 rows** —
97.1% ≤12 identities, modal 2, median 12 per discipline. ⭐ **Member re-home (`CN:`) was
built in Session 54, tested, reversible, never re-mints — and had ZERO uses**: the picture
was the missing half of a shipped feature. ⚠️ **The graph shipped two clicks down** and the
first cell was a dead end — *"I don't see the graph view"*; my check missed it by selecting
`.cell.demo`. ⚠️ **ESL: 1,846 folds today, not 2,149**, the dry-run **cannot be re-run
fresher** (static baseline + overlay), every `target` is `null`, and the transfer-level
carve-out the plan called riskiest **was already 64% consumed**. ⭐ **A confirmed over-merge
generalizes**: members all one college, different course numbers → **3,320 identities**.
⚠️ **`college_identity_lint_guard` went red at 00:00 UTC for everyone** (byte-for-byte vs a
date-stamped artifact); `main` did go red, now fixed and proven to still fail on real drift.
