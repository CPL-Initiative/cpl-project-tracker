...
- **AMERICAN SPELLING, ALWAYS (Sam, 2026-08-21).** *"As a Yank, I prefer
  ...
  and v) · gray · enroll · while (not whilst) · among (not amongst)** and the
  `-ize`/`-ization` family throughout. Applies to **rendered UI text first**
  (`college_briefing.js` was telling readers a "programme" was inactive and
  citing "the curated catalogue"), then docs, then comments. Enforced by
...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **REORDERABLE, AND THE REORDER REWRITES NOTHING** (SkySort, #1268). Priorities drag into position — or move via a keyboard-reachable **Position** picker — carrying title, description, metric, share, funding factor, per-student rate and strategies with them. ⭐ **The order is a PERMUTATION stored BESIDE the config, never a rewrite of it**: permuting the stored priorities has to enumerate every field, and a field it forgets silently re-points a priority at another identity's baked default — the **live overrides are PARTIAL** (Scenario 2 sets `metric`/`share` on two and neither `factor` nor `title`) and `yearPriorities[slot]` is an **object keyed by index string**, not an array. Every allocation, cap and target is byte-identical after a reorder. ⭐ **ONE translation seam** — `prioField`/`prioMetricSource`/`prioUnit`/`setPrio` + `priorities()`; above it DISPLAY index, below it SOURCE index, because the failure that matters is an edit landing silently on the wrong priority. `label` is positional; `key`/`src` are the identity. ⚠️ **The order is WINDOW-LEVEL** (Sam, 2026-08-09: the years are deliberately identical) — per-year would make P1/P2/P3 mean different things in different years. ⚠️ **My College joined the money to the advice BY POSITION and its COUNT gate could not see a reorder** (three still equals three); now an identity join, and `buildBriefing()` had been **dropping the key** in its remap. ✅ **"Price factor" → "Funding factor"** — label only; the stored `factor` and the `priofactor` edit key are unchanged, so nothing saved is stranded. ⭐ **Year-2 sync is a MIRROR, not a copy on the front-load toggle** — a copy overwrites Year 2 with no undo as a side effect of a *cash-timing* control, is a no-op for Scenario 1 and a silent policy edit for Scenario 2, and front-load is where it matters LEAST (Year 2 is already carryover, its metrics never scored). Mirror resolves, writes nothing, reverses cleanly; an explicit **Copy Year 1 → Year 2** asks first. **Default OFF.** ⚠️ **The baked defaults in `cpl_funding_data.js` are STALE by design** — the live model is the Supabase overlay (`config.projects."cpl-implementation".scenarios."Scenario 1"`, camelCase). ✅ **SAM HAS USED THE REORDER (read from the saved config, 2026-08-21): `priorityOrder [2,0,1]` — Access dragged to first at 34%, then Outreach 33%, then Success 33%; all three funding factors 1.0; front-load ON; the Year-2 mirror ON; deadline moved to 2026-11-01; all three metrics now in CPL FTES.** ⭐ **The explainer behind the tab's link is now audience-facing** — "Calculation sanity check" became [**How this funding model works**](prototype/funding_model_explainer.html) (Sam, ahead of showing the model to CO colleagues): same arithmetic, told as a guide in plain language. ⚠️ **It is a SNAPSHOT — re-run `prototype/build_funding_model_explainer.js` and re-publish to the SAME artifact URL whenever the shares, factors or order change**; every figure in it is generated by this tab's own engine, never retyped. ⭐ **It states one thing the old page did not**: the $150k minimum is funded from inside the pot, so a college above it is effectively funded at **~$5,060/FTES rather than $5,649.63**, while a floored small college reaches its target on far less (Lassen ~$23,900/FTES). **NEXT: Sam reads the explainer end to end before sending it out**; then whether the mirror should be on for Scenario 2 as well. Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) (2026-06→07 in [`..._archive.md`](docs/cpl_funding_lessons_archive.md)). |
| **My College (college action page) / MAP-team queue** | One page (not 123) where a college picks itself and gets its stats, its opportunities against the goals, and concrete to-dos — plus the same engine pointed INWARD at the MAP team's own backlog. | ✅ **LIVE. The tab opens on a CHOICE, and always asks** (Sky167 #1232–#1234; SkyAsk #1274; SkyVouch #1276). ⭐ **A remembered choice is a NAMED SHORTCUT, never a destination** — true of the COLLEGE (restoring it opened the tab for whoever was here last) and, one level down, of the **ROLE**: `audience` persists under a localStorage key SHARED with the public page and the Fact Sheet drawer, so a pick made once anywhere steered every answer for ever. It is REMEMBERED but only CONFIRMED per browser-tab session. ⚠️ The held question is RESUMED not dropped, pinned to its PANE, and `loadAudience()` ASSIGNS the flag rather than only raising it. ⭐ **THE HOST OWNS THE QUESTIONS IN EVERY SCOPE** — a group scope must not pass `null`, which returns the widget to its PUBLIC starters (one names **Riverside City College**, nonsense under a district heading). ⚠️ **Sierra has NO district dimension** (0 columns named district; `districtIndex()` is client-side from the funding roster), so a district is named only in an **advisory** question and **no member college is singled out**. ⚠️ **THE PANE'S INLINE `text-align:center` OUTRANKS THE CSS THIS MODULE INJECTS** — every lazy COBI tab ships that placeholder; `shedPlaceholder()` clears it. **Standing invariants:** `buildQueue()` is **pure**; **measure at load, never carry a list** (a failed read is `unknown`, never 0); ⚠️ **NEVER re-derive an allocation — call `_alloc()`** (Mt. SAC = **$522,239**) and **`_prios(name, slot)`**; ⭐ **join BOTH sides through `cplCollegeShort()`**; ⭐ **a ROLL-UP sums UNSUPPRESSED rows only**; ⭐ the briefing is a **docx that READS THE RENDERED DOM**; ⚠️ `askSierra()` must OPEN the section first; ⚠️ `prefill()` stays send-free; ⚠️ an ABSENT measurement must never render as an ACHIEVEMENT; ⚠️ `map_credential_student_rollup` is a **MATVIEW**. The lead figure is **ONE decision, not 300** — 98.8% of the 64,074 waiting units is Credit for Basic Military Service. ⚠️ **TWO OF FIVE SCOPES SHIP DISABLED WITH THEIR REASON** — SWP and ASCCC regions exist nowhere here, and `college_geo.region` is a **THIRD** scheme. **HELD BY SAM:** MAP deep links, the `?college=` RLS decision, the MIS side-by-side. **NEXT:** Sam looks at the redesign in a browser; the region lists when he finds them. Story: `docs/college_action_page_lessons.md`. |
