...
| **Disposition grain / student detail** | What a college has ACTED on, not just what credit exists — the gap behind a hedged Sierra answer. | ✅ **BOTH TABLES LIVE + SIERRA WIRED (cpl-chat v37).** `map_student_credit` 220,588 rows (student grain, reviewer-only RLS, **no write policies**) · `map_college_cr_unit` 204,714 (credit funnel, aggregate) · published aggregates `map_college_goal2` + `map_college_credit_summary` (suppression at write time) · lookup `map_colleges` (128). 🎓 **Course Credit tab LIVE.** ⭐ **THE HEADLINE: 1,051,870 units at Needs Action across 106 colleges, and 63,991 of those are ALREADY ARTICULATED** — everything built, nobody acted. Lead with the second figure; the million is a ceiling (~30% of reviewed credit is correctly Not Applicable). **Number policy (Sam):** show published AND unsuppressed with a suppression chip — published 1,051,870/63,991, unsuppressed 1,052,531/64,074, **both scoped `entity_kind='college'` (106 entities)**; without that scope the figure looks unreproducible (all-entity sum is 1,053,332.50). **Never change one half of the pair alone.** Chip states why: 13 of 111 colleges withheld, each under 10 CPL students. ⚠️ **SAFETY: show both ONLY while ≥3 cells are suppressed** — at one suppressed cell the difference IS that college's figure (`adr-student-detail-aggregate-disclosure-control`). ⚠️ **FOUR THINGS TO KNOW BEFORE WIRING ANY STUDENT NUMBER (SkyLine, 2026-08-10):** ① **Sam's applied/transcribed measures are not computable yet** — `map_student_credit` is 5 columns and the credit amounts are not in that export (they were never there; an earlier "dropped at load" claim was wrong). Fix = re-load from **`TblSOURCE`**, the raw MAP extract, 537,908 rows, which carries the credits **and `CPLStatusPlan` at student grain**. Runbook: [`docs/map_student_credit_reload.md`](docs/map_student_credit_reload.md). Only hard blocker on route CRED·VOLUME. ② **The person key is the `tblStudentKey` surrogate for `StudentMAPID`** — **NOT `TblSOURCE.Student`, which is a grouping counter** (Sam, twice). The MAP id must never reach Supabase; the runbook now enforces this with a range tripwire, not a warning. ③ **Only 6.1% of student rows are nameable** (13,488 of 220,588) — grain is ACE ids + 32,360 `Default *` sentinels vs Sierra's `MAPICI-*`; overlap 624 of 6,280. Control: CPR/AED is 17,904 students locally, **17** through the obvious join. Per-credential counts are a FLOOR over the nameable subset. ④ **537k is fine to STORE, too slow to aggregate LIVE** — measured 2,432 ms at 220,588 rows, so ~6s at 537,908 against Sierra's 1.7–5.0s budget. Sierra reads pre-computed rollups with k=10 applied at write time; never the grain. ⚠️ Also: `map_colleges` carries 8 `entity_kind='test'` rows, and ids **122 + 131 are absent from it** but inside the credit data (432.50 units) — resolve from MAP's own list, not inference. Story: `docs/student_detail_load_lessons.md`. |
...
| **College action page / MAP-team queue** | One page (not 123) where a college picks itself + a role and gets its stats, its opportunities against the goals, and concrete to-dos — plus the same engine pointed INWARD at the MAP team's own backlog. | 🔨 **INBOX LIVE (#1073) · BRIEFING SHIPPED (#1086) · REWORK PROPOSED.** `buildQueue(sources, now)` is **pure — reuse it, do not fork the ranking rules**. Tab rule: **measure at load, never carry a list**; a failed read is `unknown` at the TOP, never 0. ⭐ **The action library already existed** — Supabase `cpl_funding_config` → Scenario 1 → Year 1 holds **22 team-written strategies**. Sam: **Y1 ≡ Y2 byte-identical, "Year 1 is the authoritative set"** — build no year-specific logic. Live shares are **50/30/20**, NOT the baked file's 30/42/28; resolve the FULL chain incl. the overlay at `config.projects."cpl-implementation".scenarios."Scenario 1".yearPriorities."1"` (camelCase, three levels deeper than the old documented path). ⚠️ **THE BRIEFING HAS THE STEPS-VS-RATIONALE DEFECT** (Sam, 2026-08-10 — *"we tend to get buried in rationale rather than just telling them the simple steps"*): it opens correctly with **"Start here"**, then prints all 22 strategies at equal weight when only **3** carry a measurement, shows **"50% of the pot"** (state allocation logic a coordinator cannot act on), and closes by explaining where the config lives — a sentence for us. **A step has a done state; advice does not.** Rework: split steps from advice, drop the pot share, demote provenance, add the low-stakes line. See `methodology-lead-with-the-steps-not-the-rationale`. **Next:** that rework. The inbound-request slot still needs the nightly feed; everything else is buildable today. Story: `docs/college_action_page_lessons.md`. |
