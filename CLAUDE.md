...
| **EACR — Exhibit & CR Adoption** | One place to see every exhibit, its credit recommendations, and the colleges that could adopt it. | ✅ **FILTER REWORK LIVE** (Sky162, #1221–#1223) — **three college scopes**, `adopted` (default) · `likely` (the prescriptive M-ID layer, which **names the local course**) · `any` (*a lead, not a match* — TOP-derived, so Rule 7 forbids it as a primary determination). **Sam has used it and confirmed the arrangement** (2026-08-17), closing that carried question. ⚠️ **FILTER, COLUMN AND EXPORTS MUST SHARE ONE SCOPE** — the exports are the layer that reaches a college by email. 🔨 **MATRIX SUB-TAB — GENERATOR SHIPPED (#1226), VIEW NOT BUILT** (Sky163). CER titles down the side, colleges across the top, CR units in the cells: **green = adopted, brown = still available**. ⭐ **SAM'S FOUR RULINGS, LOCKED:** brown is the **peer benchmark** · open on **colleges** (not regions) · default rows **≥2 adopters** · brown on **credible cells only** (the *likely* tier, not every non-adopter). Default view = **434 rows × 118 cols, 17.0% inked** (12.3% green / 4.7% brown; 59% of rows carry an opportunity). ⚠️ **BROWN CANNOT BE THE LINE TOTAL** — 83% of adoptions are PARTIAL (median **3.07 of 9.26** lines) and **no college has ever reached the total**; it would promise ~3× the strongest peer, in a column that leaves as a CSV. ⚠️ **118 NUMERIC COLUMNS CANNOT FIT** (~3,500px, ~2× a desktop) — frozen title column + rotated short-caps headers + h-scroll. ⚠️ **`chatbox_peer_articulations` IS THE WRONG UNITS SOURCE** (32.5% coverage — a join of two half-sources); the raw `View_ArticulatedMAPExhibits` row carries college+course+rec TOGETHER, so `adopter_units` is a straight read at 100%. NOT `map_college_cr_unit` (reviewer-gated, no k-anonymity, measures student disposition). ✅ **Roster rules applied at BUILD time** (`kb/reference/map_college_roster_rules.json`): sandbox dropped on `entity_kind='test'`, the three `" Credit"` twins folded as a **SUM**. ⚠️ **The sandbox was inflating a live public number** — **7 adopters published on California Real Estate Broker License where the truth is 6**. Axis = **118 = 115 credit** (Sam's number exactly) **+ 3 noncredit**; the 4th, **Mt. SAC Noncredit, has no identity in `map_colleges` or the export** (Learning Partners item 1). **NEXT: build `buildMatrixView()`.** **Curation carryover:** 4 unclassified-only titles the CER knows · 2 statewide cards matching no college · sweep `{0,N}` test bounds · the 50-group credential-view cap. Story: [`docs/eacr_scope_lessons.md`](docs/eacr_scope_lessons.md).
