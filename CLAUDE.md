...
- **Per-college approved-ADT overlay (Session 61).** The **COCI program export**
  ...
  ✓ Approved / ⏳ In progress / ◐ Teachout when one is picked; the **statewide
  approved-college count** in review mode) + a prominent **status banner** on the
  TMC detail (`adtBannerEl`), plus a **"this college's approved ADTs / not yet
  established"** Show-filter. "Approved" = COCI STATUS ∈ {Active, Approved};
