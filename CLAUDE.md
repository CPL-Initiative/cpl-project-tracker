...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **LIVE, TWO LANES ON ONE CARD SET, ONE SOLVER.** ⭐ **NEVER READ THE CONFIG — CALL `_effective()` / `_alloc()` / `_nc()` / `_prios()`**; dump the live dials with `scripts/funding_effective.js --config <file>`. Baked defaults are **STALE BY DESIGN**, `mirrorYears` makes the stored Year-2 block **INERT**. ⛔ **A CLIENT GATE STRICTER THAN ITS RLS POLICY FAILS SILENTLY, TOWARD LOST WORK (SkyLens, 2026-08-28)** — `unlocked()` asked only for the team PHRASE while all three funding tables carry `is_allowed_reviewer() OR team_pass_ok()`, so Sam's relabel, made while the masthead read "● Signed in", went to the per-browser scenario layer, **which wins the render**, and looked published. **Two credentials, one word.** Seven write paths had it; fixed as ONE `applyWriteAuth()`. ⚠️ **The routing was never at fault** — every consumer reads `_prios()`/`_ncPrios()`, nothing hardcodes a priority name. [`a-client-gate-must-mirror-its-own-rls-policy`](docs/kb-notes/methodology-a-client-gate-must-mirror-its-own-rls-policy.md). ⭐ **ONE LANE SWITCH above the cards** (Sam: all three together, every year — *"otherwise I'll be a confuseled Pooh"*); a guard forbids a per-card control. NC cards carry their own figures, metric, `$0` earning and **their own strategies, editable into Supabase** (`ncPriorities`, keyed on the SOURCE index). ⚠️ **Every other NC field is READ-ONLY** — the card is an EDITOR whose fields address the CREDIT row. ✅ **Sam's dials:** factor **0.5 both lanes**, one **FTES rate both lanes**, shares **34/33/33**, `Annual funding`/`Combined funding` (was "Disbursement/Even tranches/Front-load"). ⚠️ **`undelivered` CONFLATES artifact-not-loaded with feed-does-not-carry** — test the artifact FIRST, mirroring `earnFraction()`, or every CREDIT card claims its measure is uncarried. ⚠️ **A BOUND IS TESTED BY VALUE, NOT THE CLAMP COUNT** — Santa Ana receives exactly $100,000 without being *held* there, so `capped` says 2 while 3 receive the max ([`a-bound-is-tested-by-value`](docs/kb-notes/methodology-a-bound-is-tested-by-value-not-by-the-clamp-count.md)). ⚠️ **Award range is TWO ROWS, never merged** (115 colleges vs 33 institutions, 30 in both); both bounds count ties. ✅ **Unearned funds are anchored to Ed. Code §78093.2(d)(1)** — a stated PRIORITY, never a mechanism we built; the model only *reports* what goes unearned. ⭐ **CAREER ATTAINMENT (goal C) IS CARRIED BY THE PROJECT POOL, reported qualitatively** (Sam) — and the pool is also goal **(D)** verbatim (*"the chancellor's office's pilot projects, such as the California Mapping Articulated Pathways Initiative"*). ⚠️ **The CPL story corpus evidences the WRONG goal** — of 36, **5** destinations name a job, ~4 are genuine progression, 8 quotes mention employment at all; it documents *educational* attainment. **Fixable at INTAKE, not analysis.** ⛔ **AND FIXING *WHO MAY WRITE* DID NOT RESCUE *WHAT WAS ALREADY WRITTEN* (#1371)** — everything edited while locked sits in the SCENARIO overlay, which **WINS THE RENDER**, so it paints back and looks published, and re-typing it fires no `change` event. **The promotion step already existed and only the TEAM-PHRASE unlock row reached it**; a magic-link reviewer stranded the overlay for ever. Now `promoteScenarioToShared()` reachable from both credentials, a standing **"⚠ This browser holds changes nobody else can see"** + Publish button, and an **expired** sign-in that says so rather than reading as ordinary exploring ([`fixing-who-may-write-does-not-rescue-what-was-already-written`](docs/kb-notes/methodology-fixing-who-may-write-does-not-rescue-what-was-already-written.md)). ⚠️ **STILL UNPROVEN END TO END: that a reviewer save reaches Supabase** — every layer is fixed in isolation, nothing has done the round trip in a browser. Sam's three relabels (`Access: Statewide` / `Access: Outreach` / `Completion`, source indices 2/0/1) are STILL only in his browser; **do NOT write them for him** — his call: *"I want the tab to save it."** **NEXT: ① Sam re-applies his relabels (the gate is fixed); ② the ABCD spine + goal-tagged project line items, with superscript ᴬᴮᶜᴰ links from whatever serves each (his ask, designed in the handoff); ③ the threshold/floor coupling — 400 FTES is the last feasible step at the $50k floor, still unruled; ④ the optional Combined award row.** Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) ([archive](docs/cpl_funding_lessons_archive.md)).
