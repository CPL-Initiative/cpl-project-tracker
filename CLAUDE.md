...
| **Excel→Supabase Phase 2-4** | Migrate remaining Excel-driven tabs (Dashboard project cards, Budget, Vision 2030, Personnel). Per-tab inline editors. Excel file retires once Phase 4 cuts over; periodic Supabase→xlsx export retained as backup. **Phase 2 (projects) build PR-1/PR-2/PR-3 + the live RLS tighten landed Session 15 (rows below); seed dispatch + PR-4/5/6 remaining.** Phases 3-5 (Budget/Vision/Personnel) follow the same shape. | Phase 2 build DONE 2026-05-28 (Session 15); seed + cutover pending; Phase 3-5 queued |
