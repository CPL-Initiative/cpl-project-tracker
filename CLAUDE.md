...
| **Implementation Funding tab / the $35M model** | The three priorities, their shares and factors, what each earns against, and the college-by-college allocation. | ✅ **LIVE, TWO LANES, ONE SOLVER.** ⚠️ **Baked defaults in `cpl_funding_data.js` are STALE by design** — the live model is the Supabase overlay (`cpl_funding_config`, `projects."cpl-implementation".scenarios."Scenario 1"`, camelCase; the reorder is a PERMUTATION stored BESIDE it, and `yearPriorities` is SPARSE — read it, do not assume the baked value). ✅ **SAM'S SAVED CONFIG (2026-08-23):** `priorityOrder [2,0,1]` · Access 34/Outreach 33/Success 33 · factors **0.5 in BOTH years** (the mirror; the stored Year-2 `1` is inert) · front-load ON · deadline 2026-11-01 · all metrics in CPL FTES · credit floor **$150,000** cap **$400,000** · carve-out **$1,800,000** · NC threshold **500 FTES**, floor **$50,000**, cap **$100,000**. Credit 55 of 115 floored / 5 capped; NC 30 of 33 floored. ⭐ **BOTH BOUNDS SOLVE TOGETHER** — `solveBounded()` bisects `lambda` in `clamp(lambda*size, floor, cap)` for BOTH lanes; `cpl_funding_cap.test.js` is the guard if you touch it. ⭐ **A CEILING CANNOT LIFT A FLOORED COLLEGE — the floor is the only real equity lever**, and it is a transfer priced in the earn rate of the middle. ⚠️ **A FLOOR THE POOL CANNOT HONOR SAYS SO** — `floorInfeasible` REPLACES the note in both lanes and on the explainer (#1302). ⚠️ **NEVER RE-DERIVE AN ALLOCATION — CALL THE MODEL (`_alloc()`, `ncModel()`), IN BOTH LANES** ([`a-total-that-balances-is-not-a-total-that-is-right`](docs/kb-notes/methodology-a-total-that-balances-is-not-a-total-that-is-right.md)). ⚠️ **NC money never sums into the credit total** — its own column on the tab, CSV, memo and My College (Sam: *"the neglected step child"*). ⚠️ **A DEDUP HAS A SCOPE**: Mt. SAC NC's FTES is the duplicate, the institution is not — it stays listed at `—`, with its reason. ⚠️ **The opt-in prompt keys on the GATE, never on the size of the withheld figure.** ⚠️ **Money figures in the explainer must carry an id** (live page AND frozen snapshot) or they are unpaintable and unlintable. ⭐ **THE EXPLAINER IS LIVE** at **`/funding-model/`**, off the engine + live config via `funding_model_payload.js`. ✅ **SAM RULED 2026-08-26: THE NC LANE EARNS LIKE CREDIT** — the NC award becomes a CAP earned against three targets (the CR three, origin-filtered), not a display split. ⭐ **ROUTE, DON'T SPLIT** — each unit earns in ONE lane by origin and still counts three times INSIDE it (eligible/applied/transcribed are MILESTONES); **`share` splits the MONEY, not the FTES**. ⚠️ **Year-1 P3's strategy list names *"noncredit mirror courses"*** — today a SINGLE dip in the WRONG lane; remove that line when NC goes live. ⚠️ **Leakage UNDERcounts NC rather than double-paying** (Sam: that is the incentive to use the NC landing page). ✅ **SAM'S DRAFT-MODEL POSTURE (2026-08-27): a missing upstream field is NOT a blocker and NOT a design flaw** — *"freedom to play to pay"*; build the mechanism, let the value show 0 until the data lands. ⚠️ **But `measurability()` matches a metric to its data by SUBSTRING-MATCHING ITS PROSE**, so all three NC metrics match the **CREDIT** sources and one naming the NC landing page matches `pp_u` FIRST — that is not a missing value, it is a **plausible wrong one that looks right**, which his same sentence rules out (*"values calculate correctly based on the available data"*). So it is the FIRST BUILD STEP, not a gate: **explicit per-priority `src` → NC earns → display.** ✅ **SAM RULED 2026-08-27: NC KEEPS FACTOR 0.5, same as credit** — no discount for the newer lane. Base rate **$12,423/CPL FTES**, so a floored NC institution needs **8.0 CPL FTES** over the window (capped: 16.1). **The binding constraint is ORIGIN CAPTURE, not difficulty**; revisit only with measured NC volume. ⚠️ Do NOT vary the factor *within* NC — credit is uniform 0.5 deliberately. ⚠️ **NC floor still Sam's:** $50,000 → 30 of 33 floored, break-even 3,909; **$60k INFEASIBLE**; dropping the threshold fails at this pool (parity is only +$406k → a $16,216 floor). Sweep: `scripts/funding_effective.js --nc-sweep`. ⭐ **NEVER READ THE CONFIG — CALL `_effective()`** (#1359): `mirrorYears` makes the stored Year-2 block INERT (says factor 1, model uses **0.5**). Story: [`docs/cpl_funding_lessons.md`](docs/cpl_funding_lessons.md) ([archive](docs/cpl_funding_lessons_archive.md)); durable notes indexed in [`docs/INDEX.md`](docs/INDEX.md). |
...
### SkyMatch S198 — the evidence a college does not keep (2026-08-27)
...
Story `docs/college_cr_evidence_lessons.md` · handoff `docs/session_199_handoff.md`.
