...
| **Title 5 §55050 → Ed. Code Article 9** | A regulation that does not implement the statute it operates under — and the amendment package that fixes it. | ✅ **PACKAGE DRAFTED, TRACKED-CHANGES FILE VERIFIED AND DELIVERED** (Sky196, #1339 · #1341–#1347). ⭐ **THE ASK WAS A FRESH REDRAFT AND THE ANSWER IS A RESTORE.** A final §55050 already exists — adopted by the BOG **2026-08-12** — and **its renumbering is Sam's own November 2025 work**, so it is not a competing structure, it is the same section with its substance missing. ⭐ **NOBODY ERRED**: the accessibility stamp reads **6/19/26**, Article 9 took effect **7/13/26**. Lead with that — it makes the package a conformity amendment, not a criticism. ⭐ **THE ARGUMENT IS ABOUT DISTRICTS**: §78093.2(b)(1) (evaluate ALL incoming students' documents) and (b)(2) (accept transcribed CPL from other campuses) bind every campus **today**, so a district following the adopted §55050 to the letter is still out of compliance with the Ed. Code. ⭐ **FILE TIER 1 ALONE** — Tier 1 asks *does Article 9 say this?* (five minutes with the statute); Tier 2 asks *is this good policy?* (a debate already had in November, which these lost). Bundling makes Tier 1 inherit Tier 2's burden and the easy move becomes declining the package. ⭐ **NAME YOUR OWN TWO STRETCHES** — *shall consider* in (c) sits on a permissive *may adopt*, and the new (n)'s second sentence adds *no secondary review* where the statute says only *accept*. ⚠️ **NOTHING RE-LETTERS**: adopted §55051(d) reads *"as defined in section **55050(i)**"* — a pointer the SAME rulemaking created, so it is invisible to a search of the operative text; new material appends as **(n)**. ⚠️ **THE ADOPTED PDF IS A REDLINE AND `pdftotext` DROPS THE FORMATTING** — 21 run-together collisions + 7 unmarked struck paragraphs; draft against the generated `t5_55050_clean_after_2026-08-12.txt`, never the extraction. ⚠️ **The same phrase resolves two ways in one section** (`course contentoutcomes` → (f) *outcomes*; identical phrase unstruck in (h) → *content*). ⭐ **REJECT-ALL MUST REPRODUCE THE ADOPTED TEXT** — `kb/_verify_55050_redline.py`, **71 checks**, 4 perturbations; it caught a deleted `" and"` that orphaned its comma (*"66700 70901"*). ✅ **§88782 is what the adopted NOTE carries**, so this action mirrors it — whether §88782 *is* Career Passport in the Ed. Code is a SEPARATE, unverified question and must not be folded in. **NEEDS SAM (none of it drafting):** ① the §88782 code check; ② Executive Sponsor / Staff Lead; ③ the 2026–27 timeline; ④ whether Tier 2 gets a companion action. **Separate, bigger opening:** §78093.2(d)(2) conditions every allocation on *"the metrics described in paragraph (1)"* and paragraph (1) supplies **none** — the Implementation Funding model already quantifies (A) and (B), (D) holds because MAP is the named pilot, **(C) career attainment is the gap**. Needs no rulemaking, which is its advantage. Story: [`docs/t5_55050_lessons.md`](docs/t5_55050_lessons.md); durable [`a-change-inherits-every-reference-into-it`](docs/kb-notes/methodology-a-change-inherits-every-reference-into-it.md) · [`resolve-a-redline-as-an-edit-list`](docs/kb-notes/methodology-resolve-a-redline-as-an-edit-list.md). |
