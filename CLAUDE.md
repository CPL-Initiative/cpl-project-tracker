...
| **EACR — Exhibit & CR Adoption** | One place to see every exhibit, its credit recommendations, and the colleges that could adopt it. | ✅ **LIVE — filters, matrix, CSV export** (Sky162 #1221–#1223 · Sky163 #1226 · Sky165 #1229 · #1230). **Three college scopes:** `adopted` (default) · `likely` (the prescriptive M-ID layer, which **names the local course**) · `any` (*a lead, not a match* — TOP-derived, so Rule 7 forbids it as a primary determination). **Matrix sub-tab:** CER titles down the side, colleges across the top, CR units in the cells — **green = adopted, brown = still available** (in parentheses, so the distinction survives greyscale). Default view **434 rows × 118 cols, 17.0% inked** (12.3% green / 4.7% brown; 59% of rows carry an opportunity); **337 partial-adopter gap cells**. Axis = **118 = 115 credit** (Sam's number exactly) **+ 3 noncredit**; the 4th, **Mt. SAC Noncredit, has no identity in `map_colleges` or the export** (Learning Partners item 1). ⭐ **SAM'S FOUR RULINGS, LOCKED:** brown is the **peer benchmark** · open on **colleges** (not regions) · default rows **≥2 adopters** · brown on **credible cells only** (the *likely* tier). ⚠️ **FILTER, COLUMN AND EXPORTS SHARE ONE SCOPE, STRUCTURALLY** — `matrixCell()` is ONE function called by both the grid and the CSV, so the spreadsheet cannot drift from the screen; the exports are the layer that reaches a college by email. ⚠️ **BROWN CANNOT BE THE LINE TOTAL** — 83% of adoptions are PARTIAL (median **3.07 of 9.26** lines) and no college has ever reached the total. ⚠️ **A content filter must never drop a column** — that reads as *"this college has nothing"*; narrow columns under college-shaped filters only. ⚠️ **Roster rules (`kb/reference/map_college_roster_rules.json`) are the ONE place identity folds belong** — sandbox `entity_kind='test'` dropped, duplicate spellings folded as a **SUM** (never a pick). **NEXT: Sam looks at the grid in a browser** — density is his call. Then the Cañada tilde, then fix the mojibake at source in `_build_statewide_prescriptive()`. **Curation carryover:** 4 unclassified-only titles the CER knows · 2 statewide cards matching no college · sweep `{0,N}` test bounds · the 50-group credential-view cap. Full history (the label-layer fold, the Sky164/165 measurement disagreement, the units-source finding): [`docs/eacr_scope_lessons.md`](docs/eacr_scope_lessons.md). |
...
| **MAP Custom Reports (3 new) / ITPI automation** | Wire the three new MAP Custom Reports — exhibit credit recommendations, the same **by Catalog Year**, and student details — and decide how they stay fresh. | 🔨 **NEXT SESSION'S PRIORITY. Nothing fetched yet.** Sam, 2026-08-19: the three reports were generated from the Access DB steps and queries worked through with earlier sessions. Per his screenshot: **College Exhibit Credit Recommendations (11 fields)** · **…By Catalog Year (13 fields** — CollegeID, Source Code, ExhibitID, Credit Recommendation, College Course, +8) · **Student Details and Credits (30 fields)**. ⭐ **CATALOG YEAR is the genuinely new dimension** — nothing we hold carries it, and it is what pins an articulation to the catalog it was approved under rather than letting it float. ⭐ **THE FIRST JOB IS A RECONCILIATION, NOT A LOAD** — two of the three overlap tables we already have (`map_student_credit` 537,908 rows; the exhibit/CR surfaces), so the question is whether the new extract AGREES with Supabase and, where it disagrees, which is right. ⚠️ Relevant prior: `cpl_memory` `two-student-counts-disagree-indicator-suspected` — our extract is stale because the MAP team was pulling records off MAP to correct Exhibit references and reload them, so **a disagreement may be OUR staleness resolving**, the expected outcome, and should be confirmed rather than filed as a defect. ⭐ **ITPI OFFERED A DAILY PUSH INTO SUPABASE; THE RECOMMENDATION IS TO DECLINE IT** — `fetch_custom_report.py` already POSTs to `mapwebapinew.azurewebsites.net/api/CustomReport/getReport` with `MAP_API_KEY` and pulls **EIGHT** datasets by `viewName` on the daily cron. Three more is **three entries in `REQUEST_PAYLOAD`** — a config change, not an integration. A push would invert the trust direction (MAP is READ-ONLY for us), need a credential we must issue (and the only one that trivially works is the **service key, which bypasses ALL RLS**), make failures invisible (ours surface in Actions and are versioned), and create a **second writer**. If a push ever IS needed on volume: dedicated Postgres role, INSERT-only on NAMED STAGING tables, never the service key, validated promotion we own. ⚠️ **ESTABLISH FIRST:** Sam pointed at `customreportingmodule.azurewebsites.net` (the report BUILDER UI) while our fetcher consumes `mapwebapinew.azurewebsites.net` (the API) — whether the three are exposed on the existing endpoint decides whether this is 20 lines or a real integration. **BLOCKED ON PEDRO:** the three `viewName` values + column lists. |
