...
- **Tests** (jsdom, `npm test` — 96/96 files pass): `raci.test.js` updated (📣 now on a row with no R/A; opt-out asserted via null href); `raci_nudge_optout.test.js` rewritten (button on every signed-in row; opt-out → null href, mailto targets the opted-in member); new `raci_card_nudge.test.js` (per-card deep-link consumer + "Nudge All" rename + anon behavior — no bulk/per-row 📣 signed out, deep-link still routes). New exports on `window.CPL_RACI_TAB`: `_openItemNudge`, `_consume`, `_itemByKey`.
