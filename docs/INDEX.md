...
| KB notes | 372 | [`catalog/kb-notes.md`](catalog/kb-notes.md) |
...
| Session handoffs | 199 | [`catalog/session-handoffs.md`](catalog/session-handoffs.md) |
| **total** | **767** | |
<!-- /generated:corpus -->
...
## Update history
- **2026-09-03 (SkyTune, S224 — the chip is live, both re-mint dry runs are receipts, fourteen readings wait on Sam)** — The queue's head was the re-mint series Sam ruled the evening before, and the session ran it the way the playbook asks: measure first. **#1447** (merged) shipped item 19's C-ID chip — `kb/_seed_authority_codes.py` attributes every C-ID / CCN subject code to a discipline from the promotions evidence plus the rulings and the CSR tab, the CCR Subject list and SkyView show `C-ID AJ` beside `CRIM` (12 disciplines on a CCN code, 14 on a C-ID code, 120 proposed, 29 with a chip). **#1448** is the recode dry run: a scratch run showed the June allocator would move 62,638 of 70,946 ids to change nothing, so the ruled code changes are planned as a **keep-number prefix re-key** (10,292 ids, 10,039 keeping their number; the languages by rule, the agriculture families by two agreeing signals). **#1449** is the Z-band retirement dry run, computed after it: 4,053 Z identities take free M numbers (3,836 Z numbers were already M numbers), 218 legacy anchors fold, and **Kinesiology credit lands at 996 of 999**. Fourteen readings for Sam on one sheet ([`visuals/2026-09-03-remint-series-readings`](visuals/2026-09-03-remint-series-readings.html)); nothing applied. 1 KB note ([`kb-notes/methodology-a-code-change-is-a-prefix-rekey-not-a-resequence`](kb-notes/methodology-a-code-change-is-a-prefix-rekey-not-a-resequence.md)), new suites `authority_codes_seed_test.py` (17), `authority_recode_dryrun_test.py` (20), `zband_retire_dryrun_test.py` (17), `csr_authority_chip.test.js` (18), `ccr_subject_authority_label.test.js` (10).
