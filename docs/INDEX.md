...
created: 2026-05-27
updated: 2026-06-23 (Session 70 — PaintSky: the Pending-merges panel + the mint→Common SUBJ preview + the reviewer-gated kb_curation DELETE policy, PR #500, re-landing the empty-squashed #499; NEW kb-notes reference-common-vs-local-subj-and-discipline-cardinality + methodology-stacked-pr-empty-squash + session_71_handoff)
...
| 70 → 71 | PaintSky: the Pending-merges tracking panel (target ← members + per-member/group Undo) + the mint→Common SUBJ preview + the reviewer-gated kb_curation DELETE policy #500 (re-landing the empty-squashed #499; NEW methodology-stacked-pr-empty-squash) → the unverified-M-ID renumber re-mint (when merges settle) + the TMC acceptance engine; bonus: more First Light CA plein-air landscapes | [`session_71_handoff.md`](session_71_handoff.md) |
