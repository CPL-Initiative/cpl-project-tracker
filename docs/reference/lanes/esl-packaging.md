...
## ⚠️ Open — the band guards test the SUPERSEDED path (found 2026-08-29, Session 208)

`tests/esl_relevel_bands_test.py` imports **`_esl_relevel_dryrun`**, whose own
docstring says it implements the absolute bands that Sam **superseded the same
day** he authored them (`cpl_memory`: `sam-esl-per-ladder-level-sets`). The live
script — **`kb/_esl_ladder_relevel_dryrun.py`**, which implements the per-ladder
sets and holds the purpose-carve-out skip — has **no test referencing it at
all** (verified: `grep -rln _esl_ladder_relevel_dryrun tests/` returns nothing).

So the band assertions guard a path nothing runs, and the path that actually
produces re-levels is unguarded. **NEXT:** point the guards at the ladder script,
or add a parallel suite for it; keep the existing pair of assertions that made
the old one honest (*"no purpose carve-out was re-banded"* **and** *"carve-outs
were actually seen, so the guard can fire"*).

⭐ Found incidentally by a doctrine probe whose own run was void — the probe was
verifying a premise it had been handed, which is how it noticed. Worth keeping in
mind when judging what a contaminated run is worth.
