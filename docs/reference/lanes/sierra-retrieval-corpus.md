...
✅ **`chatbox_credentials` LIVE (1,987 rows)** — public-read/no-write, loaded by `kb/_sync_credential_catalog.py` from the PUBLISHED artifact so suppression is inherited by construction. Routes CRED·STD, CRED·VOLUME, COLLEGE·ADOPT, ALIGN live. ✅ **`chatbox_credential_recs` — 2,205 rows LIVE** (134 statewide/351 lines · 2,071 local/3,357) on the nightly `credential-catalog-sync`. ⭐ **Sam's rule:** statewide exists → quote the **statewide set ONLY**; no statewide → the **most common** local recs with their college counts. Never both. ⭐ **The builder REUSES `fact-sheet/_build_statewide_recs.py`** — Sierra quoting different credit from the Fact Sheet is a credibility failure. ⚠️ **Lead with the LIST, never a count:** POST measures **10 lines · 9 carrying a C-ID · 8 DISTINCT · 1 with none**, and the `AJ 110` repeat is **flagged, never auto-resolved** (Sam: *"AJ 110 may be C-ID and it is Elective"*). **Standing retrieval rules, each earned by a failing probe:** search is **TRIGRAM, never `tsquery`** (`to_tsquery('english','aed:*')` → `'a':*` took the CPR corpus out); score the **best single name**, never the concatenation (length-normalized similarity ranks the BEST-CURATED record WORST); **`statewide` is a FILTER, not a tie-break**; **no pure-fuzzy** (tier-4 floor 0.25 + `matched_via`); **zero rows is a RESULT**, not a license to offer a neighbour. ⚠️ **Every student count is a FLOOR and the denominator ships as a COLUMN** — only 4.2% of student rows are nameable; `students_suppressed=true` must never render like `colleges_with_student_data=0`. ⚠️ **The statewide-rec gate is `ccc_rec` OR a published statewide set** — `ccc_rec` is derived from ADOPTIONS, so gating on it alone hid **38 statewide credentials with zero adopters, 36 of them carrying 75 published rec lines** (Carpenters ladder, NCCER, CSLB, ICC, OSHA 10/30) from *every* credential route. ⚠️ **Rec lines are ENRICHMENT, never a filter** — the map is declared OUTSIDE the try and a credential with no line is **still named**; dropping it re-creates the false zero. Every credential route renders through the **shared** `renderRecLines` off **one** batched `credential_recs_for_titles()` — a second lookup is a second matcher that can drift. ✅ **THE `surface` FIELD HAS BEEN LIVE SINCE 2026-08-22 (v56):** `KNOWN_SURFACES` (8) on the request, `sierra_guidance.surface` (⚠️ the live CHECK still lacks `skyview-ask`, so a curator picking it gets a hard save failure — one statement, NEEDS SAM), the Training-tab picker; **2 of the 7 active rows are scoped `my-college`** (`15ec666b`, `7d8641be`), 5 ship everywhere. ✅ **DEPLOYED 2026-09-17 as cpl-chat v66 (#1568); the five days it sat unshipped were an INVISIBLE OUTAGE.** The page ships on merge, the function only on a `cpl-chat-deploy.yml` dispatch — so from 09-12 a widget sending `x-team-pass` met a v65 preflight lacking it and browsers refused the whole request; the probe passed ~40 times because curl never preflights — it preflights now (#1595). `deriveViewer()` derives `reviewer` · `team` · `public` by calling `is_allowed_reviewer()` / `team_pass_ok()` through PostgREST with the ANON key and the caller's own credential (JWT bearer / `x-team-pass`; CORS admits the header), fails closed to `public`, files `viewer` + `surface` on `chat_interactions` and echoes `event: meta`, which the COBI widget renders as one line of words. The two COBI callers send what they hold; the public page and the Fact Sheet stay on the anon key. Guard: `tests/sierra_viewer.test.js` (63). ⚠️ **IT WIDENS NOTHING:** no prompt line, no retrieval change — letting a verified reviewer see COBI data is the SECOND build (the boundary inside COBI is aggregate vs student-detail), routed through Governance and the student-detail disclosure ADR (Rule 10 a3) first. **Open:** corpus covers **59 of 123** colleges; `chatbox_college_profiles` stale since 2026-06-25 **except contacts** (live — see the MAP Users row); ⚠️ its **`credit_distribution` column is no longer read by anyone** — it was Sierra's per-college credit source until 2026-08-24 and had drifted two months (#1325, see the My College row); 12 adoption-file statewide titles absent from `chatbox_credentials`; ✅ **Sierra Training queue CLEARED by Sam 2026-08-26 — 0 still to do, 51 of 51 handled, 7 instructions in use** (screenshot; unblocks the alignment feedback loop). **NEXT:** Sam reads the actual prose — no session has, the sandbox is egress-blocked from `*.supabase.co`. ✅ **PROGRAM SEARCH IS LIVE (S272)** — the third view of a college, what it AWARDS; `coci_college_programs` (22,335 rows / 118 colleges) was read by nothing. Schema of record and every measurement: `chatbox/supabase_search_college_programs.sql`; its `verify_` companion passes A 8/8 · B 2/2 · C 4/4 · D 2/2 live. Matches program TITLE **and** code with **neither gating**, reporting `matched_via` — a code-only row renders in its own labeled bucket, because a code names the field and cannot say who a program is FOR. **CIP loaded, never gated** (19,349 of 22,335 rows carry one). ⭐ **"LVN" needed PHRASES, not a synonym** — single tokens either overshoot (*vocational*, 82 colleges) or prefix-match junk (*practical* stems to `'practic':*` and pulls in PRACTICE/PRACTICUM); the two phrases through `phraseto_tsquery` give **56 colleges and 0 noise**, up from 28. ⚠️ **NEVER INDEX `coci_college_programs`** — three GIN indexes timed out `coci_programs_replace` (57014) for a measured **4 ms**. ✅ **DEPLOYED 2026-09-17 23:31Z as cpl-chat v67** (#1601 + #1603; Sam: *"apply and deploy...I'm OK with a few anomalies... We can hone as we go"*). ⚠️ **The first A/B run (35275472821) passed every mode on BOTH slugs while `search_college_programs` timed out 3× underneath** — per-term DF scans at ~320 ms each, 4.3 s for 6 terms, 19.8 s for 30, and `Promise.all` waits for the slowest route; a fail-safe route drops its section silently, so **READ `function_logs` AFTER EVERY A/B, NOT ONLY THE GRID**. The one-pass rewrite (vectors once per call; identical rows on nine term sets; 0.9–2.6 s) is **APPLIED** (migration `search_college_programs_one_pass`; A 8/8 · B 2/2 · C 4/4 · D 2/2 live) and A/B run 2 (35285864076) was clean in grid AND logs. **NEXT:** read v67's real questions and logs; Sam reads program answers in a browser; a client-side time limit on every retrieval RPC; generated tsvector columns only as a MEASURED loader-side option. **NEEDS SAM:** auto-deploy on merge (recommended, unruled). Guards: `tests/sierra_program_search.test.js` (37) · `tests/coci_program_cip_test.py` (20) · smoke **7p**. Story: `docs/sierra_credit_recs_lessons.md` · `docs/sierra_credential_naming_lessons.md` · `docs/cpl_assistant_lessons.md`.
...
ACCOUNT pays** — the Supabase secret *named* `ANTHROPIC_API_KEY` and a Console key
*displayed as* `ANTHROPIC_API_KEY` are different namespaces; filter the Console by key.
