...
3. **Supabase grants before 2026-10-30.** #1664 merged (e8b3582) and the daily run was dispatched. The grants
   ...
   `tests/supabase_table_grants_test.py` guards every created table (49 live ones baselined). ⏭ **The live apply
   waits on Sam's OK** (apply_migration prompts by design): only the five `create or replace function` statements,
   never whole files, then `has_table_privilege` after the next promotion. Detail:
