...
### Decisions and scope Ashley set this run (2026-08-19)

Recorded because the narrative loses them. Ashley (SJCOE) drove this session; Sam
was not in it.

- **Scope the crosswalk to ONE college.** Explicit: *"narrow the focus specifically
  to what San Joaquin Delta College currently has available."* That instruction is
  what made this a separate instrument rather than a filter on the statewide engine.
- **Reuse the prior methodology and structure**, not a fresh start — *"use the same
  general methodology and spreadsheet structure"*. So the occupation→credential
  rulings were reused verbatim and the workbook keeps the partner run's shape.
- **Three explicit alignment categories**, her wording: existing/confirmed ·
  potential needing validation · no apparent alignment. These became the `fit`
  axis; MAP exhibit status was kept as a SEPARATE column rather than folded in,
  which is the design decision that produced the 42-row adopt-now cell.
- **"Practical and easy to use for an upcoming conversation... rather than simply
  creating a broad list of programs."** This is why there is a Priority
  Opportunities tab at all, and why priority is derived from (fit × exhibit status)
  rather than being a list of everything Delta offers.
- **Purpose stated as building exhibits, not just reporting**: *"identify
  opportunities to expand their CPL offerings, and build CPL exhibits into MAP for
  applicable occupations if those exhibits do not already exist."* The
  build-first-in-state tier exists to serve that second half.
- **She asked for an HTML visual mid-run**, alongside the spreadsheet. Both were
  delivered; the HTML is generated by the same tool so the two cannot drift.

Not decided, and open for her: whether this becomes a COBI tab. Offered at the end
of the session (it is a snapshot today, and the numbers will move as Delta adopts);
no answer yet. Sam separately authorized a partner-crosswalk tab back in August, but
scoped to the **regional-capacity** view, not the judgment-based occupation matching
— that constraint would apply here too.
