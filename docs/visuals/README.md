...
| 2026-09-07 | [`2026-09-07-skyview-globe-prototype.html`](2026-09-07-skyview-globe-prototype.html) | SkyView's committed layout wrapped onto a rotating three.js sphere — Sam's *"3-d 360 globe that rotates"* ask, built to look at rather than argue about: an Outside and an Inside view, the Articulations light, and a line counting the points that face the reader. The data is a build-time snapshot; nothing in the map's own code changed. The one visual with a second external host (three.js from cdnjs), because 49,896 points on a sphere need WebGL. | 🔨 **Second round the same day**, on Sam's reaction (*"I like the globe view! Love the glow on the articulations"* and five asks): islands eased apart into the empty sky and sized to fit (Spread | Committed), Round | Wrapped islands side by side, the M-ID dots white with color chips, a slower turn, and an all-sky Inside view — the sky as a disc from 30° to 360° across, stars a pixel or two wide. Awaiting his eye on the second round (NEEDS SAM ⑧ in the SkyView lane). Artifact: https://claude.ai/code/artifact/51f5249d-1884-406d-889e-259b262b86e5 |
