...
✅ **THE PIN VERIFICATION IS CLOSED — don't redo it.** S264 flagged that every pin was read off the screen while Supabase was in maintenance. The table was read after the window closed and **agrees exactly**: Outreach `ppa_u`, Awards `pac_u`, Completion unpinned and prose-deriving `p3_u`. ⚠️ **Querying it: the path is NOT `config->'priorities'`** (that returns empty rows and looks like an empty table). It is `config -> projects -> <project> -> scenarios -> <scenario> -> yearPriorities -> <year> -> <slot>`, and the year and slot levels are OBJECTS keyed "0"/"1"/"2" — `jsonb_array_elements` fails there, use `jsonb_each`.
