...
--
-- Schema captured from the MAP Builder + a value-signature probe (PR #618):
-- the API columns are College, CollegeId, FirstName, LastName, Email, RoleName,
-- UserName (case-sensitive). RoleName ∈ {Ambassador, Articulation Officer,
-- CALVET, Faculty, Implementation, Initiator, Student Intake Aide}. There is NO
-- last-login field in the view, so "stale" is tracked by OUR synced_at.
--
...
returns table (college text, user_count bigint, role_mix jsonb, last_synced timestamptz)
