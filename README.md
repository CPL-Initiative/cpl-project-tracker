...
8. **A public funding-model explainer — the public view** — a plain-language
   ...
   Since 2026-09-02 its **Every institution** section, directly after the
   introduction, is the tab's own institution table rendered in embed mode
   (`window.CPL_FUNDING_EMBED = "college"`) — the same rows, drill-in, search
   and Excel export, not a copy — and the steps that explain the mechanics
   fold closed on open; the timing milestones and each priority's recommended
   strategies paint from the engine too.
