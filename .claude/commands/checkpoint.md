...
Then commit all four (or however many actually changed) in **one commit** with a `Rule 8 checkpoint: <one-line summary>` subject. Push to the current branch.

Before starting, briefly state what you're going to update and why — don't bombard with detailed plan, just one or two sentences. After committing + pushing, give a tight summary (which files changed, the new commit SHA, what the next session will pick up).

If the user says "skip kb/README" or "skip README" etc., honor that; otherwise default is update-everything-that-needs-it.
