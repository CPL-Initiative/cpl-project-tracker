...
| `_seed_coci_unified_courses.py` | Course | One-shot generator for the `coci_unified_courses.json` variant-unification clusters (`UC-XXXXX`). **Output DISSOLVED 2026-05-30** — the token-sorted title key collapsed distinct course levels (e.g. "Algebra 1: Part 2" == "Algebra 2: Part 1"), the clusters were never curator-reviewed, double-emitted their members as Stand-Alone rows, and carried zero articulations. The level-safe **Suggested-merges worklist** supersedes it. `clusters` dict now empty (archived at `archive/coci_unified_courses_clusters_2026-05-30_pre-dissolution.json`). The only `id_system: Cluster` rows in the CCR now come from curator `merge_into` targets (`UC-CUR-*` + folded M-IDs). **Do not re-run.** | — |
