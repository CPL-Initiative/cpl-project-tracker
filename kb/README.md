...
| `_desc_consolidation_dryrun.py` | Course | **Re-runnable description-similarity consolidator for DARK M-IDs** (no C-ID/CCN claim, no c-id.net coverage, no promotions, no curation — 13.9k of 16.1k post-#379). TF-IDF cosine over catalog descriptions; gates = cosine ≥0.60 + credit + units(±0.5) + not-the-title-lane's + **level/gender/sport** safety. Receipt `desc_consolidation_out/candidates.json` (474 groups, 135 cross-college) — **committed input** the generator joins into the worklist as `desc_groups`; re-run termly like the c-id.net refresh. Never auto-applies. | — |
