...
(function(){
  ...
  // ⚠️ `hw` IS PASSED IN WHERE THE MARK MUST PREDATE THE CALLER'S OWN WRITES.
  // Complete stores a reply for every as-proposed item, so a line built after
  // that loop reports the whole sheet as reviewed no matter where the reader
  // stopped. Measured live on 2026-09-22: a sheet completed with nothing ruled
  // said "0 of 12 items your own call" and "reviewed through 12 (the whole
  ...
  function line(hw){
    ...
    var h = hw || highWater();
    if (h.mark) text += " — reviewed through " + h.mark
      + (h.whole ? " (the whole sheet)." : "; the items after it were not reached.");
    return text;
  }
  ...
  if (sbtn) sbtn.addEventListener("click", function(){
    ...
    var hw0 = highWater(), through = hw0.mark;
    ...
    var text = "Decisions done on " + SHEET + " — " + n.done + " of " + n.total + " items your own call"
      ...
      + (n.blank ? ", " + n.blank + " left blank (no verdict on those)" : "") + ". " + line(hw0);
