...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* 100dvh, not 100vh: on iOS/Android `vh` is sized to the viewport WITHOUT the
     browser chrome, so a 100vh app column is taller than the screen and the
     footer sits under the address bar. The vh line stays first as the fallback
  ...
  /* The three layer toggles measured 75x19 — the checkbox itself is 13x13 and
     the label is what you actually press, so 19px tall was the real target and
     it is under the WCAG 2.2 AA 24x24 floor. Padding, not a bigger checkbox. */
  label.toggle{display:flex;align-items:center;gap:7px;min-height:24px;padding:2px 0;cursor:pointer}
  ...
  /* One visible focus ring everywhere. :focus-visible so a mouse click on a
     marker or a list row does not leave a ring behind after the pointer moves
     on. SVG takes `outline` in every current browser. */
  .btn:focus-visible, .tab:focus-visible, .search:focus-visible, a:focus-visible,
  label.toggle:focus-within, .dirlist li:focus-visible, .mk:focus-visible{
    outline:3px solid var(--co-blue);outline-offset:2px;border-radius:6px}
  .mk:focus-visible{outline-offset:0}
  /* Skip link — off-screen until focused. */
  .skip{position:absolute;left:8px;top:-60px;z-index:60;background:#fff;color:var(--dk-blue);
        border:2px solid var(--co-blue);border-radius:8px;padding:9px 14px;
        font-size:13px;font-weight:700;text-decoration:none;transition:top .12s ease-in}
  .skip:focus{top:8px}
  /* A directory row is operated by script, so it has to advertise itself as a
     control: focusable, named, and driven by Enter/Space as well as a click. */
  .dirlist li:focus-visible{background:#f1f6fc}
  /* floating tooltip */
  ...
  /* ⚠️ This breakpoint used to read `#side{flex-basis:0;display:none}` — on any
     phone the ENTIRE side panel was removed: the Details pane a marker click
     writes into, both directories, both searches, and every CPL landing-page
     link. Tapping a college still selected it and still rendered its detail,
     into a panel that was not on the page. The map remained, so nothing looked
     broken; the page had simply stopped being able to answer the question it
     exists to answer.

     Below 760px the two panes STACK instead. The map keeps the top 52%% (enough
     to stay legible and pannable) and the panel takes the rest with its own
     scroll, border-top replacing border-left. selectCollege/selectBase scroll
     the panel into view, so a tap on a marker still shows its result. */
  @media (max-width:760px){
    #main{flex-direction:column}
    #stage{flex:1 1 52%%;min-height:210px}
    #side{flex:1 1 48%%;max-width:none;min-height:150px;
  ...
  <div id="main">
    <div id="stage">
      ...
      <div class="ctl tr zoombtns">
        <button class="btn" id="zin" type="button" title="Zoom in" aria-label="Zoom in"><span aria-hidden="true">+</span></button>
        <button class="btn" id="zout" type="button" title="Zoom out" aria-label="Zoom out"><span aria-hidden="true">&minus;</span></button>
        <button class="btn" id="zreset" type="button" title="Reset view" style="font-size:12px;font-weight:700">Reset</button>
      </div>

      <div class="ctl bl card regions" id="regions"></div>
    </div>

    <aside id="side" tabindex="-1" aria-label="Details, colleges and installations">
      <!-- aria-pressed + aria-controls, NOT role="tab". A real tablist promises
           arrow-key roving focus, which these do not implement; describing them
           as toggle buttons that control a panel is what they actually are. -->
      <div class="tabs" role="group" aria-label="Choose what the panel shows">
        <button class="tab on" data-tab="detail" type="button"
                aria-pressed="true" aria-controls="panel-detail">Details</button>
        <button class="tab" data-tab="colleges" type="button"
                aria-pressed="false" aria-controls="panel-colleges">Colleges</button>
        <button class="tab" data-tab="bases" type="button"
                aria-pressed="false" aria-controls="panel-bases">Bases</button>
      </div>
...
/* The hover tooltip is built from a MouseEvent, so a keyboard user got nothing.
   Same content, positioned from the marker's own box. */
function tipHtml(kind, obj){
  if(kind==="col"){
    const vet = obj.m==null? "" : `<div class="b" style="color:var(--navy)">&#127894; <b>${num(obj.m)}</b> veterans &amp; service members served via CPL</div>`;
    return `<div class="t">${escapeHtml(obj.n)}</div><div class="c">Credit for Prior Learning</div>${vet}`;
  }
  const demo = pairOf[obj.n]? ` &middot; demo with ${escapeHtml(pairOf[obj.n].join(', '))}`:'';
  return `<div class="t" style="color:var(--crimson)">${escapeHtml(obj.n)}</div><div class="b">Military installation${demo}</div>`;
}
function showTipAt(x, y, kind, obj){
  tip.innerHTML = tipHtml(kind, obj);
  tip.style.display="block";
  tip.style.left = Math.max(6, Math.min(x+12, window.innerWidth - tip.offsetWidth - 6)) + "px";
...
// ---------- marker events ----------
/* Every marker was mouse-only: a <g> is not focusable and the only handler was
   "click". So the map's entire content — which colleges partner with which
   installation, and every CPL landing-page link — could not be reached with a
   keyboard at all (WCAG 2.1.1). tabindex makes each marker a stop, role+label
   say what it is, and Enter/Space do what a click does. The panel scrolls into
   view on selection, which is also what makes the stacked phone layout work. */
function activate(kind, obj){
  hideTip();
  if(kind==="col") selectCollege(obj.n); else selectBase(obj.n);
  const side = document.getElementById("side");
  if(side && window.matchMedia && window.matchMedia("(max-width:760px)").matches){
...
function bindEvents(g, kind){
  ...
  g.setAttribute("aria-label", kind==="col"
    ? obj.n + " — community college. Show details and CPL page."
    : obj.n + " — military installation. Show partner colleges.");
  g.addEventListener("keydown", ev=>{
    if(ev.key==="Enter" || ev.key===" " || ev.key==="Spacebar"){
      ev.preventDefault(); ev.stopPropagation(); activate(kind, obj);
    }
  });
  // Focus is the keyboard's equivalent of hover, so it gets the same tooltip.
  g.addEventListener("focus", ()=>{
    const b=g.getBoundingClientRect();
    showTipAt(b.left+b.width/2, b.top, kind, obj);
  });
...
function switchTab(name){
  document.querySelectorAll("#side .tab").forEach(t=>{
    ...
    t.setAttribute("aria-pressed", on ? "true" : "false");   // the class alone tells assistive tech nothing
  });
...
function buildColList(filter){
  ...
  listCol.querySelectorAll("li").forEach(li=>{
    li.onclick=()=>selectCollege(li.dataset.n);
    li.tabIndex=0; li.setAttribute("role","button");
    li.setAttribute("aria-label", li.dataset.n + " — show details");
    li.onkeydown=ev=>{ if(ev.key==="Enter"||ev.key===" "){ ev.preventDefault(); selectCollege(li.dataset.n); } };
  });
}
function buildBaseList(filter){
  ...
  listBase.querySelectorAll("li").forEach(li=>{
    li.onclick=()=>selectBase(li.dataset.n);
    li.tabIndex=0; li.setAttribute("role","button");
    li.setAttribute("aria-label", li.dataset.n + " — show partner colleges");
    li.onkeydown=ev=>{ if(ev.key==="Enter"||ev.key===" "){ ev.preventDefault(); selectBase(li.dataset.n); } };
  });
}
